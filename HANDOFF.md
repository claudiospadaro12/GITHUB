...
3. 🟢 **Campagna matrice** motore×simbolo×TF → **CODA FASCIA B LANCIATA la notte del 10/08** (decisione di Claudio: "PARTIAMO STANOTTE, ORA" — non si aspetta venerdì): 48 lavori in `prove/CODA.csv` (SuperWave, SupertrendInvert, PTE, WOL, Nightly, FiboH4_Multi × 8 simboli), driver `coda_weekend.ps1` sul PC di backtest. Stadio 1 OHLC = solo screening; stadio 2 tick reali automatico per chi ha una cella >0 in ENTRAMBE le finestre. Stato vivo (se il token c'è): `report/STATO_CODA_WEEKEND.md`. Ripresa dopo stop: stesso comando, i CSV fatti non si rifanno. (Nightly: bocciato in FASE 0 su EURUSD — "si deve guadagnare il posto in squadra".)

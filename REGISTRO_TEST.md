...
- 🟢 **Pronte con PASS dello strato 2 (24/09)**: righe `RIGA_R243_SUPREV_ANCORADOW.txt` (pin `7e255a82`), `RIGA_R244_MAXMIN_RIEMPIMENTO.txt` (pin `223dc654`, raccoglie il PERTRADE per k), `RIGA_PASSATA_STOP_SUPREV.txt` (ripinnata `7e255a82`). 🔴 `RIGA_R214EF_COSTTOCOST.txt`: **FAIL** dello strato 2 — la colonna FTMO usa il confine fisso alle 23 BCM, ma l'orologio BCM **non sembra avere offset costante** con l'Italia (classe 762). Patch pronta nello scratchpad del cancello, **ferma finche' `report/OROLOGIO_BCM_2026-09-24.md` non data il cambio d'orologio**. Classi 749-752.

...
- **01/08:** VPS confermato da Claudio. **Squadra forward core 8/8 ATTACCATA** (5 SupRev: XAUUSD/XAGUSD/D30EUR/225JPY H4 + NASUSD H1; 3 GoldenCross: USDCHF/USDCAD/NZDUSD H4).
  ...
  - 🔴 **CONFERMATO 01/08 da screenshot: il VPS gira l'`.ex5` VECCHIO** (manca il gruppo "Filtro VWAP" nell'`_Ottimizzato` DAX apertura → mancano anche fix Hedge-safe + anti-duplicato). **RICOMPILAZIONE NON FATTA.** → Lanciare `scarica_ottimizzati.ps1` (branch default `creating-agents-SgGpD`, che HA il codice giusto) + `abtg_news.csv` in MQL5/Files. Finché non ricompila: rischio bug +804→neg (flotta piena su Hedge) e **forward NON valido** (pagella parte dalla ricompilazione). Ri-verifica F7: deve comparire "Opera solo dal lato giusto della VWAP di sessione".

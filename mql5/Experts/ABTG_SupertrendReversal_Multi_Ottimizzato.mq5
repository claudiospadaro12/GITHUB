//+------------------------------------------------------------------+
//|                                 ABTG_SupertrendReversal_Multi.mq5 |
//|                                                                  |
//|  EA "SUPERTREND REVERSAL - MULTI (ORO/INDICI/FOREX)"            |
//|  MT5 - VERSIONE TUTTO-IN-UNO                                    |
//|  (metti in MQL5\Experts e compila con F7: niente cartelle)      |
//|                                                                  |
//|  Versione della Supertrend Reversal con distanze in ATR invece  |
//|  che in pip: si adatta da sola alla scala dello strumento,      |
//|  quindi va bene su ORO e INDICI oltre che sul forex.           |
...
input int    InpStAtrPeriod   = 12;         // OTT XAUUSD H4 (era 10)
...
input double InpPendingAtr      = 0.5;  // distanza del pendente dal 1o ingresso, in ATR (scala-indipendente)
...
input double InpSLBufferAtr     = 0.2;  // buffer extra sullo SL, in ATR (scala-indipendente)
...
input double InpTP_RR         = 3.0;    // OTT XAUUSD H4 (era 2.0)
...
input group "=== Rischio ==="
input double InpRiskPercent   = 2.0;    // OTT: rischio 2% (PF 3.17). DD backtest ~5% -> occhio concentrazione oro
...
input string InpComment   = "STREV MULTI OTT";
input long   InpMagic     = 971001;
...
void Enter(bool isLong,double stLine)
  {
   ...
   if(!ABTG_GuardiaIngresso(InpUsaGuardian,"ABTG_SupertrendReversal_Multi_Ottimizzato")) return;

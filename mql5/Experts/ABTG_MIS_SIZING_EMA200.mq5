...
//|                                                                   |
//|  A COSA SERVE, in una riga: misurare DI QUANTI LOTTI cambia il    |
//|  volume piazzato quando si passa dal sizing del binario IN CAMPO  |
//|  (344a11b9, tick value nudo) a quello del pin che stiamo per      |
//|  compilare (26a18566, OrderCalcProfit).                           |
//|                                                                   |
//|  COM'E' FATTO, e perche' cosi':                                   |
//|  e' ABTG_EMA200.mq5 al pin 26a18566, COPIATO SENZA TOCCARE        |
//|  NIENTE, con TRE sole aggiunte:                                   |
//|    1. l'input InpSizingVecchio (default false);                   |
//|    2. dentro LotByRisk(), il ramo che riproduce ALLA LETTERA il   |
//|       calcolo di 344a11b9 quando l'input e' true;                 |
//|    3. il nome del file per-trade, che adesso porta dentro il      |
//|       ramo di sizing, il bilancio iniziale e la data d'inizio.    |
//|                                                                   |
//|  PERCHE' UN EA SOLO CON UN INTERRUTTORE E NON DUE EA:             |
//|  cosi' il binario e' LO STESSO, il segnale e' identico per        |
//|  COSTRUZIONE e l'unica variabile della misura e' il sizing.       |
//|  Due EA diversi avrebbero richiesto di DIMOSTRARE che i segnali   |
//|  coincidono; qui non c'e' niente da dimostrare.                   |
//|                                                                   |
//|  PERCHE' IL NOME DEL FILE PER-TRADE E' CAMBIATO (classe 455):     |
//|  il nome di casa e' abtg_trades_<EA>_<SIMBOLO>_<MAGIC>.csv. In    |
//|  una corsa a due celle le due celle hanno lo STESSO magic, quindi |
//|  la seconda cancellerebbe la prima e la misura leggerebbe una     |
//|  gamba sola credendo di leggerne due. Qui il nome porta dentro    |
//|  TUTTE le variabili della corsa: collisione impossibile per       |
...
input double InpOrder1Atr   = 0.10; // 1o ordine: verso il prezzo, oltre la EMA200 di N*ATR (guida ~5 pip)
input double InpOrder2Atr   = 0.35; // 2o ordine: oltre la EMA200 (overshoot) di N*ATR (guida ~15 pip)
...
input long   InpMagic     = 778411;   // VERGINE: strumento di misura, mai una sedia
input int    InpMaxSpread = 0;
input bool   InpVerbose   = true;

//--- MARCATORE_MIS_SIZING_v1
//--- STRUMENTO DI MISURA, NON UNA SEDIA. Vedi l'intestazione in cima.
//    false = sizing NUOVO (quello del pin, che andrebbe in campo con l'F7)
//    true  = sizing VECCHIO (quello del binario 344a11b9 che gira ADESSO)
//    E' l'UNICA variabile della misura: tutto il resto e' byte-identico al pin.
...
void Log(string m){ if(InpVerbose) Print("[EMA200-MIS] ", m); }
...
double LotByRisk(double slDist,double riskPct)
  {
   ...
   if(InpSizingVecchio)
     {
      //--- RAMO VECCHIO: copia ALLA LETTERA del calcolo di 344a11b9,
      //    cioe' del binario che gira ADESSO sul 50503392. Tick value
      //    nudo, senza conversione in valuta conto.
...
void ExportTrades()
  {
   ...
   //    Col nome di casa (solo EA+simbolo+magic) la seconda cella cancellava
   //    la prima e la misura leggeva una gamba sola credendo di leggerne due.
   ...
   FileClose(h);
   PrintFormat("[EMA200-MIS] ramo=%s deposito=%.2f file=%s",

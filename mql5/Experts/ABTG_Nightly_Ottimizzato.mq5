...
input double InpEdgeOffsetPips = 2; // scostamento dall'estremo (0=sul bordo; >0=conservativo, piu' esterno)
...
input double InpSLatrMult    = 1.25; // se InpSLpips=0: SL = N*ATR oltre il bordo
input double InpTPfrac       = 0.3; // TP = frazione del range verso il centro (0.5 = meta')
...
input string InpComment   = "NIGHTLY OTT";
input long   InpMagic     = 971701;
...
bool TryPlace()
  {
   ...
   if(!ABTG_GuardiaIngresso(InpUsaGuardian,"ABTG_Nightly_Ottimizzato")) return(false);
   if(InpAllowShort)
     {

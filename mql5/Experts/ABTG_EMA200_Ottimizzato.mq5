...
long cO_pavimento=0;  // gamba saltata dalla politica del pavimento lotto
...
void ImbutoStampa(string quando)
  {
   ...
   string b="[EMA200-IMBUTO] "+_Symbol+" "+quando+
            ...
            " | pavimento lotto "+IntegerToString(d[16])+
            " | invio fallito "+IntegerToString(d[17])+
            " | PIAZZATI "+IntegerToString(d[18])+
...
void PlaceLimit(bool isLong,double px,double sl,double riskPct,string tag)
  {
   ...
   if(!ABTG_GuardiaIngresso(InpUsaGuardian,"ABTG_EMA200_Ottimizzato")){ cO_guardian++; return; }
   ...
   if(gLastFloorMorso)
     {
      ...
      if(InpFloorPolicy==ABTG_FLOOR_SALTA_GAMBA)
        { cO_pavimento++; Log("PAVIMENTO: gamba "+tag+" NON piazzata (politica SALTA_GAMBA)."); return; }
   ...
   if(ok){ cI_ordini++; gTradesToday++; gRischioSegnale += lot*gLastLossPerLot; Log(StringFormat("%s LIMIT %s @ %s SL %s TP %s lot %.2f",

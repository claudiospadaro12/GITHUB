...
#define ABTG_DEF_NAME         "DAX Live5m v2"
#define ABTG_DEF_MAGIC        770121
...
#define ABTG_DEF_RANGE_MODE   1      // 1 = candela 5 min PRIMA dell'apertura (natura Live5m)
#define ABTG_DEF_PREVWIN      5      // finestra pre-apertura: 07:55-08:00 server (08:55-09:00 IT)
#define ABTG_DEF_BUFFER       700    // 7 punti indice oltre il max/min (come Live5m)
#define ABTG_DEF_MINRANGE     1500   // FILTRO: candela >= 15 punti (niente candeline morte)
#define ABTG_DEF_MAXRANGE     4000   // FILTRO: candela <= 40 punti (niente candeloni da news)
...
#define ABTG_DEF_RISK         1.0    // v2: rischio 1% (prudenza)
#define ABTG_DEF_TRAIL_MODE   1      // trailing base candela precedente (come Live5m)
...
input double InpSlippagePts   = 100;    // v2: slippage onesto 100 pt
...
int ABTG_OnInit()
  {
   ...
   ABTGLog(StringFormat("CONFIG IN USO -> motore=%s | rangemode=%s | range=%d min | buffer=%.0f pt | lati=%s | rischio=%.2f%% | TP=%.1fR | parziale=%.0f%% | BE=%s | trail=%s %s | trail da=%.2fR",
...
bool TryPlaceBreakout()
  {
   ...
   if(!ABTG_GuardiaIngresso(InpUsaGuardian,"ABTG_DAX_Live5m_v2")) return(false);

...
//|  1. 15:30–16:00 CET: osservare, tracciare High/Low del range    |
...
//| SESSIONE: Apertura Wall Street 15:30-16:00 CET                   |
//|                                                                  |
...
//+------------------------------------------------------------------+
#property copyright "ABTG ORB Base PM – Emiliano Monza"
#property version   "1.00"
#property description "ORB DAX PM: OR 15:30-16:00 CET apertura Wall Street"
...
input int    InpORStartHour    = 15;       // Ora inizio OR (CET) — apertura Wall Street
input int    InpORStartMin     = 30;       // Minuto inizio OR
input int    InpOREndHour      = 16;       // Ora fine OR (CET)
input int    InpOREndMin       = 0;        // Minuto fine OR
...
input int    InpMagicNumber    = 20260315; // Magic Number (diverso da EA mattino)
...
double   g_orHigh   = 0;  // Massimo range 15:30-16:00 (incluse wick)
double   g_orLow    = 0;  // Minimo range 15:30-16:00 (incluse wick)
bool     g_orValid  = false;

// Nomi GlobalVariable per persistenza range OR (sopravvive al riavvio EA)
string GV_OR_HIGH  = "ORB_DAX_PM_OR_HIGH";
string GV_OR_LOW   = "ORB_DAX_PM_OR_LOW";
string GV_OR_VALID = "ORB_DAX_PM_OR_VALID";
string GV_OR_DAY   = "ORB_DAX_PM_OR_DAY";
...
int OnInit()
{
    ...
    // Se l'EA si riavvia durante o dopo la finestra OR PM (15:30-16:00)
    // recupera il range già formato per non perdere il trade.
       ...
       GlobalVariableCheck(GV_OR_DAY))
    {
        ...
        if(dtSaved.year == dtNow.year && dtSaved.mon == dtNow.mon && dtSaved.day == dtNow.day)
        {
            ...
            g_lastDay = savedDay;
            g_state   = DS_READY;
            Print(StringFormat("[INIT] Range OR PM ripristinato da GlobalVar | High=%.1f Low=%.1f",
                               g_orHigh, g_orLow));
        }
        else
        {
            Print("[INIT] GlobalVar OR PM di giorno diverso → ignorate");
        }
    }

    Print("════════════════════════════════════════");
    Print("  ORB DAX PM EA v1.00");
...
void OnDeinit(const int reason)
{
    ...
    Print("ORB DAX PM EA – Rimosso | reason=", reason);
}
//+------------------------------------------------------------------+

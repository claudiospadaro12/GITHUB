...
- **Due di fila**: arriverebbero a ~72.117, **443 EUR SOTTO** l'emergenza: il Guardian chiude tutto a **72.560** (DD 9,30%) e blocca SENZA SCADENZA finché qualcuno non cancella a mano la GV ABTG_GUARD_541452707_FAILED: il codice non la azzera mai, ripete FlattenAll a ogni secondo (r.771-773 a HEAD; r.423 nella v1.12 in campo) e rinnova a ogni giro la scadenza di 30 giorni della pausa (r.781 a HEAD; r.436 in v1.12). 👉 **Il secondo stop pieno di fila ferma la challenge.** Il muro FTMO del 10% resta intatto (cuscino di 560 EUR; lo slittamento della chiusura d'emergenza e' [NON MISURATO]).
...
- **Cap 3,25 contro 4,00**: con sedie al 2% la differenza e' solo sulla **terza** posizione (due lotti arrotondati possono sommare 3,99% < 4,00 e lasciarla passare; con 3,25 no — ma solo per un ordine inviato DOPO: un pendente piazzato quando il cap era libero scatta con tutti e due i tetti, ABTG_PausaGuardian.mqh r.49-51, classe 645; le sei sedie entrano tutte per pendente). ⚠️ Il commento del preset che dice *"3.25 lasciava passare UNA SOLA posizione"* e' **sbagliato** per lo stesso motivo.

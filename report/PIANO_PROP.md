...
| B2 | **Cap totale interno** | **chiusura d'emergenza a 9,9%** (= 10 − 0,1 tecnico); un eventuale fermo deliberato prima (es. 9,0) e' una **scelta di gestione**, non un buffer tecnico, e va decisa come tale | 🥉 il 9 secco viene da **UNA casa sola** (Profalgo, due prodotti); le tre fonti a buffer sottile direbbero **9,9** · 🥇 p99 nostro statico 8,51% (esatto, M1): sta sotto entrambe le soglie, il portafoglio ci passa | stessa correzione d'evidenza di B1: il "9" non e' "quello che dicono tutti" | 📋 PROPOSTO (P2, valore rivisto v3) |
...
1. **C1+C2 — il cap sul rischio aperto: 3,25% (v4, la nuova piu' urgente).**
   Non e' piu' un timore: il **5,85% simultaneo E' SUCCESSO** (03/08) e il
   p99 di agosto sta a 5,67% — oltre il muro. La misura c'e', la proposta e'
   sul tavolo, manca solo la firma. _(C3, che era la n.1, e' gia' in firma
   con Claudio proprio ora — versione tre corsie: non serve spingerla, serve
   solo chiuderla.)_
2. **B1+B2+B3 — il pacchetto Guardian** (P2+P1, riscritto v3): **doppia
   ...
   caso letto che sta sul muro esatto. Con C1 firmato, la pausa a 4,0 diventa
   la SECONDA rete, come da referto M2.
...
**34 parametri censiti: 3 congelati · 18 proposti · 12 aperti · 1 chiuso per
misura (C5).**
Congelati: A1 (0,65% — dopo M1 da leggersi "su DD statico", 09/08) · F4
(niente challenge prima del forward maturo + risposte scritte, 13/08) · F5
(cancello d'acquisto EA, 18/08). Cambi di stato del v4: **C5 CHIUSA-MISURATA**
(trailing p99 12,05 a 0,65) · **C1 e C2 APERTE→PROPOSTE** (cap 3,25% = 5 SL
vivi, misura M2) · **F3 da divieto a tabella delle taglie** (1-Step: ≤0,50 su
muro 10, ≤0,40 su muro 8, mai muro 6) · **C3 in firma** (tre corsie, si
congela solo con la parola di Claudio). Delle 12 aperte, 2 si chiudono con
misure di casa a costo basso (E3, E4).

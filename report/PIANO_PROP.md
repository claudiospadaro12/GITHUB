...
- ⏳ **NON ancora incorporata**: `ANALISI_TRASCRIZIONI_2026-08-18.md`
  (l'analista-trascrizioni sta girando ADESSO su 11 trascrizioni video). I
  parametri che quell'analisi puo' chiudere sono marcati
  **🎬 in arrivo dall'analista-trascrizioni**. Secondo giro alla consegna.
...
|---|---|---|---|---|---|
| A1 | **Rischio per trade a taglia prop** | **0,65%** | 🥇 MC R16 + 27 serie (`METRO_PROP` §1-bis: p99 12,47% a 1% → ~8,1% a 0,65% vs muro 10%); decisione in `DEPLOY_GUARDIANO_100K.md` | 🥉 blog MQL5 (E0) suggerisce 0,25-0,4%; 🥉 PROPstyle ragiona per rischio TOTALE ≤1%; 4° script CrewAI (18/08): 0,5% per trade contro muro 5% ("10 perdite per sfondare"). Le voci esterne stanno tutte SOTTO il nostro 0,65 — ma la nostra misura vince, e vale **solo su DD statico**: col trailing il numero e' da rifare (→ C5) | 🧊 **CONGELATO (09/08/2026, decisione di Claudio, verbale in `DEPLOY_GUARDIANO_100K.md`: "Rischio per trade: 0,65% (non 1%!)")** |
...
| A3 | Taglia in **fase 2** della challenge | ridurre (meta', o −20%) | 4° dichiarazione singola: blog MQL5 E0-bis ("fase 2: lotto ridotto della meta'") | una sola fonte: non chiude niente | 🔓 APERTO — 🎬 in arrivo dall'analista-trascrizioni (se piu' video convergono sale a 🥉) |
...
| B5 | **`InpDDMode=2`**: trailing EOD sul saldo di fine giornata piu' alto | aggiungere la modalita' (oggi: 0 statico, 1 trailing equity) | 🥉 KT Equity Protector dichiara esattamente 3 modelli · 🥈 FTMO 1-Step usa proprio questo ("il limite puo' solo salire") | scrivere il codice **non risponde** alla domanda vera (METRO_PROP §1: MC trailing mai fatta) — la rende solo misurabile. L'uso resta bloccato da C5 | 📋 PROPOSTO (P7) — il codice si'; l'USO resta APERTO |
...
| F3 | **Prop 1-Step / DD trailing** | **vietato guardarle** finche' la MC trailing (C5) non esiste | 🥇 `METRO_PROP` §1: "comprare una challenge col trailing e' comprare un biglietto per una gara di cui non conosciamo il percorso" | nessuno | 📋 PROPOSTO (divieto temporaneo, si scioglie con C5) |

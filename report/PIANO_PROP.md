...
_Prodotto dall'**architetto-prop**. Versione **v22**, aggiornata il 13/09/2026
...
**🆕 Fonti nuove del diciottesimo giro (v18, 03/09/2026)** — la mattinata piu'
densa del progetto: una violazione di contratto **scoperta e riparata in 8 ore**,
un difetto di codice **inchiodato e censito su 126 file**, un debito di misura
**aperto dal 23/08 e pagato**, e il primo candidato della caccia frequenza che
**passa il passo 0 e arriva a un round firmato**:
- 🥇 `report/giornata_2026-09-02.md` (pagella automatica del 02/09): il
  **+1.680,35 del dry-run 100k e' un DIFETTO, non un merito** — sovrataglia ORB
  `770611` **3,3× il contratto** (1,0% contro 0,3%), scoperta confrontando le
  **sei coppie appaiate** piccolo/100k (rapporto lotti da ~6 a ~20 fra il 21 e
  il 24/08); e il rilievo **EMA200 oro**: due posizioni morte **nello stesso
  secondo sullo STESSO prezzo di stop** = _"un rischio solo contato due volte"_,
  −100,33 € in un tocco
- 🥇 `report/ORB_GEMELLI_DIVERGENZA_2026-08-22.md` §§03/09 (il dossier che si
  chiude su due fronti): **FOTO 2** (pannello 100k: i due pannelli identici in
  ogni riga **tranne il rischio**) → **correzione ESEGUITA da Claudio alle
  08:10** (1.0 → **0.3**, screenshot prima/dopo, nessun altro campo toccato) ·
  **FOTO A delle 08:19 = la PISTOLA FUMANTE**: `LARRY DOW S` (772341) aperta dal
  **01/09 08:45** su U30USD, viva per tutta la vita del trade ORB → **ipotesi A
  (SelPos/hedging) confermata su 3 giorni su 3** (19/08, 21/08, 02/09)
- 🥇 `report/AUDIT_POSITIONSELECT_HEDGING_2026-09-03.md` (**126 file
  esaminati**: **26 🔴 vulnerabili · 8 🟠 "mezzo fix" · 85 🟢 sani · 7 ⚪ non
  tradano**; la scoperta che allarga il caso: la stessa `SelPos()` governa
  **`HandleOCO()`** e **`InpOneTradePerDay`** → **e' un difetto di RISCHIO**, non
  solo di gestione) + 🥇 `report/VERIFICA_CHIUSURE_INCROCIATE_2026-09-03.md`
  (la verifica storica chiesta dall'audit: **ZERO chiusure incrociate** in 21
  chiusure `expert` esaminate una per una → **i 🟠 NON passano davanti ai 🔴**;
  ma **il danno passivo dei 🔴 e' agli atti**: `ABTG_ORB` nativo ha aperto il
  **secondo lato vietato dal pannello in 4 giornate su 16**, correlate **4 su 4**
  con un vicino a ticket piu' basso, **+25% di frequenza rispetto al contratto**)
- 🥇 `report/CENSIMENTO_SEDIE_MUTE_2026-09-03.md` (**6 sedie su 6 VIVE e
  attaccate**, foto dei grafici 08:47-08:54 → **il semaforo della FASE 4 della
  migrazione e' VERDE**)
- 🥇 `backtest_pipeline/risultati_archivio/SPREAD_FLOTTA_MISURA_2026-09-03.md`
  (**il debito del 23/08 e' PAGATO**: 252 milioni di tick, **solo-bid 0,000%**
  su 3 simboli su 3, mediane **in sessione 1,6-2,0 punti indice**, **DAX notte
  3,5-3,9**, Dow ora 23 con P95 7,0 e max 101)
- 🥇 `backtest_pipeline/risultati_archivio/REFERTO_SONDARSIEMAV8_2026-09-03.md`
  (**V8 NON PROMOSSO, ora confermato DA UNA MISURA**: l'ablazione dice che il
  filtro RSI toglie solo il **9-13%** degli incroci EMA → il V8 **e' un incrocio
  EMA(5/20)**, famiglia gia' morta due volte in casa)
- 🥇 `backtest_pipeline/risultati_archivio/REFERTO_SONDALONDONFX_2026-09-03.md`
  (**IL PRIMO SUPERSTITE DEL PASSO 0** di tutta la missione frequenza: EURUSD
  **19/24 righe vive**, GBPUSD **24/24**, e il filtro RSI che **taglia il
  73-77%** — l'esatto opposto del V8)
- ✍️🥇 `report/FIRME_2026-09-03.md` + `backtest_pipeline/risultati_archivio/LONDONFX_TICK_CRITERI.md`
  (**R116 FIRMATO** alle ~09:45, testuale _"FIRMO TUTTO, ANCHE LA A SU F5"_:
  **F5=A** (RSI short simmetrica 20), **ora 8**, **rischio 0,65%**, terzo motore
  dentro il contenitore, prova di rischio sul vecchio come **round separato**,
  **cap 2%/giorno acceso**) + `mql5/Experts/ABTG_LondonFx.mq5` v1.00 (EA
  contenitore **repo-only, MAI compilato**, **hedge-safe dalla nascita**)
- 🥉 `backtest_pipeline/caccia_strategie/CACCIA_FREQUENZA4_GH_TV_FF_2026-09-02.md`
  (**RELATIVO** promosso 🟢 8/10: divergenza relativa fra due indici che
  quotiamo entrambi — **il primo motore il cui segnale non esiste senza un
  secondo strumento**; e il numero della battuta: **7 sorgenti Pine su 8 non
  contengono UN SOLO `strategy.exit`**) + `CACCIA_FREQUENZA4_CB_PAPER_2026-09-02.md`
  (**VGRSI** promosso 🟠 6/10 **in coda**, arXiv 2605.01300 — frequenza
  dichiarata 3,3-4,8/giorno **sui nostri simboli dentro MT5**, ma i numeri
  dell'autore nascono da una **ri-ottimizzazione settimanale** = la regola che
  in casa e' misurata **anti-predittiva**; e **Code Base CHIUSO su 400 id**)
- 🥇 `report/M31_RISCHIO_REALIZZATO_2026-09-02.md` (**M31 ESEGUITA** — era
  arrivata dopo la chiusura del v17.1 e viene incorporata qui: il **16,39%** che
...
- ✒️🥇 **`report/FIRME_2026-09-07.md`** — **due firme** (_"firmo il punto 1,
  ...
  🔴 **FIRMATO MA NON ATTIVO** → riga **C10**. 📌 **Appendice dello stesso
  verbale, aggiunta in giornata**: il tetto e' stato **IMPLEMENTATO** (Guardian
  **v1.13** + include **v1.60**, input **spenti di default**) — lo stato passa da
  _"firmato ma non attivo"_ a _"implementato ma **non compilato e non
  collaudato**"_, che vuol ancora dire **NON ATTIVO**
- ✅🥇 **`report/CENSIMENTO_CONTRATTI.md`** (07/09, da un altro agente) — **il
  prerequisito della C3, aperto da tre settimane, e' CHIUSO**: **47 righe-sedia /
  41 sedie uniche**, per CONTO, con DD promesso + **fonte** + deposito/rischio del
  backtest + `n` + merito + frequenza. **40 MISURATE · 3 da riprodurre · 4 NON
  MISURATE**; merito **PIENO su 15**, **SOSPESO su 28**. 🔴 Quattro buchi
  dichiarati (le tre PostNews **senza DD promesso** e gia' operative · il
  `BREAKOUT_EA_JPY_v3` col **magic illeggibile** · la **771322** con **tre DD
  incompatibili** · l'ultima foto `.chr` ferma al **25/08**)
...
### 🥈 BLOCCO 2 — `ExecutionMode=0`: **39 round a ritardo ZERO** — ✅ **MISURATO IN GIORNATA, e l'ipotesi congelata E' FALSIFICATA**
...
✅ **IL SECONDO GIRO E' ANDATO, NELLA STESSA GIORNATA** — `..._v3_EXECMODE`,
avvio **16:23:55**, **cancello G0: MORDE**, **16 CSV su 16 letti**, **gemelli G1
IDENTICI su tutte e 8 le corse**, **0 rilievi**, **ESITO: MISURATO**
(🥇 `backtest_pipeline/risultati_archivio/ritardo_r119b_csv/REFERTO_RITARDO_R119.txt`).
Stessa finestra **2024.09.26 → 2026.06.30**, tick reali, **preset del conto
REALE** (`InpRiskPercent=0,65` su tutte e due le sedie).

🔴 **E l'ipotesi congelata — _"LIMIT contro STOP: `770101` degrada poco,
`770611` molto"_ — E' FALSIFICATA SU TUTTE E DUE LE META'.** 🥇 [lettura di
questo giro sui 16 CSV grezzi]:

| sedia (come entra) | finestra | 0 ms | 50 ms | 100 ms | 500 ms |
|---|---|---:|---:|---:|---:|
| **770611 ORB** (BuyStop) | IS · Profit | 569,12 | **=** | **=** | 566,88 (**−0,4%**) |
| | **OOS · Profit** | 2.484,17 | **=** | **=** | **= (identico alla cifra)** |
| | OOS · DD% | 6,5389 | **=** | **=** | **=** |
| **770101 DAX** (BuyLimit) | IS · Profit | 280,02 | = | 280,52 (+0,2%) | 285,02 (**+1,8%**) |
| | IS · DD% | 3,1749 | = | 3,1912 (+0,5%) | 3,2696 (**+3,0%**) |
| | **OOS · Profit** | 1.103,31 | 1.106,11 (+0,3%) | 1.108,89 (+0,5%) | **1.120,25 (+1,5%)** |
| | OOS · DD% | 4,3501 | 4,3490 | 4,3484 | **4,3139 (−0,8%)** |

👉 **Il contrario di quello che avevamo scritto**: **lo STOP e' immune** (OOS
**identico alla cifra** fino a 500 ms; in IS si muove dello **0,4%** solo a 500),
e **il LIMIT e' quello che si muove** — di poco (**≤2% sul profitto, ≤3% sul
DD**) e **in MEGLIO**, con perfino **un trade in piu'** a 500 ms (175 → 176).

🔴 **Tre cose da dire prima che qualcuno usi questi numeri:**
1. ⚠️ **Il MECCANISMO non e' spiegato.** Un ritardo che **migliora** i risultati
   non e' un costo: e' un **cambio di quale pendente viene toccato**. Va capito
   **prima** di tarare qualcosa su questa misura.
2. ⚠️ **Questo NON sostituisce il BLOCCO 1**: `ExecutionMode` ritarda
   l'**esecuzione**, non modella lo **scarto di prezzo**. Lo slippaggio resta
   **NON MISURATO** (0 deal).
3. ⚠️ **Un solo regime** (toro pieno) e stessa finestra per tutte e otto le corse.
   E il **verdetto formale** contro i criteri **S1-S4** del contratto **non e'
   ancora scritto**: il referto della macchina dice esplicitamente _"non giudica…
   il verdetto lo scrive la chat"_.

🟢 **Il regalo grosso, che vale piu' del round**: per la **prima volta** le due
sedie del conto REALE hanno un **DD promesso misurato ALLA TAGLIA VIVA
(0,65%), su tick reali, nella stessa finestra e con lo stesso driver** —
e **la 770101 ha il merito PIENO in tutte e due le meta'** (n **175** IS e
**270** OOS, ≥ 150):

| sedia | IS: n · PF · DD% | OOS: n · PF · DD% | merito |
|---|---|---|---|
...
verificato**.
**Fonti:** 🥇 `REFERTO_RITARDO_R119_PRIMO_GIRO.md` §2, §4, §6 (il giro fallito) ·
🥇 `backtest_pipeline/risultati_archivio/ritardo_r119b_csv/` (16 CSV + referto
macchina del giro **MISURATO**) · classe **156** in
`backtest_pipeline/CHECKLIST_RIGA_DI_LANCIO.md`.

### 🥉 BLOCCO 3 — IL CENSIMENTO DEI CONTRATTI: ✅ **CONSEGNATO OGGI** (e quello che resta scoperto)
**Era** il prerequisito **dichiarato alla FIRMA 2 del 18/08** (_"senza tabella,
la regola e' inchiostro"_), **aperto da tre settimane**. ✅ **Chiuso oggi da un
altro agente** con `report/CENSIMENTO_CONTRATTI.md` (07/09): **47 righe-sedia =
41 sedie uniche**, organizzate **per CONTO** (reale / 100k / piccolo), con DD
promesso, **fonte** (CSV o referto + round), **deposito e rischio del backtest
da cui viene il numero**, `n`, se il merito e' sospeso, frequenza promessa.
**40 righe MISURATE · 3 da riprodurre · 4 NON MISURATE · merito PIENO su 15,
SOSPESO su 28.**
👉 **La corsia RISCHIO della C3 diventa applicabile su 40 sedie su 47.** E'
il singolo passo avanti piu' grosso del giro.

🔴 **Cio' che NON e' coperto, e resta un blocco — dichiarato dal censimento
stesso, non da noi:**
- **tre sedie PostNews** (771201 ECB · 771202 FOMC · **771203 NFP**) girano in
  forward **senza nessun DD promesso**; la 771203 rischia **1,30% per evento** e
  **ha gia' operato** (+37,36 € il 04/09);
- **`BREAKOUT_EA_JPY_v3`** su USDJPY: **nemmeno il magic e' leggibile**, e la
  famiglia ha DD **30-48%** agli atti;
- **771322 PTE GBPUSD** ha **TRE DD promessi incompatibili** (R23 **2,64%** ·
  R78 **17,68%** · R103 **13,1%**): su quella sedia la corsia RISCHIO **non puo'
  scattare in modo prevedibile**;
- ⚠️ **l'ultima foto `.chr` e' del 25/08**: da allora il **conto reale**, tre
  sedie nuove e un riavvio del terminale piccolo **non sono mai stati censiti**
  — il perimetro e' ricostruito da quattro fonti, non da una lettura fresca.

📌 E cio' che **questo** giro ha aggiunto alla tabella: il **DD promesso della
`770611`** — **5,65% IS / 6,54% OOS**, dep. 10.000, **gia' alla taglia viva
0,65%**, tick (🥇 `REFERTO_RITARDO_R119_PRIMO_GIRO.md` §5) — che il censimento
definisce _"il piu' recente e il piu' pulito del parco"_. **Merito SOSPESO**
(71/119 < 150): il PF 1,675 dell'OOS **non va citato come merito**.

⚠️ **Resta scoperta la PEGGIOR GIORNATA promessa**, che nessun contratto ha mai
avuto — ed e' proprio il vincolo che morde (`ANALISI_DD_TOTALE_2026-08-26.md`:
margine **5%** sul giorno contro **36%** sul totale). E nella tabella del
**cancello 1** la colonna "DD reale **forward** per famiglia" resta **`n/d`**:
il censimento da' la **promessa**, non il **realizzato** (→ M20).

### 4️⃣ BLOCCO 4 — IL TETTO PER CLUSTER 3,0%: **FIRMATO E IMPLEMENTATO OGGI, NON COMPILATO E NON COLLAUDATO**
**Il fatto, e va ripetuto ogni volta che si cita:** ✍️ firmato oggi
(`report/FIRME_2026-09-07.md`) e, **nella stessa giornata, gia' implementato**
da un altro agente — `ABTG_Guardian.mq5` **v1.13** + `ABTG_PausaGuardian.mqh`
**v1.60** (commit `cdb2037`). 🔴 **Ma lo stato e' cambiato di UNA parola sola,
e non di piu': da _"FIRMATO MA NON ATTIVO"_ a _"IMPLEMENTATO MA NON COMPILATO NE'
COLLAUDATO"_ — che vuol ancora dire NON ATTIVO.**
🟢 **Cio' che rende l'implementazione sicura, ed e' verificato nel commit, non
promesso**: i due input nuovi **nascono SPENTI** (`InpMaxClusterRiskPct=0`,
`InpClusterMappa=""`), i due parametri nuovi di `ABTG_GuardiaIngresso()` sono
**in coda alla firma** (nessuna chiamata esistente si sposta) e **tutto** il
codice nuovo sta dentro `if(StringLen(cluster_mappa)>0)` → col default **viene
saltato in blocco**. **Nessun EA in campo lo passa: no-op verificato.**
...
FAMIGLIA = piu' simboli) rende **piu' probabile** — e la firma che allarga **vale
da subito a costo zero**, quella che protegge **e' spenta**.
**I tre passi, e dove sono adesso:** (1) 🟠 **definire i cluster —
PROPOSTO, NON FIRMATO** (`report/CLUSTER_PROPOSTA.md`): resta **una scelta di
Claudio**; (2) ✅ **implementarlo nel Guardian — FATTO** (v1.13 / mqh v1.60,
**non compilato**: in questo ambiente non c'e' MetaEditor); (3) ❌
**collaudarlo — DA FARE, sul PC/VPS di Claudio** (`backtest_pipeline/attese_cluster.txt`),
col metro dell'enforcement e non con _"sembra funzionare"_.
⚙️ **Lavoro di un altro agente**: qui e' registrato come **riga C10**, non
duplicato. 🔴 **Fino al collaudo, ogni volta che si cita il tetto per cluster
si dice che NON e' una protezione attiva.**
...
| ~~**2**~~ ✅ | 🐤 ~~R119 v3 — rifare il round del ritardo con la chiave GIUSTA~~ — **FATTO IN GIORNATA** (`..._v3_EXECMODE`, 16:23:55: G0 **MORDE**, 16/16 CSV, gemelli identici 8/8, 0 rilievi). 🔴 **Esito: l'ipotesi congelata e' FALSIFICATA** — lo **STOP e' immune**, il **LIMIT** si muove **≤2%** e **in meglio**. ⏳ **Resta da fare a tavolino, non sul PC**: il **verdetto formale contro S1-S4** e **capire PERCHE'** un ritardo migliora i risultati | — | — | ✅ **BLOCCO 2 chiuso come misura**; resta aperta la spiegazione |
...
| **C10** 🆕 | **Tetto per CLUSTER / valuta** | **3,0%** | ✍️ `FIRME_2026-09-07.md` + appendice | 🔴 **FIRMATO · IMPLEMENTATO (v1.13, spento di default) · NON COMPILATO · NON COLLAUDATO = NON ATTIVO** |
...
| **J1** 🆕 | **Preset su file di `770202`** | ✅ **SCRITTO** — `mql5/Presets/ABTG_Dow_Apertura_US_U30USD_M5_770202_100K.set`, 81 righe **copiate** dal `.chr` vivo del 100k | 🥇 `CODA_08_..._20260911` r.3267-3386 | 📋 **PROPOSTO** — il file e' nel repo; **caricarlo nel terminale 50504263 e' di Claudio** |
| **J2** 🆕 | **DD promesso di `771531`** | **7,8323%** @1% (tick, dep. 100.000, OOS, n 517 uscite = **257 posizioni**) · ~**5,09%** @0,65% 🔴 [APPROSSIMATO lineare] | 🥇 R31 = R110 = R112 (**riprodotto 3 volte al centesimo**) | ✅ **CONFLITTO SCIOLTO** — ma 🔓 **APERTO** alla taglia della sedia che gira (~5.000 EUR): **M40** |
| **I1** 🆕 | **Slippaggio reale** | **NON MISURATO** — 0 deal | 🥇 R118 §11 | ❔ NON MISURATO |
| **I2** 🆕 | **`ExecutionMode` del tester** | **0 in tutti i round** (dichiarato dal 07/08, mai letto) — ✅ **misurato il 07/09**: STOP **immune**, LIMIT si muove **≤2%** e **in meglio** | 🥇 R119 §4 + `ritardo_r119b_csv/` | ✅ **MISURATO — ipotesi congelata FALSIFICATA** |
...
> 🆕 **v18 (03/09) — LA MATTINATA IN CUI IL PIANO HA IMPARATO TRE COSE, E DUE
> RIGUARDANO IL CANCELLO 4.**
> - ✅ **FASE 4 DELLA MIGRAZIONE: SEMAFORO VERDE.** Le 6 sedie mute erano il
>   dubbio (_"vive, o morte in silenzio?"_): **6 su 6 VIVE e attaccate**, foto
>   dei grafici alle 08:47-08:54 con Algo Trading verde e terminale connesso
>   (🥇 `report/CENSIMENTO_SEDIE_MUTE_2026-09-03.md`). Larry U30 aveva perfino
>   **una posizione aperta** — la stessa che poi ha inchiodato il difetto
>   hedging. ⚠️ Limite dichiarato dal censimento stesso: e' la foto del
>   **grafico** (EA attaccato + oggetti disegnati), **non** la scheda Esperti
>   riga per riga: se una sedia resta muta a lungo, quella e' la verifica dopo.
> - ⏳ **COLLAUDO ENFORCEMENT, SESSIONE 1: IMMINENTE.** Le stringhe della
>   sessione **cap + fail-open** (criteri 7 e 8, ~45 min) sono **in costruzione
>   dal verificatore stamattina**. Finche' non gira, i criteri 5-9 restano
>   "procedura scritta" e **il cancello della fase 2 resta chiuso**.
> - 🔴 **E LA COSA NUOVA CHE PESA SUL CANCELLO 4 PIU' DI TUTTE: una parte della
>   flotta NON esegue la gestione che il suo pannello dichiara.** L'audit del
>   03/09 ha censito **34 file su 126** col difetto `PositionSelect` su conto
>   HEDGING (**26 🔴 + 8 🟠**), e la verifica storica ha trovato **il primo
>   danno MISURATO in forward**: `ABTG_ORB` nativo ha aperto **il secondo lato
>   che il suo pannello vieta in 4 giornate su 16**, correlate **4 su 4** con un
>   vicino a ticket piu' basso. 👉 **Un enforcement che funziona non serve a
>   niente se l'EA sotto non fa quello che dice.** Riga nuova: **C9** — e il
>   conflitto con la firma **D1** (_"niente ricompilazioni in fase 1"_) sta
>   scritto li' dentro, perche' e' la decisione piu' urgente di oggi.
...
| **2** | **Censimento dei contratti COMPLETO** (DD e frequenza promessi, sedia per sedia) | 🆕✅ **v20 (07/09): IL CENSIMENTO E' CONSEGNATO** — `report/CENSIMENTO_CONTRATTI.md`, **47 righe-sedia / 41 sedie uniche** per CONTO, con DD promesso + fonte + deposito/rischio del backtest + `n` + merito + frequenza: **40 MISURATE · 3 da riprodurre · 4 NON MISURATE**, merito **PIENO su 15** e **SOSPESO su 28**. 👉 **La corsia RISCHIO della C3 e' applicabile su 40 sedie su 47.** 🔴 **Il cancello NON e' ancora verde**, per quattro motivi dichiarati dal censimento stesso: le **tre PostNews senza DD promesso** (e la 771203 ha gia' operato, rischia **1,30% per evento**), il `BREAKOUT_EA_JPY_v3` col **magic illeggibile**, la **771322 con TRE DD incompatibili** (2,64 / 17,68 / 13,1%), e **l'ultima foto `.chr` ferma al 25/08** (il conto reale non e' mai stato censito). ⚠️ E manca ancora la **PEGGIOR GIORNATA** promessa. _(stato precedente: 🟡 GIALLO)_ — `report/CONTRATTI_SEDIE.md` esiste dal 18/08 (44 sedie: 40 pieni, 2 parziali, 2 senza) ed e' stato riscritto dalla firma "A+b" del 24/08 per le 5 sedie ridotte; **manca la PEGGIOR GIORNATA**, che nessun contratto ha mai avuto — **R112 la sta misurando adesso** sull'EMADOW (metro + 3 dial) | 🥇 `report/CONTRATTI_SEDIE.md` · ✍️ `report/FIRMA_REVISIONE_FLOTTA_2026-08-24.md` · 🥇 `R112_CRITERI.md` §4 (convenzione congelata prima) e §8 ("la peggior giornata del METRO entra agli atti — oggi il censimento non ce l'ha") | referto R112 consegnato + la colonna "peggior giornata promessa" estesa almeno alle sedie che pesano. ⚠️ E' un **pavimento**: R112 misura i CHIUSI, il muro guarda il **flottante** (limite dichiarato nei criteri) |
...
| **4** | **Preset Guardian sui muri della prop scelta + firma** | 🔴 **ROSSO** — il Guardian in campo e' tarato **solo su FTMO** (4,0 / 4,9 / 9,9 / reset 23 / DD statico). Il dossier Upcomers l'ha dimostrato per assurdo: su muri 3/6 ogni singola soglia sta **oltre** il muro che dovrebbe proteggere. Il preset "muri stretti" e' **proposta P1**, non esiste ancora | 🥇 preset+sorgente `mql5/Experts/ABTG_Guardian.mq5` (`InpDDMode` gia' presente, righe 53/355/367) · `DOSSIER_PROP_UPCOMERS_2026-08-26.md` §5 e §9-P1 | un preset per **famiglia di muri** (non uno solo), collaudato con autotest sul 100k, e la firma di Claudio. ➕ ~~**manca l'enforcement**~~ → 🆕 **v17: L'ENFORCEMENT E' IN COLLAUDO.** ✍️ firma 02/09 (strada **(b)**: e' il cancello della fase 2); **criteri 1-4 VERDI sui binari in campo**, **5-9 con procedura scritta**, **canarino P-C1 verde 8/8** in campo il 02/09 → i criteri 5/7/8 sono ora **deterministici**. Restano le due sessioni di Claudio (45 + 40 min, **mai lo stesso giorno**) e la settimana di pagelle. ⏳🆕 **v18 (03/09): la SESSIONE 1 (cap + fail-open) ha le stringhe in costruzione dal verificatore stamattina** — e' il primo dei due passi che mancano. 🔴 **E si aggiunge un ostacolo nuovo, che non c'era il 02/09**: l'enforcement misura che il Guardian **blocca** gli ingressi in eccesso, ma l'audit del 03/09 misura che **34 EA su 126 non eseguono la gestione che dichiarano** (→ **C9**): le due cose vanno lette insieme, perche' il cancello 4 promette che **il rischio sia governato**, non solo che una bandiera venga scritta |
...
| A2 | Rischio sedie **giovani** (<30 trade OOS-forward) | **0,3%** (mezzo peso) | 🥇 stessa decisione del 09/08 (ORB a 0,3% nel dry-run: "+41k dei +73,8k sono suoi: mezzo peso finche' non ha 30 trade") | 🔴🆕 **v18 — LA A2 E' STATA VIOLATA IN CAMPO PER NOVE GIORNI, ED E' STATA RIPARATA**: la pagella automatica del 02/09 ha scoperto che l'ORB `770611` **sul 100k** girava a **1,0% invece di 0,3% = sovrataglia 3,3×**, misurata su **sei coppie appaiate** piccolo/100k (stesso secondo, stesso prezzo): il rapporto dei lotti passa da **5,86-6,44** (contratto rispettato) a **20,29 e 19,70** fra il **21 e il 24 agosto**, cioe' esattamente il rapporto dei saldi. Verifica indipendente sull'ultima coppia: stop 59,00 punti × 19,70 lotti × 0,8625 = **1.002 € = 0,996% del conto**. 👉 **Il dry-run FTMO aveva una sedia in positivo solo perche' era fuori contratto**: riportata a 0,3%, l'ORB sul 100k vale **circa −383 €** invece di **+496,44**. ✅ **CORRETTA DA CLAUDIO IL 03/09 ALLE 08:10** (legge dello screenshot: foto 07:34 a 1.0 → modifica → foto 08:10 a **0.3**, nessun altro campo toccato; controllo delle 08:17 sul piccolo: 1,0% = contratto rispettato, chiuso con Annulla) | 🧊 **CONGELATO (18/08/2026, verbale FIRME, FIRMA 4: "ogni sedia con meno di 30 trade in forward gira a 0,3%" — era la prassi del dry-run, da oggi regola scritta per tutte)** — ✅ **v18: caso sovrataglia CHIUSO e fotografato** (`report/ORB_GEMELLI_DIVERGENZA_2026-08-22.md` §03/09 08:10 e 08:17). 🔍 **La verifica sul campo e' la PRIMA COPPIA GEMELLA FUTURA**: il rapporto dei lotti deve tornare **~6**. ⚠️ **E la lezione di processo, che vale piu' del caso**: la deriva e' vissuta **nove giorni** perche' nessuno confrontava le coppie appaiate — l'esperimento proposto dalla pagella (confronto automatico dei lotti gemelli contro il contratto) **e' la spia che mancava** |
...
| # | parametro | valore PROPOSTO | fonti (rango) | conflitti | stato |
|---|---|---|---|---|---|
| C8 🆕 | 🎯 **TETTO MAX POSIZIONI PER SIMBOLO + LATO** — la forma di rischio che il cap C1 **non vede** (nuova, v17) | input nell'**include di guardia**, **opt-in**: `0 = SPENTO` di default, comportamento **identico a oggi**. Valore di riferimento pronto da una fonte esterna: **max 2 posizioni per simbolo** (E1 del censimento). 🔴 **NOTA DI CANTIERE, dal controllo C2 del 02/09**: alcuni EA hanno **GIA'** un input _"A1: tetto posizioni+pendenti sul simbolo"_ (visto **vivo** su `ABTG_DAX_Apertura_EU`, oggi a **0**) → **il cantiere CENSISCE l'esistente prima di scrivere codice nuovo** | ✍️🥇 `report/FIRME_2026-09-02.md` §P0 (**FIRMATA**, cantiere ~3 h, default spento) · 🥇 la misura che la rende necessaria, `report/CONFIG_PROP_2026-08-31.md` §3.3: **62 grappoli simbolo+direzione** sul conto piccolo, il **peggiore −533,52 € = −10,67% del conto** (29/07, D30EUR short: **5 posizioni, 4 magic, 9 minuti, tutte stoppate**) contro un tetto prop del **3%** → **sforato 3,5×** · 🥇 il Dow del 31/08 (5 EA, due direzioni, 8,5 ore) · 🥇 `VERBALE_CHIUSURA_770101_2026-09-02.md` §C1 (l'anomalia del 29/07 non e' riproducibile oggi, ma **la forma di rischio si', ed e' questa**) | 🔴 **La ragione per cui il cap C1 non basta, ed e' aritmetica**: C1 conta **il rischio totale in %**, quindi **non distingue** 5 SL su 5 simboli diversi da 5 SL sullo **stesso simbolo nella stessa direzione** — che per una prop e' **UNA sola idea di trade** e per il mercato e' **una sola scommessa con leva 5×**. Il cap globale e' cieco su questa forma **per costruzione**, non per difetto. ⚠️ **VINCOLO DICHIARATO ALLA FIRMA**: la modifica vive **nel repo**; i **binari in campo NON cambiano** finche' Claudio non ricompila/ridistribuisce — coerente con **D1** (l'allineamento a HEAD e' un round separato, **dopo** la fase 1). Quindi: **firmata ≠ in vigore** | 🧊 **CONGELATO (02/09/2026, P0 FIRMATA — `report/FIRME_2026-09-02.md`)** come **decisione di cantiere**, con default **spento**. 🛑 L'**accensione** su una qualunque sedia e' una decisione separata e non e' firmata qui. ✅🆕 **v17.1 — IL CENSIMENTO PRIMA DEL CODICE E' STATO FATTO, e ha trovato una cosa che cambia il disegno** (commit `36b4ba6`): il tetto **"A1" (`InpMaxPosSimbolo`) esisteva GIA'**, **copiato a mano e identico byte per byte in 5 EA** della famiglia Aperture (DAX · Dow · Nasdaq · 3Ingressi · Marco) — conta posizioni **+ pendenti** su **tutti i magic**, ma **NON per lato**: conta il **totale sul simbolo**. 🔴 **E su conto HEDGING quella e' la distinzione che conta**: un long e uno short sullo stesso simbolo sono una **COPERTURA**, non un pile-up — con `A1=1` verrebbero bloccati **entrambi**. 👉 Il nuovo tetto vive nell'**include di guardia** e ragiona **per simbolo+LATO** (nucleo puro separato dal filo che legge il terminale, con autotest dedicato): **non duplica A1, lo corregge**. ⚠️ **Resta vero il vincolo della firma: il repo cambia, i binari in campo no**. 💸🆕 **v18 — IL PREZZO DEL P0 NON ANCORA ACCESO HA UN NUMERO, ED E' DI IERI: −100,33 €.** Il 02/09 `EMA200 OTT L1` e `L2` (XAUUSD, stesso lato) sono morte **nello stesso secondo (02:51:18) sullo STESSO prezzo 4300,17**, pur essendo entrate a 4371,07 e 4343,14 con **distanze di stop diverse** (70,90 e 42,97 $) e **livello di stop identico**: 🥇 _"non sono due rischi indipendenti, sono **un rischio solo contato due volte**"_ (conti al centesimo nella pagella). 🔴 **E questo aggiunge una forma che nemmeno il tetto per numero di posizioni vede**: il cap C1 contava **due slot su cinque**, il mercato ne ha spenti **due con un tocco** — cioe' il rischio **effettivo** era 1 evento, l'**esposizione** era doppia. Un tetto simbolo+lato l'avrebbe impedita; il cap globale no. ✅ Nota metodologica che paga: i due ingressi distavano **4,4 ore** — col vecchio criterio "entro 600 s" la coppia sarebbe stata **invisibile**, e il rilevatore riscritto il 01/09 (accoppia sulle **finestre**) l'ha segnalata da solo |

| # | parametro | valore PROPOSTO | fonti (rango) | conflitti | stato |
|---|---|---|---|---|---|
| C9 🆕 | 🕶️ **IL DIFETTO `PositionSelect` SU CONTO HEDGING — 34 EA su 126 non fanno quello che il loro pannello dice** (nuova, v18) | **niente si tocca in forward oggi.** La proposta e' una **coda di fix ordinata per gravita' misurata**, uno alla volta, ognuno con la sua ricompilazione firmata: **(1)** `ABTG_ORB_Ottimizzato` (770611, U30USD, **9 vicini**, difetto misurato 3 volte) → **(2)** `ABTG_MaxMinNotte` (770402 oro, **13 vicini**, motore a OCO) → **(3)** `Gold_Ichimoku_TK_ATR_EA` (250604) → **(4)** i due Aperture 🟠 su D30EUR/U30USD; **+ due voci aggiunte dalla verifica storica**: **A** `ABTG_ORB` **nativo** sale nella coda (e' l'**unico col difetto misurato in forward**), **B** `InpOneTradePerDay` **dichiarato e mai letto** in `ABTG_ORB.mq5:54` e `ABTG_MaxMinNotte.mq5:65` (**il pannello mente**; fix di due righe, **non tocca la selezione delle posizioni → si fa per primo**). La **forma** del fix e' congelata nell'audit: **lettura per ticket E scrittura per ticket, insieme** — farne una sola ricrea la categoria 🟠, che e' il caso peggiore | 🥇 `report/AUDIT_POSITIONSELECT_HEDGING_2026-09-03.md` (**126 file**, codifiche verificate una per una: **26 🔴** `PositionSelect(_Symbol)`+magic · **8 🟠** lettura hedge-safe ma **scrittura per simbolo** · **85 🟢** · **7 ⚪**; la mappa dei vicinati: **XAUUSD ~14 sedie**, **U30USD 10**, **D30EUR 8**, **NASUSD 7**) · 🥇 `report/VERIFICA_CHIUSURE_INCROCIATE_2026-09-03.md` (**1264 posizioni chiuse**, 19+2 chiusure `expert` esaminate **una per una**) · 🥇 `report/ORB_GEMELLI_DIVERGENZA_2026-08-22.md` §03/09 08:19 (**FOTO A**: `LARRY DOW S` 772341 aperta dal 01/09 08:45, viva per tutta la vita del trade ORB → ipotesi A confermata **3 giorni su 3**) | 🔴 **PERCHE' E' RISCHIO E NON GESTIONE**: la stessa `SelPos()` governa **`HandleOCO()`** (da cieco **il pendente opposto non si disarma** → i due lati possono riempirsi entrambi) e **`gHadPos`/`InpOneTradePerDay`** (non si arma mai → rientro nello stesso giorno). **MISURATO in forward**: `ABTG_ORB` nativo ha fatto **20 operazioni in 16 giornate** dove il contratto ne prevede al massimo 16 = **+25% di frequenza**, con **separazione perfetta 16/16** (cieco+2 lati = 4 · vede+1 lato = 12; probabilita' di coincidenza casuale **1 su 1820**). ⚖️ **La parte scomoda, scritta e non nascosta: quelle 4 operazioni non autorizzate hanno GUADAGNATO +201,15 €** — e' **fortuna su un campione di quattro**, non un argomento per tenersi il difetto. ✅ **Due buone notizie misurate**: il secondo lato si e' aperto **sempre dopo** la chiusura del primo (**zero sovrapposizioni** → il "doppio rischio simultaneo" temuto **non si e' verificato**), e il **flatten news non ha mai sparato** (`data/abtg_news.csv` **vuoto**, `InpUseNewsFilter=false`). 🔴🔴 **IL CONFLITTO CHE DECIDE L'ORDINE DEI LAVORI, ed e' con una firma di ieri**: il fix **v1.04 dell'ORB** richiede una **ricompilazione**, e la firma **D1 del 02/09** dice _"fase 1 SENZA ricompilazioni"_ — perche' i binari del 100k sono quelli **dimostrati identici** e ricompilare **brucia il criterio 4** (rischio **X13** della matrice del collaudo). L'ORB `770611` e' **uno dei 5 mirror del 100k**: non e' un caso di frontiera, e' esattamente il file in mezzo. 👉 **Raccomandazione dell'architetto-prop, dichiarata come tale: fixare PRIMA e SOLO sul terminale del CONTO PICCOLO** (dove il difetto morde davvero: 9-14 vicini per simbolo) **lasciando intatti i binari del 100k fino a fase 1 conclusa** — sul 100k l'ORB e' quasi sempre solo, e infatti **traila**. Se questa strada non e' praticabile sul VPS, l'alternativa e' **aspettare la fine della fase 1**: la parola e' di Claudio. ⚠️ **Correzione di premessa, misurata e da tenere**: l'audit dice _"la posizione piu' VECCHIA"_, la verifica misura che e' **quella col `position_id` piu' basso** — che su **28 coppie su 170 (16,5%)** **non** e' la piu' vecchia in orologio, perche' una posizione nata da un **pendente** eredita il ticket di quando l'ordine e' stato *piazzato*. Tutti i nostri motori a BuyStop/SellStop piazzano a inizio sessione: **entrano in lista "vecchi" anche se si riempiono per ultimi** | 📋 **PROPOSTO (v18)** — **i due referti non hanno toccato nessun EA**, per mandato. Cio' che serve e' **una firma sull'ORDINE e sul PERIMETRO** (quale terminale, quando), non sul merito tecnico: la forma del fix e' gia' scritta e il **test di non-regressione e' dichiarato prima** (nel tester il difetto e' **invisibile** — nessun vicino → il risultato atteso e' **identico al centesimo**; se cambia, il fix ha rotto altro). 🛠️🆕 **AGGIORNAMENTO DELLE 08:25 — IL PRIMO FIX DELLA CODA E' SCRITTO** (commit `718a9d8`, cantiere dell'altra sessione): `ABTG_ORB_Ottimizzato.mq5` e' a **v1.04**, con lettura **e** scrittura per ticket **nello stesso commit** (nucleo puro autotestabile + guscio sottile, **10 blocchi / 33 casi** di autotest, nessun `PositionSelect/Close/Modify(_Symbol)` rimasto nel file) e un log nuovo — **`ORB SELEZIONE:`** — che stampa **quale ticket di quale magic avrebbe agganciato la v1.03**: e' cosi' che il fix si verifica in campo al primo trade con vicini. 🛑 **E' SOLO REPO: non compilato, non in forward, nessun terminale toccato.** 👉 **La decisione di Claudio non e' piu' "si fa o non si fa": e' SE e DOVE si ricompila** — ed e' esattamente il conflitto con **D1** scritto qui accanto. 🛑 **E cosa NON si promette**: non piu' profitto, ma che **l'EA faccia quello che dice il suo pannello** |

> 🧾 **Nota di lettura su C9, perche' tocca due cose che il piano gia' contava.**
> **(1) I numeri di forward delle sedie 🔴 sono SPORCHI, non solo peggiori**: le
> famiglie ☠️ "morte in osservazione" (`Live5m`, `ORB`, `ORB_Fibo`) sui giorni
> con vicini **non hanno mai eseguito la loro gestione** — se un domani si legge
> il loro forward, va detto che il campione e' inquinato (limite dichiarato
> dall'audit, non da noi). **(2) La corsia MERITO della C3 va letta con questa
> lente**: l'ORB nativo ha fatto **+25% di operazioni** rispetto al contratto, e
> `IchiCross_Gold_722` — l'EA della strategia personale, **non ancora in campo** —
> **nascerebbe cieco** su XAUUSD, il simbolo piu' affollato della flotta.
> 👉 Va corretto **prima** del deploy, non dopo.

| # | parametro | valore PROPOSTO | fonti (rango) | conflitti | stato |
|---|---|---|---|---|---|
| C10 🆕 | 🧱 **TETTO DI RISCHIO APERTO PER CLUSTER CORRELATO** (nuova, v20) | **3,0%** per cluster (valuta comune, o classe: indici / metalli / JPY-crosses...), **accanto** al cap complessivo C1 = 3,25% gia' firmato il 18/08 — **i due non si parlano: vince il piu' stringente**. 🔴 **Il valore e' firmato; i CLUSTER no**: quali simboli stanno insieme e' **una scelta che va firmata a parte**, ed e' il primo dei tre passi mancanti | ✒️🥇 `report/FIRME_2026-09-07.md` §"tetto per cluster" (firmato con la stessa riga del pavimento di frequenza) · 🥇 la misura che lo motiva: i due portafogli "prop firm ready" a larga base letti dalla caccia hanno DD **MISURATI del 32,59% e del 45,64%** per saldo — _"la larghezza senza controllo della correlazione e' la trappola"_ · 🥇 sorgente `mql5/Experts/ABTG_Guardian.mq5`: `OpenRiskPct()` somma gli SL vivi **senza nessuna nozione di cluster** | 🔴🔴 **IL CONFLITTO E' CON SE STESSO, E VA DETTO OGNI VOLTA CHE SI CITA QUESTA RIGA: e' FIRMATO e IMPLEMENTATO, ma NON COMPILATO e NON COLLAUDATO — quindi NON ATTIVO.** In campo il Guardian che gira e' ancora la **v1.12**, che conosce il cap complessivo (`InpMaxOpenRiskPct=3.25`) e il tetto simbolo+lato (C8), **non i cluster**. Finche' non e' compilato e collaudato **e' codice spento, non una protezione**. ⚠️ E c'e' una tensione **interna alle due firme dello stesso giorno**: il pavimento per FAMIGLIA (H13) spinge verso **piu' simboli**, che e' esattamente la condizione in cui il tetto di cluster serve — **la firma che allarga e' attiva, quella che protegge no** | 🔴 **FIRMATO (07/09/2026) · IMPLEMENTATO (stessa giornata) · NON COMPILATO · NON COLLAUDATO = NON ATTIVO** — i tre passi: **(1)** 🟠 definire i cluster = **PROPOSTO, NON FIRMATO** (`report/CLUSTER_PROPOSTA.md`) · **(2)** ✅ implementarlo = **FATTO** (`ABTG_Guardian.mq5` **v1.13** + `ABTG_PausaGuardian.mqh` **v1.60**, commit `cdb2037`; input nuovi **spenti di default**, `InpMaxClusterRiskPct=0` e `InpClusterMappa=""`, tutto il codice nuovo dentro `if(StringLen(cluster_mappa)>0)` → **no-op verificato**) · **(3)** ❌ collaudarlo = **DA FARE sul PC/VPS di Claudio** (`backtest_pipeline/attese_cluster.txt`), col metro dell'enforcement e non con _"sembra funzionare"_. ⚙️ **Lavoro di un altro agente**: qui e' registrato, non duplicato |
...
🔴 **AGGIORNAMENTO 05/09 — le due righe FORWARD sono OTTIMISTE, confermato dal
broker:** BCM ha risposto direttamente (contatto diretto, non fonte di
seconda mano) che **sul conto demo non c'e' slippage, solo sul conto reale**
(dettaglio completo in `backtest_pipeline/caccia_strategie/CONFIG_PROP_SPREAD_SLIPPAGE_2026-09-05.md`,
domanda D6). Le due righe **FORWARD 100k** e **FORWARD conto piccolo** qui
sopra mancano quindi per costruzione del costo di esecuzione che R109 aveva
misurato a **21,5 punti su un solo stop** — un costo che sul reale c'e'
sempre e sulla demo non compare mai. **Non risolve il conflitto di segno**
(mancare slippage renderebbe il forward PIU' ottimista, non spiega perche'
il piccolo sia negativo), ma e' un'aggravante che va dichiarata ogni volta:
**il vero E, sul conto che conta, e' probabilmente peggiore di entrambe
le righe FORWARD scritte qui.** Il rischio di drawdown vero, finche' non
c'e' un conto reale, lo dice solo il backtest a tick con lo slippage
simulato (gia' la prassi dei round R109/R116/R117), mai il forward demo da solo.
...
### 🆕 03/09 — IL VIVAIO IN CHIARO: **un morto per misura, un superstite, due in coda**

_Tutto quello che segue e' successo fra le 06:35 e le 10:08 del 03/09. Nessun
candidato e' in forward, nessuno e' promosso: sono **conteggi**, che e' esattamente
cio' che il passo 0 deve produrre._

| candidato | esito del PASSO 0 | il numero che decide | dove sta adesso |
|---|---|---|---|
| 🪦 **RSI+EMA V8** | **NON PROMOSSO — verdetto di carta CONFERMATO DA UNA MISURA** | **l'ABLAZIONE**: il filtro RSI toglie solo il **9-13%** degli incroci EMA (7 corse su 7) → il V8 **e', nei numeri, un incrocio EMA(5/20)**, famiglia SuperWave/ChaosLyapunov **gia' morta due volte in casa**. La frequenza non e' mai stata il problema (2,0-6,6 segnali/giorno/lato) | **registro dei caduti.** 🚫 Niente seconda griglia (regola della seconda caccia). ✅ Restano vivi **l'esperimento MANUALE di Claudio** (che misura _"Claudio col V8"_, non il V8 nudo) e l'uso del segnale come **sveglia** |
| 🥇 **LondonFx** | **PRIMO SUPERSTITE del passo 0 della missione frequenza — su DUE simboli** | EURUSD **19/24** righe vive · GBPUSD **24/24** · la riga regina **EUR M15, RSI, ora 8**: **2,26 segnali/giorno**, MFE **13,4** / MAE 11,8 pip, RR 1,136 · e soprattutto **il filtro FILTRA: −73/77%** degli segnali nudi (l'opposto esatto del V8) | ✍️ **round a TICK R116 FIRMATO** (03/09 ~09:45) + **EA contenitore costruito** (`ABTG_LondonFx.mq5` v1.00, **2.012 righe, repo-only, MAI compilato**) |
| 🟢 **RELATIVO** | promosso **8/10** dalla 4ª battuta (fronte GitHub/TradingView) — **passo 0 da fare** | e' **il primo motore in quattro battute il cui segnale non esiste senza un secondo strumento** (z-score del rapporto `D30EUR/U30USD`, `NASUSD/U30USD`): la **frequenza e' una manopola** (la finestra `n`), non una speranza. Costo onesto: **niente da portare, tutto da scrivere** | **in coda alle sonde**, coi cancelli gia' congelati: **< 2,00 attraversamenti/giorno = SCARTO** · **RR < 0,70 = scarto per aritmetica** · e la domanda che uccide o salva la tesi: **quanta parte delle divergenze NON converge entro fine sessione** |
| 🟠 **VGRSI** | promosso **6/10 IN CODA** (fronte Code Base/paper) — **passo 0 da fare** | l'unico oggetto delle 4 battute con **frequenza dichiarata sopra il pavimento (3,3-4,8/giorno) sui NOSTRI simboli dentro MT5** con spread e commissioni reali | **in coda.** 🔴 Rovescio scritto prima dei complimenti: i numeri dell'autore nascono da una **ri-ottimizzazione settimanale** (= scelta del **picco**, che in casa e' misurata **anti-predittiva**, 12 Spearman IS→OOS negative su 13) e i suoi **DD dichiarati sono al muro o sopra** (DJI30 **18%**, EURUSD 12%, XAUUSD 10% contro un muro prop del 10%). 👉 **Dell'autore si prende l'INDICATORE, non il P/L** |
| ⏸️ **DayFlow VWAP Relay** | **9/10 di carta**, sonda **non ancora costruita** (~4-5 h) | invariato dal v17 (la tensione **M5 frequenza vs M15 geometria**) | **scavalcato da LondonFx**, e per un motivo misurato: LondonFx ha **numeri di passo 0 su due simboli**, DayFlow ha una lettura di sorgente |

🧾 **Le tre onesta' di LondonFx, prese dal referto e non da noi** — perche' un
superstite del passo 0 **non e' un edge**: **(1)** MFE ≈ MAE quasi ovunque, la
**geometria da sola non regala niente** — il passo 0 conta **occasioni**, il
merito lo decide **solo il tick**; **(2)** il nostro short RSI a **20** e' **piu'
permissivo dell'autore (10)** — lo short passa F1 anche per questo, ed e' la
scelta che Claudio ha firmato esplicitamente (**F5 = A**, soglia simmetrica);
**(3)** la fascia SOSPESA di M5 su EURUSD dipendeva **dallo spread mai
misurato** — e quel buco, sugli **indici**, e' stato chiuso poche ore dopo
(**H12**), ma **sul forex e' ancora aperto** (→ **M36**).

🏛️ **Cosa dice R116 in ottica PROP** (criteri congelati **prima** dei numeri,
`LONDONFX_TICK_CRITERI.md`): rischio **0,65%** = la taglia di casa · **cap
2%/giorno acceso dentro il motore** (che LondonFx ha e DayFlow no) · **flat non
disattivabile** · **una posizione**, tetto **6 trade/giorno** · **niente
gestione** (non e' un input a `false`: e' codice che non c'e') · lo **spread
registrato all'ingresso** in colonna (mediana e P95: `InpMaxSpread=0`, _"lo
spread si MISURA, non si filtra"_) · **slippage simulato sulla geometria**
(R55-bis dentro il round, non dopo) · e l'EA e' **hedge-safe dalla nascita** —
zero `PositionSelect(_Symbol)`, scrittura **solo per ticket**, cioe' nasce gia'
fuori dal difetto **C9** censito la mattina stessa. ⚠️ **Non e' compilato**:
sintassi, runtime e frequenza del terzo motore sono **tutti da verificare**.
...
| I2 🆕 | ⏱️ **`ExecutionMode` DEL TESTER — il ritardo di esecuzione, mai usato in 39 round e MISURATO il 07/09** | **0 (nessun ritardo) in tutti i round**, scritto in **tutti e due** i blocchi `[Tester]` di `walkforward_generico.ps1` **dal 07/08/2026**. 📌 **Correzione di formulazione, dovuta**: *non era uno stato nascosto* — era **dichiarato nell'`.ini`**, e **nessun referto l'ha mai letto**. I valori possibili: `0` normale · `-1` casuale · `1..600000` millisecondi | 🥇 `REFERTO_RITARDO_R119_PRIMO_GIRO.md` §2 e §4 · contratto congelato `backtest_pipeline/prove/RITARDO_TESTER_R119.txt` (G0-G2, S1-S4) · classe **156** in `backtest_pipeline/CHECKLIST_RIGA_DI_LANCIO.md` | 🔴 **Il primo tentativo di misurarlo e' FALLITO, e il fallimento va scritto**: la chiave usata si chiamava **`Delay`** (il nome della tendina nel tester), **che in MT5 non esiste**; **MT5 ignora in silenzio le chiavi `[Tester]` che non conosce** — niente errore, uscita 0, CSV prodotti, gemelli a posto, referto verde. 🟢 Il **canarino** — scritto nel contratto **prima dei numeri**, come codice fra le due corse (classe 151) — ha detto `IGNORATA` e ha fermato la spesa a **2 corse su 8**, risparmiando **6 corse a tick**. *Senza, il referto avrebbe detto "le due sedie vive reggono fino a 500 ms" — una bugia con i numeri sotto.* ⚠️ **`NON MISURATO` non vuol dire `immune`** | ✅ **MISURATO (07/09 sera, secondo giro `..._v3_EXECMODE`)** — G0 **MORDE**, **16/16 CSV**, gemelli **IDENTICI 8/8**, **0 rilievi**. 🔴 **E l'ipotesi congelata e' FALSIFICATA su tutte e due le meta'**: lo **STOP (`770611`) e' immune** (OOS **identico alla cifra** fino a 500 ms), il **LIMIT (`770101`) e' quello che si muove** — **≤2% sul profitto, ≤3% sul DD**, e **in MEGLIO** (+1,5% di profitto e **−0,8% di DD** in OOS a 500 ms, con **un trade in piu'** in IS). ⚠️ **Il meccanismo NON e' spiegato** (un ritardo che migliora non e' un costo) e **il verdetto formale contro S1-S4 non e' ancora scritto** — il referto macchina dichiara di **non giudicare**. ✅ Il default resta `ExecutionMode=0` **byte per byte**: **tutti i round gia' pinnati riproducono identici** → **M38** |
...
# 🪑 AREA J — LE DUE SEDIE SBLOCCATE IL 11/09, e cosa resta appeso

> **Da dove nasce questo blocco.** `report/PIANO_CHALLENGE_OTTOBRE_v2.md` (11/09)
> ha misurato che **nessun round di quel giro muove il numero di sedie
> schierabili** e che le azioni con **delta +1** erano **tre**, su tre sedie
> bloccate da tre requisiti diversi. Due non richiedevano la firma di Claudio:
> **scrivere il preset su file di `770202`** e **sciogliere il conflitto sul DD
> promesso di `771531`**. Questo blocco le chiude. La terza (geometria OPPRANGE
> di `770611`) resta a Claudio.

| # | parametro | valore PROPOSTO | fonti (rango) | conflitti | stato |
|---|---|---|---|---|---|
| **J1** 🆕 | 📁 **IL PRESET SU FILE DI `770202`** (Dow Apertura, U30USD M5, conto **100k 50504263**, cartella `-V3`, profilo `SQUADRA 100K`) | ✅ **SCRITTO**: `mql5/Presets/ABTG_Dow_Apertura_US_U30USD_M5_770202_100K.set` — **81 righe `Inp*`, COPIATE** dalla foto del `.chr` vivo, **zero valori scelti da me** (verifica meccanica: `diff` fra il blocco del log e le righe del preset = **vuoto**). Porta `InpRiskPercent=0.65` e `InpAllowShort=false`, che prima **vivevano solo dentro un `.chr`** | 🥇 `backtest_pipeline/coda/referti/CODA_08_preset_dai_chr_20260911_033002.log` righe **3267-3386** (lettura dei `.chr`, nessuna scrittura) · 🥇 `CODA_01_..._20260911` r.74 (simbolo/TF/rischio) · 🥇 `CODA_03_..._20260911` (cartella → **conto 50504263**, letto dai giornali) · 🥇 `CODA_06_..._20260911` r.256 (binario in campo) · 🥇 `backtest_pipeline/prove/R16c_pertrade_Dow.txt` (la cella del round) | 🟠 **DUE DIFFERENZE DICHIARATE, nessuna appianata.** (1) Il log stampa **113** parametri, il preset ne ha **81**: i **32** esclusi **non sono dell'EA** — sono gli input dell'indicatore `mql5/Indicators/ABTG_Look.mq5` appeso al grafico, che lo strumento `CODA_08` **accorpa al blocco `<expert>`** (prova: compaiono identici sotto `ABTG_TradeExporter`, che ha **4** input in tutto; e `InpVerbose` comparirebbe **due volte**, cosa che in MQL5 non compila). 🔴 **E' un difetto dello strumento** → **M41**. (2) Lo **stesso magic 770202** gira anche sul **piccolo 50503392** con **due** campi diversi (`InpRiskPercent` 1.0 e `InpUsaGuardian` **assente**, perche' quel binario e' la v1.00 dell'08/08 che l'input non ce l'ha): 79 campi su 81 identici carattere per carattere. **Un preset per il piccolo NON e' questo file con 0.65→1.0** e non l'ho scritto: sarebbe un file dedotto | 📋 **PROPOSTO** — il **file esiste nel repo** (scriverlo e' un gesto sul repo, non sul campo). 🔴 **Caricarlo nel terminale 50504263 e' un gesto sul campo e resta a Claudio.** Il preset **non cambia niente da solo**: oggi fotografa cio' che gia' gira |
| **J1-bis** 🆕 | 🧬 **R1 DI `770202`: esiste un round che ha promosso ESATTAMENTE cio' che gira?** | ✅ **SI', ed e' verificato campo per campo** (non citato: confrontato). I campi del vivo che **non** sono il default compilato sono **NOVE**: `InpRangeMinutes` 35 · `InpEntryMode` 2 · `InpBufferPoints` 1000 · `InpAllowShort` false · `InpRetestOffsetPts` 400 · `InpTP1_R` 1.0 · `InpTP1_ClosePct` 50 · `InpBreakevenAtTP1` true · `InpRiskPercent` 0.65. **OTTO su nove sono fissati da `R16c_pertrade_Dow.txt` allo stesso identico valore**; il nono e' la taglia (**banco 1,0% / campo 0,65%**, la A1 firmata il 09/08). Gli altri **72** campi sono i default compilati, cioe' quelli con cui R16c e' girato | 🥇 `prove/R16c_pertrade_Dow.txt` (`@SIMBOLO U30USD @PERIODO M5 @DAQUANDO 2024.09.26`) · 🥇 `mql5/Experts/ABTG_Dow_Apertura_US.mq5` righe **57-69** (i `#define` in vigore) | ⚠️ **Una riga del piano di ottobre va corretta**: `PIANO_CHALLENGE_OTTOBRE_v2.md` r.131 dice _"il sorgente porta `ABTG_DEF_RISK 2.0`"_. In **questo** EA il default e' **1.0** (riga 67); il **2.0** e' il fallback generico del blocco `#ifndef` (riga 140), che qui **non si applica**. La sostanza regge (il default **non** e' 0,65), il numero no | ✅ **MISURATO** |
| **J1-ter** 🆕 | 🧊 **QUALE BINARIO DESCRIVE IL PRESET DI `770202`** — il contro-esempio del "campo fermo ad agosto" | 🟢 **Su QUESTO terminale la notizia e' buona, ed e' un numero**: `CODA_06` r.256 legge sul `-V3` **`ABTG_Dow_Apertura_US.mq5` v1.01, 2148 righe, GUARD SI, compilato il 19/08 23:09**; il repo ha **v1.01, 2147 righe**, e **l'ultimo commit che tocca quel file e' `d83c196` del 19/08** (Migrazione Guardian, famiglia Apertura). 👉 **Scarto di UNA riga e stessa data.** Conferma indipendente: il `.chr` del `-V3` contiene `InpUsaGuardian` **nell'ordine giusto**, quello del piccolo no (112 parametri contro 113) | 🥇 `CODA_06_..._20260911` r.256 e r.69 · 🥇 `git log -- mql5/Experts/ABTG_Dow_Apertura_US.mq5` · 🥇 `report/IL_CAMPO_E_FERMO_A_AGOSTO_2026-09-11.md` | ⚠️ **Il referto "42 su 42" guarda il terminale del PICCOLO, non il `-V3`**: questa sedia **non e' fra le 42 ferme ad agosto in senso pieno**. 🔴 **Restano due limiti, gli stessi del referto**: MT5 esegue l'**`.ex5`**, e il `.mq5` accanto **non e' una prova del contenuto del binario**; e **lo scarto di 1 riga non e' spiegato** (probabile riga finale senza a-capo, **NON VERIFICATO**) | 🟡 **PARZIALE** — il piu' allineato della flotta, non "certificato" |
| **J2** 🆕 | 💥 **IL DD PROMESSO DI `771531` (EMA200 U30USD H1): IL CONFLITTO E' SCIOLTO — le misure non si contraddicono, descrivono TRE SEDIE DIVERSE** | **Il numero che descrive la sedia PIANIFICATA per la challenge (100k, 0,65%): DD OOS `7,8323%` a rischio 1,0%**, deposito 100.000, **tick reali**, finestra `2024.09.26→2026.06.30` split 40/60 (**OOS 10/06/2025→30/06/2026**), cella CENTRO (`O1 0,20 / O2 0,3 / TP 2,0`), **n = 517 uscite = 257 posizioni MISURATE** sui `position_id`. Alla taglia 0,65% fa **~5,09%** — 🔴 **[APPROSSIMATO lineare]**, convenzione di casa, **non una misura** | 🥇 `R112_CORSA_20260826/REFERTO_R112.txt` (intestazione: finestra, split, **modello 4**, **deposito 100000**) + `ABTG_EMA200_U30USD_OOS_00_metro.csv` (riga `Pass 1`: `7.8323`, `517`) · 🥇 `R112_REFERTO.md` §G0-B · 🥇 `risultati_prove/ABTG_EMA200/*_r29b.csv` (pass 17: `7.2138`, `444`) · 🥇 `prove/R31_pertrade_ema200_dow.txt` r.4 e r.10 (_"Attesi (deposito 10k)"_, lancio `-Deposito 100000`) · 🥇 `R103_REFERTO_DRIVER_BLOCCO1_INDICI_20260824_1348.txt` r.78 e r.330-340 (**modello 1 = OHLC**, finestra **senza split**) · 🥇 `report/DOSSIER_SCHIERAMENTO_EMA200_DOW_2026-09-09.md` §1 (che aveva gia' messo in fila le righe) | ✅ **NESSUNA COPPIA DELLA STESSA CONFIGURAZIONE DA' DUE NUMERI.** Vedi la tabella J2-bis: le tre misure differiscono per **modello** (OHLC vs tick), **deposito** (10.000 vs 100.000) e **finestra** (piena vs OOS). E la prova che il deposito e' **parte della configurazione**, non un dettaglio, e' che **cambia l'insieme dei trade**: `n` 444 a 10k contro **517** a 100k, stesso giorno, stessa cella, stesso banco (`R31`: _"Chiusure 517 vs 444 di R29: i PARZIALI a taglia 100k"_). 🔴 **MA IL CONFLITTO SI RIAPRE SE QUALCUNO LE NORMALIZZA**: a 10k il `MathFloor` sullo step 0,10 taglia il lotto e la sedia rischia **~0,87%** invece dell'1,00% → riportato a 1,00% col metro lineare di casa il 7,2138% diventa **~8,29%**, che **non coincide** col 7,8323%. 👉 **I tre numeri stanno in piedi come DESCRIZIONI di tre sedie; NON stanno in piedi come tre STIME della stessa sedia.** Il metro lineare DD~rischio, qui, **sbaglia del 6%** | ✅ **CHIUSO COME CONFLITTO** — 🔴 **ma con un buco dichiarato**: vedi **J2-ter** |
| **J2-bis** 🆕 | 📋 **LE MISURE DI DD DI `771531`, TUTTE, CON ACCANTO COSA DESCRIVONO** | **R103** (24/08) · 🔴 **OHLC M1** · dep. 100.000 · 1% · **finestra PIENA** `2024.09.26→2026.06.30` **senza split** · n **712 uscite** · **DD 6,48%** (DD sul saldo chiuso 6,16%) → **limite inferiore, NON un verdetto**, e 🔴 **classe 224: senza split non ha fuori campione** · • **R29** (12/08) · 🟢 tick · dep. **10.000** · 1% · OOS 40/60 · n **444 uscite** (~222 posizioni **[INFERITO]**) · **DD 7,2138%** → **la sedia RIMPICCIOLITA dal pavimento del lotto** · • **R31 / R110 / R112** (12/08 e 26/08) · 🟢 tick · dep. **100.000** · 1% · stesso OOS · n **517 uscite = 257 posizioni [MISURATO]** · **DD 7,8323%** → **il numero della sedia a taglia prop**, ed e' **l'unico della flotta riprodotto TRE volte al centesimo** (R112 lo certifica come **primo G0-B applicabile della storia della macchina**) | 🥇 le fonti di J2 · 🥇 `R112_REFERTO.md` §3 (_"il `n` dell'OPTFRAME conta i DEAL DI USCITA, non le posizioni: 517 righe per 257 posizioni distinte"_) | 🟢 **L'ORDINE DEI TRE NUMERI HA UN VERSO FISICO, e questo e' il controllo che li tiene insieme**: `OHLC 6,48% < tick@10k 7,21% < tick@100k 7,83%`. L'OHLC **sottostima per costruzione** (non vede il percorso intra-barra); il 10k **rischia meno del dichiarato** perche' il `MathFloor` taglia il lotto. Se l'ordine fosse un altro, la spiegazione non reggerebbe: **e' un controllo che poteva fallire, e non fallisce** | ✅ **MISURATO** (salvo il `~222` di R29, marcato **[INFERITO]**: il rapporto uscite/posizioni **non e' costante** — misurati **2,01 / 2,16 / 2,25 / 2,31**, classe 226 — quindi per R29 **non si divide per due**, si dichiara la forbice) |
| **J2-ter** 🆕 | 🕳️ **IL BUCO CHE RESTA, ed e' il piu' importante: NESSUNA DELLE TRE MISURE DESCRIVE LA SEDIA CHE GIRA OGGI** | `771531` gira sul **PICCOLO 50503392** (profilo `ORO`, `chart33.chr`) a **`InpRiskPercent=1.0`**, su un saldo di **~5.000 EUR**. I banchi di casa girano a **10.000** e **100.000**: 🔴 **alla taglia vera non esiste NESSUNA misura.** Il piu' vicino e' R29 (10.000, **il doppio** del saldo vero) | 🥇 `CODA_08_..._20260911` righe **2087-2167** (il `.chr` vivo: `InpRiskPercent=1.0`, `InpOrder1Atr=0.2`, `InpOrder2Atr=0.3`, `InpTP_RR=2.0` — **la cella R29/R112 alla lettera**) · 🥇 `CODA_01_..._20260911` r.43 · 🥇 classe **229** in `backtest_pipeline/CHECKLIST_RIGA_DI_LANCIO.md` | ⚠️ **CORREZIONE A UNA PREMESSA CHE CIRCOLAVA OGGI**: l'**1,62% contro l'1,00%** della classe 229 **NON e' di questa sedia**. E' di **`EMA200_Ottimizzato` `971501` su XAUUSD** (classe **228**: gambe da 0,0049 e 0,0082 lotti alzate a 0,01; le soglie 10.983/6.656 EUR sono quelle due gambe convertite in euro). 🟢 **Su `771531`/U30USD il pavimento NON morde, ed e' misurato sui P/L veri**: il 04/09 le due gambe sono andate in campo a **0,20 e 0,30 lotti** (2× e 3× il `VOLUME_MIN` di 0,10) per **−20,15 EUR ciascuna** (`DIARIO.md` 04/09). 🔴 **Ma la classe 229 morde lo stesso, in un altro punto**: il binario in campo sul piccolo e' **`ABTG_EMA200.mq5` v1.00, 487 righe, compilato il 06/08** (`CODA_06` r.71), mentre il repo ne ha **552** — e **fra le due c'e' il commit `3af47ed` dell'08/08, _"fix sizing su 41 EA: `OrderCalcProfit` al posto del tick value nudo"_**. 👉 **Il codice che DIMENSIONA IL LOTTO in campo non e' quello con cui e' stato misurato il 7,8323%** | 🔓 **APERTO** — si chiude con **una corsa sola**: stessa cella, stesso banco, **`-Deposito 5000`** (e, se si vuole il numero della sedia pianificata, **`-Deposito 100000 -Rischio 0,65`** per sostituire l'approssimato lineare con una misura) → **M40** |
| **J3** 🆕 | 📜 **IL CONTRATTO SCRITTO DI `771531` E' ANCORA QUELLO VECCHIO** | `report/CENSIMENTO_CONTRATTI.md` r.130 promette **DD 7,21% · n 444**, con la nota _"[deposito NON DICHIARATO → default driver 10.000 €]"_. 👉 **PROPOSTA**: portarlo a **7,8323% · n 517 uscite / 257 posizioni · deposito 100.000 · tick**, **dichiarando la taglia accanto al numero** | 🥇 `CENSIMENTO_CONTRATTI.md` r.130 · 🥇 `DOSSIER_SCHIERAMENTO_EMA200_DOW_2026-09-09.md` §1.1 (che l'aveva gia' proposto il 09/09 e **non e' stato eseguito**) | ⚠️ **Non e' un cavillo**: la corsia **RISCHIO** del criterio di uscita del 18/08 scatta quando il **DD forward supera il DD promesso**. Con il contratto vecchio un forward al **7,5%** sarebbe una **violazione**; col contratto giusto **non lo e'**. 🔴 E vale anche il contrario: il contratto **non si alza** per far stare dentro un forward brutto — si alza perche' **la misura a quella taglia dice 7,83%** | 📋 **PROPOSTO** — cambia un **numero di contratto**, quindi **serve la riga di Claudio**. Zero macchina |

---

# 🌙 AREA K — LA NOTTE 12/13-09: IL TAPPO SI E' SCIOLTO, E UN ALLARME VA RIDIMENSIONATO (nuova, v22 — 13/09/2026)

> **Da dove nasce questo blocco.** Tre cose sono arrivate nello stesso giro e
> toccano tutte e tre lo **stato** di parametri gia' scritti nel piano, non il
> loro valore: **(1)** il referto del runner della notte 12/13-09 dice che i
> round che il piano dava per *"mai girati"* **sono girati**; **(2)** il rilievo
> sui 5 round a **codice d'uscita 3** e' stato **riletto nel codice** e la sua
> causa piu' probabile **non e' quella scritta ieri**; **(3)** il referto
> `IL_CAMPO_E_FERMO_A_AGOSTO` ha una conseguenza che il piano di ottobre
> **sottostima**. 🛑 Nessuna riga di quest'area tocca il forward: le azioni che
> lo toccano sono marcate **👤 Claudio**.

| # | parametro | valore PROPOSTO | fonti (rango) | conflitti | stato |
|---|---|---|---|---|---|
| **K1** 🆕 | 🌙 **LO STATO VERO DEI ROUND DELLA NOTTE 12/13-09** — il piano (e `IL_TAPPO_2026-09-11.md`) li dava per **"mai girati"** | 🟢 **GIRATI. Il conto esatto, ricavato riga per riga dal referto e non dal riassunto: 48 righe di coda, 42 eseguite, di cui 36 in corsia ROUND.** Delle 36: **1 e' un giro a vuoto** (`canfrz`, lanciato `-SoloControllo` → *"MT5 NON e' stato aperto"*, r.665-667 del driver: **non e' una misura**), **34 hanno prodotto CSV validi** (29 a uscita 0 + **5 a uscita 3, che per costruzione hanno CSV freschi, non vuoti e con Trades>0**, perche' il cancello su `$mancanti`/`$zero` viene **prima**, r.739-747), **1 sola e' NON MISURATA** (`cemad02`, uscita 2). 👉 **Lo stato da scrivere non e' "bloccati" ma: `girati, CSV sul VPS, in attesa di caricamento`** | 🥇 `backtest_pipeline/coda/referti/REFERTO_RUNNER_20260913_033003.txt` (riepilogo: *eseguiti 42, di cui ROUND 36, rifiutati 0, falliti 6*) · 🥇 `backtest_pipeline/righe/RIGA_ROUND_VPS.ps1` r.665-670 (`-SoloControllo`), r.739-750 (ordine dei cancelli), r.835-837 (codici) · 🥇 `report/IL_TAPPO_2026-09-11.md` (lo stato precedente) | ⚠️ **CONFLITTO DI STATO, non di numero, e va scritto:** `IL_TAPPO_2026-09-11.md` (11/09) elenca `R120 a-e`, `R125 a-b`, `R126 a-b`, `R127 a-b` come **"mai girati"**. Al 13/09 **R120b (4 varianti), R120e (2), R126a/b/d, R127b/c sono girati**; **R125 a-b NO** (non compare in questo referto) e **R127a NO**. 🔴 E i CSV **non sono nel repo**: il runner pubblica il referto e **mai** i CSV (**classe 307**). Finche' non arrivano, ogni numero di questi round e' **[NON MISURATO]** — girato ≠ letto | 📋 **PROPOSTO** — correzione di **stato** in tutti i punti del piano che dicono *"mai girato"* per queste etichette. 🔴 Il passo che manca e' **👤 Claudio**: `carica_risultati.ps1` dal VPS → **M44** |
| **K2** 🆕 | 🚨 **I 5 ROUND A CODICE 3: L'ALLARME "UN TERMINALE NON BERSAGLIO E' SPARITO" — RIDIMENSIONATO DA UNA MISURA, NON DA UN'OPINIONE** | 🔴 **La lettura di ieri e' un'ATTRIBUZIONE, non una lettura del codice.** Verificato alla fonte: l'uscita 3 scatta su `if($persi.Count -gt 0 **-or** $RILIEVI.Count -gt 0)` (r.745) — cioe' su **QUALUNQUE** rilievo, e i punti che ne generano uno sono **TRE**, non uno: r.408 (`-Modello` diverso da 4: *"screening, non verdetto"*), r.572 (tetto barre non verificabile), r.725 (PID spariti). 🟢 **E la prova che sceglie fra le due spiegazioni e' una tabella incrociata di stanotte: i 5 round a uscita 3 sono ESATTAMENTE i 5 round lanciati a `-Modello 1`, e TUTTI E 31 gli altri girano a `-Modello 4`.** Correlazione **5/5 e 0/31**. 👉 **La causa parsimoniosa dell'uscita 3 e' il rilievo di r.408, che scatta per costruzione su ogni round OHLC: non serve nessun terminale sparito per spiegare i cinque eventi** | 🥇 `RIGA_ROUND_VPS.ps1` r.114 (`function Rilievo`), r.403-409, r.565-572, r.717-725, **r.745-746**, r.835-837 — letti, non citati · 🥇 incrocio `-Modello` × codice d'uscita sulle 36 righe ROUND del referto: `(4,0)=30 · (1,3)=5 · (4,2)=1` · 🥇 `backtest_pipeline/coda/referti/RIGA_SOTTILE_ROUND_20260913_033003.log` r.138 (*"STESSI PID: forward e conto reale NON sono stati toccati"* — ma e' il log del **solo ultimo round**, `r142c`, classe 307) | 🔴 **CONFLITTO APERTO CON DUE FONTI DI RANGO 🥇 DI IERI, dichiarato per nome:** `report/RILIEVO_TERMINALI_NOTTE_2026-09-13.md` e la **classe 309** della checklist scrivono che il codice 3 significa *"**esclusivamente**"* un PID non bersaglio sparito. **Quella parola e' falsificata dal codice** (r.745 e' un `-or` su tre sorgenti di rilievo). ⚠️ **E il contro-esempio contro ME STESSO, costruito e dichiarato**: la mia spiegazione **non esclude** che un PID sia ANCHE sparito — i due rilievi possono essere scattati insieme, e il log per-round e' stato sovrascritto (classe 307). 🟢 Ma l'ipotesi "terminale sparito" deve far sparire e **rientrare** un PID **cinque volte**, ogni volta **esattamente** durante un round OHLC e **mai** durante i 31 a tick reali — e `r126a` (uscita **0**) gira **in mezzo** alle due finestre citate, fra `r127c` e `r127b`. **Una causa spiega tutto, l'altra chiede cinque coincidenze** | 🔓 **APERTO, ma con la priorita' ABBASSATA e la via di chiusura piu' corta di prima.** 🔴 Non lo chiudo io: si chiude con **una riga di lettura** — vedi **M42**. E la prova e' gia' scritta negli zip: il referto per-round porta l'esito **col numero di rilievi**, `ROUND GIRATO CON RILIEVI (N)` (r.746). **N=1 → era solo il Modello. N=2 → allora e' anche un PID, e allora e' una cosa seria.** ⚖️ Fino ad allora: **nessun allarme, nessuna archiviazione** |
| **K3** 🆕 | 🧊 **IL CAMPO FERMO AD AGOSTO — NEL PIANO VALE "COMPILAZIONE R4, +0 SEDIE, 15/09". E' SOTTOSTIMATO, ED E' IL PUNTO PIU' GRAVE DEL GIRO** | 🔴 **Quello che il piano di ottobre chiama `A5 / firma n.4` e' "F7 + ricarico di **15 sorgenti** corretti l'08/09" (il fix del lotto doppio). Ma i sorgenti che in campo sono diversi dal repo sono `42 su 42`, e il piu' vecchio e' la PROTEZIONE.** 👉 **PROPOSTA (non eseguibile da un agente: tocca il forward):** trattare **"riallineare il campo, Guardian per primo"** come **priorita' pari o superiore a qualunque round nuovo** — perche' un round compra un numero **per il futuro**, mentre questo dice se le protezioni **gia' firmate** esistono **oggi**. 🔴 **MA il COME resta quello del referto, non "ricompila tutto in un colpo"**: (1) chiudere il dubbio sul **Guardian** per primo; (2) ✅ **inventario firmato** sedia per sedia (*quale versione gira · quale dovrebbe girare · cosa cambia*) — 🆕 **FATTO il 14/09 per il PICCOLO 50503392, in sola lettura e senza nessuna ricompilazione**: `report/INVENTARIO_CAMPO_PICCOLO_2026-09-14.md` (40 sedie su 40, **5 allineate** / 20 arancioni / 15 rosse, commit in campo datato file per file). 🔴 **E ne esce un vincolo NUOVO sul gesto: per 13 sedie su 40 il bersaglio della ricompilazione NON e' HEAD**, perche' HEAD porta `b45dd00` (*"NON COMPILARE"*, ~2.000 righe mai passate dal cancello) e `b5d904a` (*"WIP build agente"* sul Nasdaq); (3) **una sedia per volta**, col **rapporto dei lotti prima/dopo** come prova che il fix ha morso | 🥇 `report/IL_CAMPO_E_FERMO_A_AGOSTO_2026-09-11.md` (42/42 diversi, date di compilazione **5-18/08**, Guardian **414** righe in campo contro **899** nel repo) · 🥇 `report/IL_GUARDIAN_IN_CAMPO_2026-09-12.md` §1-2 (le versioni per terminale e le manopole mancanti) · 🥇 `report/IL_GUARDIAN_CHE_SCHIEREREMO_2026-09-12.md` (giornale del piccolo **50503392**: *"GUARDIAN: nessuna riga"* l'11 **e** il 12/09) · 🥇 `wc -l mql5/Experts/ABTG_Guardian.mq5` = **899** (verificato in questo giro) · `report/PIANO_CHALLENGE_OTTOBRE_v2.md` r.301 (A5) e r.377 (firma 4) | ⚠️ **DUE PRECISAZIONI CHE CORREGGONO LA VERSIONE SEMPLIFICATA CHE CIRCOLA, e vanno dette perche' cambiano cosa si deve fare.** **(a)** *"manca tutto dal 09/08 in poi, comprese le firme B1/C1 del 18/08"* e' **piu' forte di cio' che e' misurato**: il `.mq5` accanto al binario e' **v1.10 del 18/08** (`a53820e`), mentre l'**`.ex5` e' del 09/08** → **cio' che GIRA e' piu' vecchio di cio' che si LEGGE**, e quale sia **`[NON MISURATO]`**. Le manopole che mancano alla v1.10 sono misurate e sono **quattro**: `InpMaxClusterRiskPct`, `InpClusterMappa`, `InpDailyBaseline`, `InpAutotest` — **B1 e C1 non sono in quell'elenco**. **(b)** 🔴 **E c'e' di peggio del numero di righe, sullo stesso conto**: sul **piccolo 50503392** il Guardian **non lascia nessuna riga di giornale** in due giorni consecutivi, e la prima sedia (`ABTG_EMA200`, `771531`) gira un binario **senza `InpUsaGuardian`**. 👉 **Li' il problema non e' "una versione vecchia": e' che non c'e' nessuna rete**, ed e' un fatto piu' grave e piu' economico da verificare della ricompilazione | 📋 **PROPOSTO — e la parte esecutiva e' 👤 di Claudio** (ricompilare **cambia i volumi** delle sedie vive: e' un gesto sul campo). 🔴 **Il PIANO DI OTTOBRE va corretto in due punti** (A5 e firma n.4): il **Δ +0 sedie** resta vero, ma *"+0"* qui **non misura il valore** — l'azione non aggiunge sedie, **dice se quelle vive sono protette**. Dettaglio del buco: **M43** |
| **K4** 🆕 | 🧪 **`ABTG_VolExpBreak` (magic `775301`) — CANDIDATO IN CODA DI VALIDAZIONE, NON UNA SEDIA** | **Motore nuovo, scritto il 13/09 dalla SPEC del par. 8 di `report/CACCIA_STOP_STRUTTURALE_2026-09-13.md`** (candidato C1, Pine di *cryptechcapital*, `tvdJe0bGvQ`, **nessuna licenza dichiarata [INCERTO]**). Donchian 20 letto dallo **shift 2** (niente ridipintura per costruzione), **ancora unica** (TP = entry ± R×`InpTP_RR` con R = distanza **vera** dello stop dopo il pavimento R109), `InpMaxSpreadPctOfStop` (R55), **Guardian incluso e chiamato prima di ogni invio**, parziale e breakeven **indipendenti**. Primi file prova: **`R145a`** (NASUSD M30) e **`R145b`** (U30USD M30), asse unico `InpKStop` 1,0/1,5/2,0/2,5 = **4 celle, 8 passate a file**, `@DAQUANDO 2024.09.26`, **`-Deposito 100000` OBBLIGATORIO** (a 10.000 la quantizzazione del lotto vale il **30%** dell'effetto su Nas e il **42%** su Dow) | 🥇 commit `6f92e7d` (13/09) · 🥇 `mql5/Experts/ABTG_VolExpBreak.mq5` r.255 (`InpMagic = 775301`, **1148 righe**, verificato) · 🥇 `backtest_pipeline/prove/R145a_volexp_kstop_M30_NASUSD.txt` r.378-380 e r.339-340 · 🥇 `R145b_volexp_kstop_M30_U30USD.txt` · 🥇 **classe 308** della checklist (l'asse su un parametro che sta anche al **denominatore** di un costo fabbrica da solo una pendenza) | ⚠️ **Nessun conflitto: non c'e' niente con cui confliggere.** Stato verificato in questo giro, non dedotto: **zero CSV in archivio**, **nessun `.ex5` nel repo** (mai compilato: qui non c'e' MetaEditor), **`CODA.txt` NON contiene R145** (40 righe di round, nessuna con questa etichetta). 🔴 Quindi **ogni numero di questo motore e' oggi `[NON MISURATO]`** — compreso il PF, il DD e la frequenza. E la licenza della fonte Pine e' **[INCERTO]**: l'attribuzione e' in testa al file, ma **non autorizza nessun uso commerciale finche' non e' chiarita** | 🧪 **CANDIDATO IN IMBUTO — NON e' una sedia, non entra in nessun conto di portata, non entra nel cap C1.** Il passo successivo e' **armare R145a/b in `CODA.txt`** (costo dichiarato: **2,44 minuti macchina in tutto**), che e' un gesto sul repo, **ma i round girano sul `50504400` e la corsia ROUND del runner e' materia di firma** |

---

# 📖 AREA L — LA NOTTE 12/13-09 LETTA: 49 COPPIE IS/OOS, UNA FIRMA PRONTA (nuova, v23 — 13/09/2026 sera)

> **Fonte**: `report/LETTURA_BACKLOG_NOTTE_2026-09-13.md` — i 98 CSV (49 coppie)
> caricati da Claudio dal VPS (`carica_risultati.ps1`, risolve **M44**) e letti
> contro i criteri congelati PRIMA dei numeri in ciascun file prova. Spot-check
> di due numeri (r137c, r127c) rifatto in questa sessione sui CSV grezzi:
> **combaciano al quinto decimale**.

**Il conto**: 49 lette · **7 PASS** · **26 FAIL** · **12 non giudicabili** (n<150
o screening OHLC) · 4 ancore d'archivio. 35 etichette sono girate stanotte, 14
erano gia' in cartella da round dell'08-09/09.

| # | cosa | esito | fonte | stato |
|---|---|---|---|---|
| **L1** 🆕 | ✍️ **`r137c` — FIRMA PRONTA, terza lettura indipendente della stessa cella.** `770101` DAX_Apertura_EU, `InpTP1_ClosePct` 50→0 | 🟢 **PF OOS 1,39709→1,49140, DD OOS 7,2328%→6,2719%, a PARITA' di 193 posizioni** (misurate per-trade, magic 786201), in ENTRAMBE le finestre (IS: PF 1,12634→1,18323, DD 5,4362%→4,9576%). Non e' selezione: e' gestione | 🥇 CSV grezzi `ABTG_DAX_Apertura_EU_D30EUR_{IS,OOS}_r137c.csv`, riletti in questa sessione — **8 numeri su 8 confermati al quinto decimale** · 🥇 `report/LETTURA_BACKLOG_NOTTE_2026-09-13.md` §2 | 📋 **PROPOSTO — manca solo la firma di Claudio.** Zero righe di codice, zero rischio nuovo: e' un parametro gia' nel preset |
| **L2** 🆕 | 🐤 **Il canarino della notte (`r127c`) riproduce**, quindi tutto il resto del batch e' leggibile | 🟢 n composito 395 contro ancora 394 (±0,25%, tolleranza ±2%), PF composito **1,39972** contro ancora 1,41 (tolleranza ±0,03) — verificato in questa sessione ricalcolando P/L dai due CSV `ohlc_r127c` | 🥇 `ABTG_CostToCost_EURJPY_{IS,OOS}_ohlc_r127c.csv` | ✅ **CHIUSO** |
| **L3** 🆕 | 🧮 **`cemad02` (EMA200): il codice 2 del runner era un falso allarme, e il certificato della sedia migliore si e' riempito di piu'** | 🟢 IS vuoto **per costruzione** (`@FRAZIONEIS 0.002`, dichiarato 3 volte nel file), OOS riproduce l'IS di R112 esatto. **IS 771531 = 132 POSIZIONI misurate** (237 deal, rapporto **1,7955**, non 2,0117 — quello e' l'OOS: la costante va corretta ovunque sia scritta). Requisito #2 del certificato chiuso per misura, non per stima | 🥇 CSV `ABTG_EMA200_U30USD_{IS,OOS}_cemad02.csv` (IS a 0 righe, verificato in questa sessione) · classe **312** nuova in checklist (il sentinella non conosce `@FRAZIONEIS`) | ✅ **CHIUSO**, con classe nuova aperta |
| **L4** 🆕 | ⚠️ **La famiglia SuperWave/SupRev NON riproduce l'archivio** (r132c nullo su 3 celle su 5, r126a "grado C", r120b metro fallito) — ma il determinismo interno e' intatto (tre corse indipendenti danno lo stesso numero al centesimo) | 🟡 **Indizio misurato, non isolato**: `r120e` (stesso deposito 100k confrontato con 10k) mostra che il **numero di operazioni dipende dal deposito** (131→184, +40%, a parita' di tutto il resto) su `ABTG_SuperWave_DOW_H1_Ottimizzato`. Coerente con quantizzazione/pavimento del lotto, ma **non isolato** | 🥇 `report/LETTURA_BACKLOG_NOTTE_2026-09-13.md` §5 | 🔴 **APERTO** — vedi buco **M45** |
| **L5** | ❌ **Tre bocciature col numero, per non dimenticarle**: `r138a` (F40EUR, DD OOS 11,82% > muro 10%) · `q770be` (breakeven non tocca la peggior giornata: −1,0793% su 4 celle su 4, +0,0000 pp) · `r139a/b/c` (EMA200/FiboH4 su AUDJPY/GBPUSD, DD 15-23% contro soglia 14%, r139b con segno che si INVERTE fra le 4 celle = regime, non edge) | ❌ **Non entrano**, e il perche' e' scritto coi numeri. ✏️ **Correzione su `r138a`** (da una lettura indipendente parallela, verificata sui CSV): il PF **cambia segno fra le finestre** (IS **1,44028** positivo, OOS **0,76965** negativo — verificato sui CSV grezzi in questo giro). Il verdetto giusto non e' *"merito negativo su campione leggibile"*: e' **"BOCCIATO PER RISCHIO (F3) — MERITO NON ANCORA MISURATO"** (il segno che si inverte e' REGIME, 3 caselle su 5 del certificato ancora mancanti). Resta fuori dal portafoglio lo stesso: il motivo del rifiuto era gia' giusto (DD), la diagnosi del merito no | 🥇 `report/LETTURA_BACKLOG_NOTTE_2026-09-13.md` tabella completa · 🥇 CSV grezzi `ABTG_DAX_Apertura_EU_F40EUR_{IS,OOS}_r138a.csv` (riletti in questo giro) · 🥇 commit `037af77` (sessione parallela "Weekend: analisi coda backtest", stessi 98 CSV, correzione indipendente) | ✅ chiuso (bocciature), corretto un dettaglio di framing |
...
| M31 | ✅🆕 **ESEGUITA (02/09, incorporata qui al v18 — il referto e' arrivato dopo la chiusura del v17.1): `report/M31_RISCHIO_REALIZZATO_2026-09-02.md`.** Solo scrivania, nessun MT5, nessun forward toccato. **I tre numeri**: DD forward della famiglia Aperture DAX **REALIZZATO 836,02 € = 16,39%** (il numero di M27, misurato a taglia doppia) → **NORMALIZZATO a contratto 375,60 € = 7,36%**, forchetta di sensibilita' **7,21-8,41%**. 👉 **Il 2,6× dell'emergenza NON esiste piu'; ma il numero onesto NON rientra del tutto: 7,36% contro 6,25% promesso = 1,18×.** La corsia passa da **allarme rosso** a **sforo marginale** — e **tutto lo sforo viene dalle modalita' BUY/SELL, che oggi non sono in campo**: la serie **RETEST-only** normalizzata ha DD **0,28%**. 🔴 **Cio' che NON cade**: la corsia **MERITO resta scattata anche normalizzata** (−277,64 € su 42 operazioni: la normalizzazione **dimezza** la perdita, non la annulla), e anche a taglia giusta **BUY −108,81 · SELL −200,02 · RETEST +53,59** → **non era (solo) la taglia**. 🟠 Rilievo nuovo: anche lo stop del **gemello `770102`** del 23/07 realizzava **2,009%** — la finestra a taglia doppia toccava **anche l'OTT**, cosa che la diagnosi del 31/08 non copriva. 📌 **Il referto NON prende nessuna decisione, per mandato**: mette le due revisioni sul tavolo **col numero pulito**, compresa la via di mezzo (aggiornare il **contratto** della 770101 alla cella realmente promossa — R83 RETEST, DD 10,60% a 1% — perche' il 6,25% di R16 descrive una cella che **non gira piu'**). _(testo originale della missione, conservato)_ 🔴🆕 **ERRATA 11/09: quella via di mezzo E' CADUTA** — il 10,60% di R83 ha `InpAllowShort=1` e magic 777120/777121, **non e' di questa sedia**; il contratto vero e' **6,7111% @1,0%** (**4,3501% @0,65%**, ⚠️ dep. 10.000 EUR), e contro quel numero il DD forward normalizzato **7,36% scatta lo stesso (1,10x)**, su tutta la forchetta 7,21-8,41% (1,07-1,25x). 📄 `report/CONFLITTO_DD_770101_2026-09-11.md` ⚖️ **LA CORSIA RISCHIO DELLA C3, RIFATTA A RISCHIO REALIZZATO** (nasce da M27 §B3, resta viva dopo la chiusura del caso 770101). Il DD forward della famiglia **Aperture DAX** — **16,39% contro 6,25% promesso, 2,6×** — e' stato calcolato **sul realizzato di una sedia che girava a taglia doppia** (stop pieni a −2,0% del conto contro l'1,0% dichiarato). 🔴 **Quindi il numero che ha fatto scattare la corsia RISCHIO e' gonfiato di un fattore noto, e prima di spegnere qualcosa va rifatto**: stessa serie, stessi trade, **stop riscalati alla taglia dichiarata**. Se dopo il riscalo il DD resta sopra il promesso, la corsia scatta **davvero** e le due revisioni proposte (modalita' BUY/SELL vs RETEST-only) tornano sul tavolo con un numero pulito; se rientra, **la famiglia e' stata giudicata da un difetto di configurazione, non dal suo edge** | chat principale / strumenti (rianalisi degli statement, **zero round, zero tester**) + **Claudio** per la decisione finale | "per la famiglia Aperture DAX: DD forward ricalcolato **a rischio realizzato riscalato all'1%**, contro il DD promesso di `CONTRATTI_SEDIE.md`. 🛑 **Nessuno spegnimento prima di questo numero**: la C3 dice _'DD forward oltre il promesso'_, e un DD misurato a taglia doppia non e' quel DD" |
| M32 🆕 | 🔎 **DIAGNOSI GBPUSD — i 10 minuti che sbloccano la sonda dell'orologio** (e decidono la questione cloud). La cella 03 costa **~1.600 s/passata contro i 54 s** della gemella EURUSD, con un primo tentativo ucciso da _"no memory for ticks generating"_. **L'EA e' escluso** (972 righe lette: nessun `CopyTicks`/`CopyRates`, `OnTick` O(1)) e **la configurazione e' identica** (verificata dal wrapper, non dichiarata): la differenza sta **nei DATI o nel BANCO**. Tre passi: **A** rileggere il censimento storico (`-SoloReferto`, ~1 min, non apre MT5) · **B** cronometro sulla finestra **2024.10→2026.06** (solo tick reali, ~4 min) · **C** cronometro sul tratto **2011→2013** (solo tick generati, ~5 min). Griglia di lettura dell'esito **gia' scritta** nel referto | **Claudio sul PC di backtest** (le stringhe le passa il verificatore) — **forward mai toccato**, i driver scrivono da soli `AllowLiveTrading=false`. ⏳🆕 **v18: il PASSO C (cronometro sul tratto 2011→2013, solo tick generati, ~5 min) e' in programma STASERA** — e' l'ultimo dei tre e quello che separa "dati vecchi" da "banco" | "GBPUSD e' lento per i **dati vecchi** (H1: generazione tick fuori scala), per **M1 mancanti** (H2: sync silenzioso dal server) o per il **banco** (H3: RAM)? 🔴 **E' bloccante per due cose insieme**: senza risposta la gamba GBPUSD della sonda non gira, **e la decisione sul cloud non si puo' prendere** — perche' se il problema e' nei dati, **il cloud non lo risolve: lo moltiplica**" |
| M33 🆕 | 🏹 **I PASSI 0 DEL VIVAIO FREQUENZA — l'ordine, e chi lo esegue** (dettaglio in **H11**/H11-bis). **(1) 🥇 ACCENDERE LA SONDA DELL'OROLOGIO**: costo di costruzione **ZERO** (7 file prova congelati dal 28/08, riga di lancio pronta, **mai girata**), pre-registrazione esterna **gia' depositata**; restituisce i 6 numeri coi cancelli congelati (C1 lordo/spread ora per ora · C2 la cella A pre-registrata · C3 altopiano non picco · C4 peggior giornata · 🆕 **lo spread BCM ora per ora, → H12** · F1 frequenza). **(2)** costruire la **sonda DayFlow** (~4-5 h riusando lo chassis `ABTG_SondaLondonFx`), coi cancelli gia' scritti: **< 2,00 segnali eseguibili/giorno = SCARTO IMMEDIATO** · **RR < 0,70 = scarto per aritmetica** · **≥25% sotto 60 s = SCARTO PROP (E9)** · e il **massimo segnali in UNA giornata**, perche' se `max × 0,65% > 3,25%` allora **`InpMaxTradesPerDay` entra nell'EA dal primo round**. **(3)** la sonda **LondonFx** (bozza congelata `prove/LONDONFX_FREQUENZA_BOZZA.txt`) | **Claudio** (corse sul PC di backtest) + **mql5-ea-developer** (sonda DayFlow) — l'ordine lo decide Claudio | "quale si accende per prima? La raccomandazione dell'architetto-prop, **dichiarata come tale**: **l'OROLOGIO**, perche' e' l'unica che costa zero, ha la macchina gia' sul banco, ha una **previsione esterna scritta prima** — e perche' **chiude comunque**, anche fallendo (§H11-bis). 🔴 Subordinata: la gamba **GBPUSD** e' bloccata da **M32**; le gambe EURUSD sono libere". 🆕 **v18 — L'ORDINE E' STATO DECISO DAI FATTI, NON DALLA RACCOMANDAZIONE**: la sonda **LondonFx** e' girata per prima (03/09 mattina) ed **e' passata su due simboli**, portando il candidato fino a un **round a tick FIRMATO (R116)**. L'**Orologio resta pronto e mai girato** (costo zero, pre-registrazione depositata): **non e' stato bocciato, e' stato scavalcato** — e la sua corsa chiuderebbe comunque **M36** (lo spread forex ora per ora), che oggi e' il buco piu' vicino al percorso critico. Le **due sonde nuove** (RELATIVO, VGRSI) entrano in coda **dietro** R116 |
| M34 🆕 | 🕶️ **LA CODA DEI FIX HEDGING — l'ordine c'e', manca il PERIMETRO** (riga **C9**). L'audit e la verifica storica hanno gia' fatto il lavoro difficile: la coda e' **ordinata per gravita' misurata** (ORB_Ott → MaxMinNotte → Gold_Ichimoku → i due Aperture 🟠, **+ `ABTG_ORB` nativo promosso** e **l'input morto `InpOneTradePerDay` come antipasto**), la **forma** del fix e' congelata (lettura **e** scrittura per ticket, insieme) e il **test di non-regressione e' dichiarato prima** (nel tester il risultato dev'essere **identico al centesimo**). 🔴 **Cio' che manca e' una decisione di perimetro, non tecnica**: il fix v1.04 dell'ORB **richiede una ricompilazione** e la firma **D1** dice _"fase 1 senza ricompilazioni"_ — e l'ORB e' **uno dei 5 mirror del 100k** | **Claudio** (la firma su perimetro e ordine) + **mql5-ea-developer** (il fix, uno alla volta, con autotest) | "si fixa **solo sul terminale del conto piccolo** (dove il difetto morde: 9-14 vicini per simbolo) lasciando intatti i binari del 100k fino a fine fase 1, oppure si aspetta la fine della fase 1 per tutti? 🛑 **La terza strada — ricompilare tutto adesso — costa il criterio 4 del collaudo** (rischio X13), e va scelta sapendolo" |
...
| M38 🆕 ✅ | 🐞 **R119 — IL RITARDO DI ESECUZIONE: ESEGUITO E MISURATO NELLA STESSA GIORNATA** (secondo giro `..._v3_EXECMODE`, 16:23:55 — G0 **MORDE**, **16/16 CSV**, gemelli **IDENTICI 8/8**, 0 rilievi). 🔴 **Esito: l'ipotesi congelata e' FALSIFICATA** — lo **STOP e' immune**, il **LIMIT** si muove **≤2% sul profitto / ≤3% sul DD** e **in MEGLIO**. ⏳ **Cio' che resta**: **(a)** il **verdetto formale contro i criteri S1-S4** (il referto macchina dichiara di non giudicare) e **(b)** capire **PERCHE'** un ritardo migliora i risultati — *un ritardo che migliora non e' un costo, e finche' non si sa cos'e' non si tara niente su questo numero*. _(testo originale della missione, conservato)_ 🐞 **R119 v3 — IL RITARDO DI ESECUZIONE, CON LA CHIAVE GIUSTA.** Il primo giro e' **NON MISURATO**: la chiave `Delay` **non esiste in MT5** e le chiavi `[Tester]` sconosciute vengono **ignorate in silenzio** (uscita 0, CSV prodotti, referto verde). Il round va rifatto con **`ExecutionMode`**, driver corretto e ri-collaudato, marcatore `..._v3_EXECMODE`. ✅ Il default (parametro non passato) resta **`ExecutionMode=0` byte per byte**: tutti i round gia' pinnati **riproducono identici** | **Claudio sul PC di backtest** (8 corse; il canarino G0 ha gia' risparmiato 6 corse a tick una volta) | "con un ritardo di esecuzione **vero**, `770101` (che entra a **BuyLimit**) degrada poco e `770611` (che entra a **BuyStop**) degrada molto, come dice l'ipotesi congelata? 🔴 **E' la differenza fra le due sedie che stanno sul conto REALE**: se la fragile e' quella che gira sui soldi veri, va saputo prima, non dopo" |
| M39 🆕 | 🧱 **LA DEFINIZIONE DEI CLUSTER + IL COLLAUDO** — i due passi che mancano al tetto 3,0% (**riga C10**), che oggi e' **firmato, implementato (v1.13, spento di default), NON compilato e NON collaudato**. Il valore e' deciso, il **codice c'e'**, ma **l'insieme dei cluster no** (`report/CLUSTER_PROPOSTA.md` e' **PROPOSTO, NON FIRMATO**) e il **collaudo non e' girato** (`backtest_pipeline/attese_cluster.txt`): quali simboli stanno nello stesso cluster (per valuta comune? per classe: indici / metalli / JPY-crosses? per correlazione **misurata** sui nostri tick?) **e' una scelta, e va firmata a parte** prima di scrivere una riga di codice | **Claudio** (la firma sull'insieme) + **architetto-prop** (la proposta di partizione, con la correlazione **misurata** e non assunta) + **mql5-ea-developer** (l'implementazione nel Guardian) — ⚙️ **lavoro gia' in corso su un altro agente: non va duplicato** | "su quale base si raggruppano i simboli, e la partizione si **misura** (correlazione dei rendimenti sui nostri tick) o si **dichiara** (classe di asset)? E **quando gira il collaudo**? 🔴 Finche' non ci sono tutte e due le risposte, il tetto per cluster **e' codice spento, non una protezione** — e va detto **ogni volta** che lo si cita" |
| M40 🆕 | 📏 **IL DD DI `771531` ALLA TAGLIA DELLA SEDIA CHE GIRA** — il buco lasciato aperto da **J2-ter**. Tutte e tre le misure di questa sedia girano a **deposito 10.000 o 100.000**; la sedia gira sul **piccolo 50503392 con ~5.000 EUR**. Per la classe **229** un difetto che dipende dalla taglia **non si trova misurando meglio: si trova misurando alla taglia giusta**. ➕ Nella stessa corsa si puo' chiudere anche l'altro approssimato: il **5,09% @0,65%** della sedia pianificata oggi e' **lineare per convenzione**, e la riconciliazione R29↔R112 dimostra che **quel metro sbaglia del 6%** | **PC di backtest** — stessa cella, stesso file prova, **due sole passate**: `-Deposito 5000` e `-Deposito 100000` con rischio **0,65** | "il DD di `EMA200` U30USD H1 **al saldo vero del conto su cui gira** e' ancora dentro il muro? E a 0,65% su 100k quanto fa **misurato**, invece che diviso per 1,538? 🔴 Finche' non c'e', il contratto della sedia descrive **una taglia che non esiste su nessun conto nostro**" |
| M42 🆕 | 🚨 **I 5 ZIP CHE CHIUDONO (O APRONO) IL RILIEVO DEI TERMINALI — riga K2.** Serve **una** informazione, e sta gia' scritta su disco: dentro `Desktop\ROUND_r127c.zip`, `ROUND_r127b.zip`, `ROUND_r139a.zip`, `ROUND_r139b.zip`, `ROUND_r139c.zip` c'e' il referto per-round con l'esito nella forma **`ROUND GIRATO CON RILIEVI (N)`** e, se e' il caso, la riga **`MANCANO ALL'APPELLO: PID ...`**. 🔴 **La domanda non e' piu' "quale PID e' sparito": e' "N vale 1 o 2?"** — perche' un rilievo e' gia' spiegato dal `-Modello 1` (r.408). 🟢 **Costo: leggere 5 file di testo. Nessun terminale toccato, nessuna scrittura.** ⚖️ **E il controllo del Giornale del conto reale `10105439` nelle finestre 04:21-05:18 e 07:47-08:08 (ora VPS) resta la SECONDA mossa, non la prima**: si fa **se** N=2, o se lo si vuole per tranquillita' — ma **non e' piu' l'urgenza che sembrava ieri** | 👤 **Claudio** (il VPS e' suo; per gli zip basta una riga di sola lettura, per il Giornale servono le sue mani su `C:\BCM_Reale`) | *"Apri i 5 zip sul Desktop del VPS e dimmi, per ognuno, la riga che comincia con `ESITO` — quanti rilievi dice fra parentesi? E se in uno compare `MANCANO ALL'APPELLO`, copiami quella riga per intero."* |
| M43 🆕 🟡 **META' FATTO (14/09)** | 🧊 **L'INVENTARIO FIRMATO CAMPO-vs-REPO, E L'ORDINE DEL RIALLINEAMENTO — riga K3.** 🆕 ✅ **LA PARTE IN SOLA LETTURA E' FATTA IL 14/09 PER IL PICCOLO 50503392** — `report/INVENTARIO_CAMPO_PICCOLO_2026-09-14.md`: **40 sedie su 40** con *versione in campo · righe · GUARD · data dell'`.ex5` · commit in campo · HEAD · scarto · commit mancanti*. **Nessun terminale toccato, nessuna ricompilazione, zero righe verso il VPS.** Numeri: 🟢 **5 allineate** (`ORB_Ott` 770611, `PostNews` x3, `TradeExporter`) · 🟠 **20** a cui manca solo il filo del Guardian e opt-in a default storico · 🔴 **15** a cui manca una riparazione che tocca i lotti. 🔴 **36 sedie su 40 girano un sorgente che non nomina nemmeno la guardia** (`GUARD=no`, cioe' ne' `InpUsaGuardian` ne' `GuardiaIngresso`), e le **4** che ce l'hanno sono comunque **fail-open** (`ABTG_PausaGuardian.mqh` r.765) perche' sul piccolo il Guardian **non gira**. 🟢 **E la sotto-domanda "economica" di questo buco E' CHIUSA: sul piccolo il Guardian NON e' attaccato a nessun grafico** (CODA_01 13/09 rr.15-54: 40 sedie, `ABTG_Guardian` non c'e'; sul 100k r.76 e sul reale r.101 invece si'), **ed e' una DECISIONE del 06/09** (`CENSIMENTO_CONTRATTI_v2.md` §4), non un guasto: il *"GUARDIAN: nessuna riga"* del giornale e' la **conseguenza**, non un mistero. 🚨 **SCOPERTA NUOVA, e cambia il gesto: per 13 sedie su 40 "quale codice DOVREBBE girare" NON e' HEAD** — HEAD e' `b45dd00` (11/09, *"NESSUNO DI QUESTI EA VA COMPILATO O CARICATO finche' non c'e' un PASS"*, ~2.000 righe) su 12 sedie + `b5d904a` (*"WIP FASE 2 DRIVE … build agente"*) su `Nasdaq_Apertura_US` 770250. 👉 **Un F7 "su tutto" metterebbe in campo codice mai passato dal cancello.** ⏳ **RESTA APERTO**: (a) 👤 la **ricompilazione** (cambia i volumi: firma di Claudio, una sedia per volta, col rapporto dei lotti prima/dopo); (b) 🤖 far passare dal cancello le ~2.000 righe di `b45dd00` cosi' HEAD torni un bersaglio legittimo; (c) 🔴 il **contenuto dei binari `.ex5`** resta `[NON MISURATO]` per costruzione. Oggi sappiamo che **42 sorgenti su 42** in campo sul piccolo **50503392** sono diversi dal repo (compilati **5-18/08**) e che il **Guardian** e' il piu' arretrato. **Quello che NON sappiamo e' il contenuto dei binari**: MT5 esegue l'`.ex5`, e sul Guardian l'`.ex5` (09/08) e' **piu' vecchio del `.mq5` accanto** (18/08). 🔴 **Finche' questo inventario non esiste, la frase *"le protezioni firmate il 18/08 sono attive"* e' `[NON MISURATO]` per ogni sedia viva del piccolo** — e va detta cosi' ogni volta che si cita B1 o C1. 🟢 **La prima domanda e' la piu' economica e non richiede nessuna ricompilazione**: perche' il giornale del piccolo dice *"GUARDIAN: nessuna riga"* in due giorni consecutivi? | 👤 **Claudio** per i gesti sul campo (F7, ricarico, un terminale per volta) · 🤖 **agenti** per l'inventario in sola lettura (`CODA_06` lo stampa gia') | *"(1) Sul piccolo 50503392 il Guardian e' davvero attaccato a un grafico? (2) Per ogni sedia viva: quale `.ex5` gira (data), quale commit dovrebbe girare, e cosa cambia fra i due? (3) Si parte dal Guardian, una sedia per volta, col rapporto dei lotti prima/dopo come prova."* 🔴 **MAI su `C:\BCM_Reale` (10105439) senza una firma dedicata** |
| M44 | ~~I CSV DEI 34 ROUND DELLA NOTTE 12/13-09~~ → ✅ **RISOLTO (13/09 sera)**: Claudio ha lanciato `carica_risultati.ps1`, 98 CSV (49 coppie) arrivati in `risultati_prove/dal_vps/`, letti tutti — vedi **AREA L** | — | — |
| M45 🆕 | 🔬 **ISOLARE LA DIPENDENZA DAL DEPOSITO su `ABTG_SuperWave_DOW_H1_Ottimizzato`** — riga **L4**. Misurato: `n` OOS passa da 131 (dep. 10.000) a 184 (dep. 100.000) a parita' di tutto il resto, **+40%**. E' la spiegazione piu' probabile del perche' la famiglia SuperWave/SupRev non riproduce l'archivio di agosto (`r132c`/`r126a`/`r120b`), ma resta **[INFERITO]**, non isolata | **PC di backtest** — stessa cella, due depositi puntati esattamente sulla quantizzazione/pavimento del lotto (es. `-Deposito 10000` vs `-Deposito 100000` vs un deposito intermedio), con i contatori di lotto-alzato/tagliato (classe 228) esportati | *"A parita' di segnali, quanti trade in piu' escono a 100k rispetto a 10k, e sono TUTTI spiegati dalla quantizzazione del lotto o resta un residuo? Finche' non e' isolato, nessun confronto d'archivio su questa EA a depositi diversi e' valido — e r132c/r126a/r120b restano fermi"* |
| M41 🆕 | 🐞 **`CODA_08` ACCORPA GLI INPUT DEGLI INDICATORI A QUELLI DELL'EA** — trovato scrivendo il preset di `770202` (**J1**). Il blocco di ogni sedia stampa **anche** i ~32 input dell'indicatore `ABTG_Look` appeso al grafico: chi copia un blocco del log dentro un `.set` ci mette **32 righe che l'EA non ha**. La prova e' nel log stesso: `ABTG_TradeExporter` ha **4** input nel sorgente e nel log ne mostra **36**, e `InpVerbose` compare **due volte** (cosa che in MQL5 non compila) | **chi mantiene la coda del runner** (`backtest_pipeline/coda/`) — sola lettura, nessun gesto sul campo | "separare, dentro il `.chr`, il blocco `<expert>` dai blocchi degli indicatori, e stampare due elenchi distinti. 🟢 **Fino ad allora la regola di casa e': il preset si confronta sempre con l'elenco `input` del sorgente dell'EA prima di essere scritto** — e' il controllo che ha preso questo difetto" |

## ✍️ LE FIRME CHE SERVONO A CLAUDIO (in ordine di urgenza)

🆕 **v18 (03/09) — LA DECISIONE PIU' URGENTE DI OGGI NON E' UN NUMERO: E' UN
PERIMETRO.**

> 🕶️ **DECISIONE "FIX HEDGING" (C9 / M34) — dove e quando si mette mano.**
> _(aggiornata alle 08:25: **il fix v1.04 dell'ORB e' gia' SCRITTO nel repo** —
> non compilato, non in forward. Quindi non si firma "se fare il fix": si firma
> **se e dove ricompilare**.)_
> L'audit ha censito **34 EA su 126** col difetto, la verifica storica ha
> misurato **il primo danno in forward** (4 giornate su 16, +25% di frequenza
> contro il pannello) e la **coda e' gia' ordinata**. Quello che manca e' una
> riga di Claudio su **DOVE** si fixa, perche' c'e' un **conflitto con la firma
> D1 di ieri**: il fix dell'ORB **richiede una ricompilazione**, e l'ORB
> `770611` e' **uno dei 5 mirror del 100k in collaudo**. Le tre strade, col loro
> costo dichiarato:
> 1. 🟢 **Fix SOLO sul terminale del conto PICCOLO, 100k intatto fino a fine
>    fase 1** — _raccomandazione dell'architetto-prop, dichiarata come tale_:
>    il difetto **morde dove ci sono i vicini** (piccolo: 9-14 sedie per
>    simbolo), e sul 100k l'ORB e' quasi sempre solo — **infatti traila**.
>    Costo: zero sul collaudo. Vincolo: va verificato sul VPS che i due
>    terminali abbiano `.ex5` separati.
> 2. 🟡 **Aspettare la fine della fase 1 e fixare tutto insieme** — costo: il
>    difetto continua a mordere per una settimana buona (e i numeri di forward
>    delle sedie 🔴 restano sporchi).
> 3. 🔴 **Ricompilare adesso su tutti e due i terminali** — costo: **si brucia
>    il criterio 4** del collaudo (rischio **X13**) e la fase 2 si allontana.
>
> ✅ **E l'antipasto che non ha conflitti con niente**: il fix di
> **`InpOneTradePerDay` dichiarato e mai letto** (`ABTG_ORB.mq5:54`,
> `ABTG_MaxMinNotte.mq5:65` — **il pannello mente**). Due righe, **non tocca la
> selezione delle posizioni**, quindi non puo' rompere nulla: e' esattamente il
> tipo di intervento con cui la regola di casa dice di cominciare.

📤 **E una richiesta operativa che costa una riga di codice (M35)**: il magic
del deal di **uscita** e le posizioni **aperte** nell'export. Senza, la
verifica del difetto non potra' mai essere conclusiva — ne' prima del fix
(per misurare), ne' dopo (per dimostrare che ha funzionato).

⚖️ **E la revisione parcheggiata torna sul tavolo COL NUMERO PULITO (M31,
eseguita)**: il **16,39%** che aveva fatto scattare l'emergenza sulla famiglia
Aperture DAX, normalizzato a contratto, e' **7,36% contro 6,25% promesso =
1,18×**. **L'emergenza cade, lo sforo resta** — e viene **tutto** dalle
modalita' **BUY/SELL, che oggi non sono piu' in campo** (la serie RETEST-only
ha DD **0,28%**). 🔴 Ma la corsia **MERITO resta scattata anche normalizzata**
(−277,64 € su 42 operazioni), e anche a taglia giusta **BUY e SELL perdono**:
_non era (solo) la taglia_. Le tre opzioni sul tavolo, senza raccomandazione
d'ufficio: **(a)** formalizzare il **RETEST-only** (che e' gia' cio' che gira
in campo dal 21/08) · **(b)** spegnere la famiglia — ma la "sedia colpevole"
per la lettera della C3 **e' una configurazione che non esiste piu'** ·
**(c)** aggiornare il **contratto** alla cella realmente promossa (R83 RETEST,
DD 10,60% a 1%), perche' il 6,25% di R16 descrive una cella che non gira piu'.
**La parola resta a Claudio.**

> ### 🔴🆕 ERRATA 11/09 — **L'OPZIONE (c) E' MORTA, E CON LEI UN'AMNISTIA**
> Il **10,60% di R83** su cui poggiava la via di mezzo **non e' di questa sedia**:
> quella corsa ha **`InpAllowShort=1`**, `InpRiskPercent=1` e magic
> **777120/777121** (📄 `report/CONFLITTO_DD_770101_2026-09-11.md`). Il contratto
> vero della cella viva e' **6,7111% a 1,0%** (e **4,3501% a 0,65%**, ⚠️
> **[MISURATO a deposito 10.000 EUR]**).
> 🧮 **Rifatto il confronto della corsia RISCHIO**, a parita' di taglia 1,0%:
>
> | DD forward normalizzato (M31) | vs 6,25% (R16) | vs **10,60% (opzione c)** | vs 🆕 **6,7111% (contratto VERO)** |
> |---|---|---|---|
> | **7,36%** (centrale) | 1,18x 🔴 scatta | **0,69x 🟢 dentro** | **1,10x 🔴 SCATTA** |
> | 7,21% (basso forchetta) | 1,15x 🔴 | 0,68x 🟢 | **1,07x 🔴 SCATTA** |
> | 8,41% (alto forchetta) | 1,35x 🔴 | 0,79x 🟢 | **1,25x 🔴 SCATTA** |
>
> 🔴 **Il verdetto CAMBIA: l'opzione (c) sembrava far rientrare la corsia
> RISCHIO su TUTTA la forchetta, e invece NON la fa rientrare su NESSUN punto
> della forchetta.** Era un'amnistia costruita su un numero di un'altra
> configurazione: aggiornare il contratto **non assolve la famiglia**, la lascia
> esattamente dov'era — **sforo marginale, 1,07-1,25x**.
> 🟢 **E l'opzione (a) ne esce rafforzata**: la serie **RETEST-only**
> normalizzata ha DD **0,28%**, contro un contratto di **6,7111%** — dentro con
> un fattore 24x. Cioe' **tutto lo sforo continua a venire dalle modalita'
> BUY/SELL, che non sono in campo**, e adesso lo si puo' dire **contro il
> contratto giusto** invece che contro uno preso in prestito.
> ⚪ **Cio' che NON cambia:** la corsia **MERITO** resta scattata (−277,64 EUR su
> 42 operazioni) — non dipende dal DD promesso. E **la parola resta a Claudio.**
...
1. 🔔 **MIGRAZIONE EA all'include del Guardian (enforcement di C1/B1) — la
   ...
   **48 EA vivi, 74 punti di ingresso** collegati alle bandiere; include
   ...
   decisione B1/C1 e' identico byte a byte** fra la v1.20 compilata e HEAD
   → **D1: niente ricompilazioni**, si collauda il software che sta in
   ...
   🔄 **Ri-verificato il 02/09 sera dopo il cantiere P0** (include a **`v1.51`**,
   commit `e72546e`): **6 nuclei su 6 ancora identici** fra `d0241ff` (v1.20) e
   HEAD (v1.51). 👉 **La ragione della D1 non e' decaduta con la versione: il
   P0 ha aggiunto codice senza toccare il nucleo che decide.**
...
🆕 **v22 (13/09): 4 righe nuove (AREA K) e 3 buchi nuovi (M42/M43/M44) — e il
giro NON ha aggiunto nessun valore nuovo, ha corretto tre STATI.**
- 🟢 **K1**: da *"mai girati"* a **"girati, CSV in attesa di caricamento"** —
  **34 round con dati veri** sul VPS, di cui il piu' costoso ha gia' bruciato
  **4.693 secondi** di macchina. Buttarli sarebbe buttare una notte.
- 🚨 **K2**: l'allarme *"un terminale non bersaglio e' sparito"* passa da
  **urgenza** a **rilievo aperto a bassa priorita'** — non perche' qualcuno si
  sia tranquillizzato, ma perche' l'incrocio `-Modello` × codice d'uscita fa
  **5/5** e **0/31**. 🔴 Resta aperto: la mia spiegazione **non esclude** l'altra.
- 🧊 **K3**: la sola riga che **peggiora** in questo giro, ed e' quella che pesa
  di piu' sul 1° ottobre — *"le protezioni firmate sono attive"* e' **[NON
  MISURATO]** su ogni sedia viva del piccolo **50503392**.
- 🧪 **K4**: un candidato in piu' nell'imbuto, **zero sedie in piu' in campo**.

🆕 **v20 (07/09): 61 parametri — 18 congelati · 21 proposti · 18 aperti · 2
chiusi per misura · 1 NON MISURATO · 1 parziale.** E' il giro in cui il piano ha
smesso di chiedersi *quanto rischiamo* e ha cominciato a chiedersi **quanto sono
veri i numeri con cui misuriamo il rischio** — e in cui **una delle due domande
ha gia' avuto risposta, in giornata**.

- 🧊 **DUE FIRME NUOVE** (`report/FIRME_2026-09-07.md`): **H13** (pavimento di
  frequenza **per FAMIGLIA**, 1,00 op/giorno — l'unita' cambia, la soglia no) e
  **C10** (tetto per **CLUSTER 3,0%**) — 🔴 e le due **vanno lette insieme**:
  quella che **allarga** e' a costo zero e vale da subito, quella che
  **protegge** e' **codice ancora da scrivere**;
- 🆕 **AREA I nuova — FEDELTA' DEL BANCO**: **I1** slippaggio vero (**0 deal**,
  ❔ NON MISURATO), **I2** `ExecutionMode` (✅ **MISURATO in giornata**), **I3**
  determinismo (🟡 parziale), **I4** spread (indici misurati, forex no);
- ✅ **R119 RIFATTO E MISURATO NELLA STESSA GIORNATA** (`..._v3_EXECMODE`, G0
  **MORDE**, 16/16 CSV, gemelli identici 8/8, 0 rilievi) — e 🔴 **l'ipotesi
  congelata e' FALSIFICATA su tutte e due le meta'**: lo **STOP (`770611`) e'
  immune** (OOS identico alla cifra fino a 500 ms), il **LIMIT (`770101`) e'
  quello che si muove**, **≤2% sul profitto e ≤3% sul DD**, e **in MEGLIO**.
  🟢 Regalo grosso: le due sedie del **conto REALE** hanno ora un **DD promesso
  misurato ALLA TAGLIA VIVA** (0,65%, tick, stessa finestra) — e la **`770101` ha
  il merito PIENO** (n **175** IS e **270** OOS): **3,17% IS / 4,35% OOS**;
- 🆕 **C11 nuova** (collisione P0-7: due sedie di lato **opposto** sullo stesso
  simbolo e minuto — **nessun controllo di casa la vede**);
- 🆕 **sezione v20 in testa**: dove siamo · la distanza dalla prop
  quantificata · i blocchi in ordine di costo · **la coda di lavoro per Claudio**;
- 🥇 **forward ricontato oggi**: piccolo **25/08→04/09** = 66 chiusure / 65
  ingressi / 27 magic / **−125,28 €** e **7,22 ingressi per giornata operativa**
  (contro 4,85 della finestra 03-28/08: **resa dal 63% all'89%** del promesso);
  100k **10/08→04/09** = 27 chiusure / 16 giornate / **+2.854,99 = +2,85%**;
- 📏 **la consistency, misurata con la FORMULA DICHIARATA** (e la vecchia riga
  "43,6%" di H9 **e' superata**): best day / **giorni positivi** = **37,27%**
  grezzo e **29,97%** normalizzato; best day / **totale** = 58,86% e 59,48%.
  ✅ Sul **2-Step non esistono consistency rule dure** (accertato): sulla prop di
  riferimento **non ci tocca**;
- ✅ **IL CENSIMENTO DEI CONTRATTI E' CONSEGNATO** (`report/CENSIMENTO_CONTRATTI.md`,
  da un altro agente): **47 righe-sedia / 41 sedie uniche**, **40 MISURATE**,
  merito **SOSPESO su 28** → **cancello 2** si muove per la prima volta da tre
  settimane, e la corsia **RISCHIO** della C3 diventa applicabile **su 40 sedie
  su 47**. 🔴 Resta rosso per **4 buchi dichiarati** e per la **peggior giornata**;
- ⚙️ **il tetto per cluster e' stato IMPLEMENTATO nella stessa giornata**
  (Guardian **v1.13** + include **v1.60**, input **spenti di default**,
  `if(StringLen(cluster_mappa)>0)` → **no-op verificato**): lo stato passa da
  _"firmato ma non attivo"_ a _"implementato ma **non compilato e non
  collaudato**"_ — **che vuol ancora dire NON ATTIVO**;
- 🆕 **M37 · M38 · M39** in COSA MANCA (deal del logger · R119 v3 · definizione
  dei cluster **e collaudo**);
- 📄 **DD promesso agli atti per `770611`**: **5,65% IS / 6,54% OOS** (R119
  §5) — merito **SOSPESO** (71/119 < 150), sono numeri **descrittivi**.

**Cosa NON e' cambiato**: nessun valore firmato toccato, **nessun candidato
promosso** (tre round, **tre verdetti negativi**), **zero modifiche al forward da
questo documento**, nessuna sedia accesa o spenta, nessun acquisto autorizzato.

🔴 **I conflitti e i limiti nuovi, dichiarati:**
**(1)** 🔴 **firmato ≠ attivo**: il tetto per cluster e' **firmato oggi e non
esiste nel Guardian** — va detto **ogni volta** che si cita, e la firma gemella
(famiglia = piu' simboli) rende il buco **piu' probabile**, non meno;
**(2)** 🔴 **l'ipotesi congelata sul ritardo e' FALSIFICATA, e nel verso
opposto**: davamo per fragile lo **STOP** ed e' **immune**; davamo per solido il
**LIMIT** ed e' l'unico che si muove — **in MEGLIO**. ⚠️ **Un ritardo che migliora
non e' un costo**: il meccanismo **non e' spiegato** e finche' non lo e' **non si
...
**(4)** ⚠️ **il determinismo non e' della macchina, e' del ROUND**: due round dello
stesso giorno, stessa macchina — uno l'ha verificato (R119), l'altro ha dovuto
dichiararlo **non verificato** (orologio indici);
**(5)** ⚠️ **campione sottile ovunque**: 16 giornate sul 100k, 9 sul piccolo,
**0 operazioni sul reale**. Le proiezioni di tempo sono **lineari e dichiarate
tali**: descrivono il passato, non promettono il futuro;
**(6)** 🟡 **meta' round sull'orologio indici**: le due celle del **Dow** non
sono girate e il criterio I1 e' **d'insieme** → su U30USD **non si conclude
niente**, ne' in un verso ne' nell'altro.

_(conteggio precedente, v18)_ **v18 (03/09): 54 parametri — 16 congelati · 21 proposti · 16 aperti · 1
chiuso per misura.** **Una riga nuova sola**, perche' questo giro non ha
prodotto parametri: ha prodotto **misure su parametri che c'erano gia'**.
- 📋 **C9 nuova** (il difetto `PositionSelect` su conto hedging: **34 EA su
  126**, coda di fix ordinata; il **primo fix, la v1.04 dell'ORB, e' scritto
  nel repo alle 08:25 — non compilato, non in forward**) — e' l'unica riga
  aggiunta;
- 🔎 **A2 aggiornata**: la sua **prima violazione misurata in campo** (ORB
  `770611` sul 100k a **3,3× il contratto** per nove giorni) **e gia' riparata**
  da Claudio il 03/09 alle 08:10, con foto prima e dopo;
- 📏 **H12 mezza chiusa**: lo spread **degli indici e' MISURATO** (252 M tick,
  solo-bid **0,000%**, mediane in sessione **1,6-2,0**, **DAX notte 3,5-3,9**) —
  la riga **resta APERTA** perche' il **forex**, dove lavora tutto il vivaio,
  non e' misurato (→ **M36**);
- 🏹 **H11 aggiornata**: **V8 morto per misura**, **LondonFx primo superstite
  del passo 0 con round R116 FIRMATO**, **RELATIVO 8/10** e **VGRSI 6/10** in
  coda, DayFlow scavalcato;
- 🚚 **H10 aggiornata**: **fase 4 verde** (6/6 sedie mute vive), **sessione 1
  del collaudo imminente**, e l'**ostacolo nuovo** del conflitto con D1;
- 💸 **C8 aggiornata** col prezzo del P0 non ancora acceso: **−100,33 €** il
  02/09 sull'oro (due posizioni, **uno stop solo**);
- ✅ **M31 incorporata** (era stata eseguita il 02/09 dopo la chiusura del
  v17.1): **16,39% → 7,36% normalizzato**, l'emergenza cade, lo sforo resta;
- 🕳️ **M34 · M35 · M36 nuove** in COSA MANCA.

**Cosa NON e' cambiato**: nessun valore firmato toccato, **nessun candidato
promosso**, **zero modifiche al forward da questo documento** (l'unica modifica
in campo della giornata — la taglia dell'ORB — l'ha fatta **Claudio**, su un
contratto **gia' firmato**, con la legge dello screenshot).

🔴 **I conflitti e i limiti nuovi, dichiarati:**
**(1)** 🔴 **il fix hedging contro la firma D1** (_"fase 1 senza
ricompilazioni"_): l'ORB e' uno dei 5 mirror del 100k in collaudo → **C9/M34**;
**(2)** ⚠️ **correzione di premessa fra due referti dello stesso rango**:
l'audit dice _"posizione piu' VECCHIA"_, la verifica **misura** che e' quella
col **`position_id` piu' basso** — diverso nel **16,5% dei casi** (28 coppie su
170), perche' un ordine **pendente** eredita il ticket di quando e' stato
*piazzato*. **Vince la misura, ed e' la formulazione da usare da qui in poi**;
**(3)** ⚖️ **il fatto scomodo**: le 4 operazioni che l'ORB nativo **non doveva
fare** hanno **guadagnato +201,15 €**. E' **fortuna su quattro casi**, e non e'
un argomento per tenersi un difetto di rischio;
**(4)** 📏 **la convenzione `spread 2.0` era giusta in sessione e OTTIMISTA
fuori**: le sedie **notturne** sugli indici (MaxMinNotte DAX) portano un costo
strutturale mai contato (**3,5-3,9** contro 2,0);
**(5)** 🧾 **campione sottile sui 🟠**: **zero** chiusure incrociate trovate, ma
anche **zero occasioni** in cui il difetto fosse armato (n=3) — **l'assenza di
prova non e' prova di assenza**: sospende il giudizio sul fatto che il danno
**sia avvenuto**, non su quello che **sia possibile** (valvola R59);
**(6)** 🕵️ **i numeri di forward delle sedie 🔴 sono sporchi** (sui giorni con
vicini non hanno mai eseguito la gestione): chi le giudichera' deve saperlo.

_(conteggio precedente, v17)_ **v17 (02/09): 53 parametri — 16 congelati · 20 proposti · 16 aperti · 1
...
|---|---|---|---|
| 13/09/2026 sera | **v23** | 📖 **I 98 CSV ARRIVATI, LETTI, E UNA FIRMA PRONTA** (nuova **AREA L**, righe L1-L5; **M44 risolto**, nuovo buco **M45**). Claudio ha lanciato `carica_risultati.ps1`: **98 CSV (49 coppie IS/OOS)** dentro `risultati_prove/dal_vps/`, tutti letti contro i criteri congelati. **Conto: 7 PASS · 26 FAIL · 12 non giudicabili · 4 ancore d'archivio.** 🥇 **(1) `r137c` e' una FIRMA PRONTA**: `770101` `InpTP1_ClosePct` 50→0 alza il PF OOS **1,39709→1,49140** e abbassa il DD OOS **7,2328%→6,2719%**, a **parita' di 193 posizioni**, in entrambe le finestre — terza lettura indipendente della stessa cella, verificata in questa sessione sui CSV grezzi al quinto decimale. **Zero righe di codice, manca solo la firma.** 🐤 **(2) Il canarino della notte (`r127c`) riproduce** (n composito 395 contro 394, PF composito 1,39972 contro 1,41): tutto il resto del batch e' leggibile. 🧮 **(3) `cemad02` era un falso allarme**: il codice 2 del runner (CSV vuoto = non misurato) non conosce `@FRAZIONEIS`, e l'IS vuoto era voluto — **classe 312 nuova**. Chiuso il requisito #2 del certificato di `771531`: **IS = 132 posizioni** (non 117,8 come dava il fattore sbagliato 2,0117 — quello e' l'OOS, il vero fattore IS e' 1,7955). ⚠️ **(4) La famiglia SuperWave/SupRev resta ferma**: non riproduce l'archivio (r132c nullo, r126a "grado C"), ma il determinismo interno regge e `r120e` ha misurato un indizio forte (il numero di operazioni dipende dal deposito, +40% da 10k a 100k) — **non ancora isolato, M45**. ❌ **(5) Tre bocciature col numero**: `r138a` (DD 11,82%>10%), `q770be` (breakeven non tocca la peggior giornata, +0,0000 pp), `r139a/b/c` (DD 15-23%>14%, r139b col segno che si inverte fra le 4 celle). | La sessione notturna (Claudio dormiva) doveva trasformare "98 file su GitHub" in "verdetti leggibili". Zero round eseguiti, zero EA toccati, zero preset cambiati — solo lettura, e una firma pronta per quando Claudio si sveglia. |
| 13/09/2026 | **v22** | 🌙 **IL GIRO DELLA NOTTE CHE HA PRODOTTO DATI E DELL'ALLARME CHE SI E' SGONFIATO** (nuova **AREA K**, righe K1/K2/K3/K4; nuovi buchi **M42**, **M43**, **M44**). 🟢 **(1) Il tappo si e' mosso davvero**: il referto `REFERTO_RUNNER_20260913_033003.txt` porta **42 righe eseguite, 36 in corsia ROUND**, e ricontate una per una fanno **34 round con CSV validi** (29 a uscita 0 + **5 a uscita 3, che hanno CSV per costruzione**), **1 giro a vuoto** (`canfrz`, `-SoloControllo`: MT5 **non aperto**) e **1 sola misura mancata** (`cemad02`). 🔴 **Ma i CSV non sono nel repo** (classe 307) → lo stato giusto non e' "bocciato" ne' "letto": e' **"girato, in attesa di caricamento"** (**M44**). 🚨 **(2) Il rilievo sui terminali e' RIDIMENSIONATO DA UNA MISURA**: l'uscita 3 non significa *"esclusivamente"* un PID sparito — r.745 e' `$persi -or $RILIEVI`, e i punti che generano rilievi sono **tre**. **I 5 round a uscita 3 sono ESATTAMENTE i 5 lanciati a `-Modello 1`** (correlazione **5/5 e 0/31**), e r.408 genera un rilievo **per costruzione** su ogni round OHLC. 👉 La causa parsimoniosa e' il modello, non un terminale caduto; l'ipotesi alternativa chiede **cinque** sparizioni-e-rientri, ognuna **esattamente** durante un round OHLC, con `r126a` a uscita **0** in mezzo. ⚖️ **Il rilievo resta APERTO** (i due rilievi possono essere scattati insieme e il log per-round e' sovrascritto), ma la via di chiusura e' cambiata: **non "controlla il conto reale", ma "leggi `N` in `ROUND GIRATO CON RILIEVI (N)`" nei 5 zip** (**M42**). 🧊 **(3) `Compilazione R4` e' SOTTOSTIMATA nel piano di ottobre**: li' vale *"15 sorgenti, F7 + ricarico, Δ +0"*, ma i sorgenti disallineati sono **42 su 42** e il piu' arretrato e' la **protezione**. 🔴 Con due precisazioni **misurate** che la versione semplificata sbaglia: l'`.ex5` del Guardian (09/08) e' **piu' vecchio del `.mq5` accanto** (18/08) → **cio' che gira e' `[NON MISURATO]`**; e le manopole che mancano alla v1.10 sono `InpMaxClusterRiskPct`, `InpClusterMappa`, `InpDailyBaseline`, `InpAutotest` — **B1 e C1 non sono in quell'elenco**. 🔴 E il fatto piu' grave non e' il conteggio righe: sul piccolo **50503392** il Guardian **non scrive nessuna riga di giornale** per due giorni (**M43**). 🧪 **(4) Candidato nuovo in imbuto**: `ABTG_VolExpBreak` (`775301`, 1148 righe, **mai compilato**) con `R145a`/`R145b` — **verificato: non e' in `CODA.txt`**, zero CSV. **Non e' una sedia e non entra in nessun conto.** | Il piano descriveva uno stato del mondo di **due giorni fa**: round "mai girati" che erano girati, e un allarme sul conto reale che il codice **non sostiene**. 🔴 **E la correzione piu' scomoda e' la mia, sulla fonte di ieri**: la parola *"esclusivamente"* nella classe 309 e' falsificata leggendo `RIGA_ROUND_VPS.ps1` r.745 — la gerarchia dice che **vince il codice**, anche contro un nostro referto |
| 11/09/2026 | **v21** | 🪑 **IL GIRO DELLE DUE SEDIE SBLOCCATE SENZA CHIEDERE NIENTE A CLAUDIO** (nuova **AREA J**, righe J1/J1-bis/J1-ter/J2/J2-bis/J2-ter/J3; nuovi buchi **M40** e **M41**). 📁 **(1) `770202` ha il suo preset su file**: `mql5/Presets/ABTG_Dow_Apertura_US_U30USD_M5_770202_100K.set`, **81 righe copiate** dal `.chr` vivo del 100k (`CODA_08` r.3267-3386) — `diff` fra il blocco del log e il preset **vuoto**, nessun valore scelto da me. Lo 0,65% e il `InpAllowShort=false` **non vivono piu' solo dentro un `.chr`**. 🧬 **E R1 si e' chiusa per strada, non citata ma VERIFICATA**: dei **nove** campi del vivo che non sono il default compilato, **otto sono fissati da `R16c_pertrade_Dow.txt` allo stesso identico valore**; il nono e' la taglia (banco 1,0% / campo 0,65%). 🧊 **Contro-esempio sul "campo fermo ad agosto"**: quel referto guarda il **piccolo**; sul `-V3` questa sedia e' **2148 righe contro 2147 del repo, compilata lo stesso giorno dell'ultimo commit** — allineata, non ferma. 💥 **(2) Il conflitto sul DD di `771531` e' SCIOLTO, e non con una sintesi**: le tre misure (**6,48%** R103 · **7,21%** R29 · **7,8323%** R31/R110/R112) descrivono **tre configurazioni diverse** — OHLC vs tick, deposito **10.000** vs **100.000**, finestra **piena** (senza OOS, classe 224) vs **OOS 40/60**. La prova che il deposito e' *parte della configurazione* e' che **cambia l'insieme dei trade** (`n` **444** a 10k contro **517** a 100k, stesso giorno e stessa cella). 🔴 **Ma il contro-esempio l'ho costruito e in parte MORDE**: riportate allo stesso rischio effettivo col metro lineare di casa, R29 darebbe **~8,29%** contro 7,8323% → **i tre numeri stanno in piedi come descrizioni di tre sedie, NON come tre stime della stessa sedia**, e il metro lineare DD~rischio qui sbaglia del **6%**. 🕳️ **E il buco vero resta, dichiarato (J2-ter → M40): nessuna delle tre misure descrive la sedia che gira** — `771531` sta sul **piccolo 50503392 a 1,0% con ~5.000 EUR**, e i banchi girano a 10k e 100k. ⚠️ **Corretta una premessa che circolava**: l'**1,62% contro l'1,00%** della classe 229 **non e' di questa sedia** ma di `971501` su **XAUUSD**; su U30USD il pavimento **non morde** (gambe reali a 0,20 e 0,30 lotti, 2-3x il minimo, `DIARIO` 04/09). 🔴 La classe 229 morde pero' **altrove**: il binario in campo sul piccolo e' del **06/08** e **non contiene il fix di sizing dell'08/08** (`3af47ed`) con cui e' stato misurato il 7,8323%. 🐞 **(3) Difetto nuovo dello strumento (M41)**: `CODA_08` accorpa al blocco `<expert>` gli input degli **indicatori** appesi al grafico (113 parametri stampati, **81 sono dell'EA**). Chi copia un blocco del log in un `.set` ci mette 32 righe false. | Il piano di ottobre aveva misurato che le azioni da **+1 sedia** erano tre e che **due non richiedevano la firma di Claudio**: erano queste. Nessun terminale, nessun `.chr`, nessun parametro vivo e' stato toccato. |
| 07/09/2026 sera | **v20** | 🧭 **IL GIRO IN CUI IL PIANO HA MESSO IN CIMA LA RISPOSTA ALLA DOMANDA DI CLAUDIO — *"come siete messi per le prop?"* — E HA SCOPERTO CHE IL PROBLEMA PIU' GRANDE NON E' UNA TAGLIA: E' LA FEDELTA' DEL BANCO.** 💶 **(1) Il fatto che pesa piu' di tutti**: sul conto **REALE 10105439** le due sedie (**770101** DAX Apertura, che entra a **BuyLimit**, e **770611** ORB, che entra a **BuyStop**) sono **attaccate, armate e a 0,65%**, con Guardian **779002 v1.12** vivo — ma **non hanno eseguito UNA SOLA OPERAZIONE**: `ABTG_SlippageLogger` ha **0 deal**. 👉 **Non esiste ancora nessuna misura di esecuzione su un conto BCM vero**, e siccome BCM ha confermato che **il demo non simula lo slippage**, tutto il forward che abbiamo e' **ottimista per costruzione**. ⚔️ **(2) TRE ROUND, TRE VERDETTI, ZERO PROMOZIONI — ed e' il risultato giusto**: 🔴 **GAPCASH Nasdaq SCARTO** (il fenomeno c'e', **77 giornate-evento** contro ~62 attese, ma **il segno si rovescia sui tick BCM**: esterno +0,0988% → BCM **−0,0487%**, monotonia rotta **4 volte su 7**; costo del verdetto **8 minuti di macchina** invece di un EA e settimane di forward) · 🔴 **OROLOGIO sugli INDICI, meta' DAX: l'orologio NON esiste** (**0 fasce asimmetriche su 72 in OOS**, 1 su 72 in IS che **perde da entrambe le parti**; in quasi tutte le celle **LONG = −SHORT esatto** → ogni ora "verde" era **deriva del toro**. L'ora 15, sopravvissuta col **solo lato long**, ha `asimm = 0`: **la regola dei due lati del 25/08 ha fatto esattamente il lavoro per cui era stata scritta**. ⚠️ Il **Dow non e' girato**) · 🟠 **R118 pavimento dello stop: nessuna cella promossa** — il conflitto col collega si spiega (**a 20 punti indice il pavimento tocca fra lo 0% e il 2% dei trade**: *non dissentivamo, misuravamo due operazioni diverse chiamandole allo stesso modo*), e il risultato solido e' l'**asimmetria RISCHIO/MERITO**: allargare lo stop **riduce il DD in modo riproducibile** (85% delle celle, le due finestre concordano) e **costa edge in modo NON riproducibile** (OOS 29/56 = monetina). 🔴 **E morde la sedia viva**: la configurazione **in campo** dell'ORB e' **l'unica che sfonda il muro del 10% sotto slippage** (DD OOS **9,76% → 10,34%**), e **la correzione che la proteggerebbe non e' promuovibile** (n=119 < 150). 🐞 **(3) R119: IL CANARINO HA SPARATO AL PRIMO GIRO VERO.** La chiave inventata si chiamava **`Delay`** — **in MT5 non esiste**, e **MT5 ignora in silenzio le chiavi `[Tester]` sconosciute** (uscita 0, CSV prodotti, gemelli a posto, **referto verde**). Round fermato a **2 corse su 8**, **6 corse a tick risparmiate**. *Senza il canarino il referto avrebbe detto "le due sedie vive reggono fino a 500 ms di ritardo": **una bugia con i numeri sotto**.* 📌 **La correzione di metodo che resta e vale per tutti i round futuri**: `walkforward_generico.ps1` scrive **`ExecutionMode=0`** dal **07/08/2026** — **non era nascosto, era dichiarato nell'`.ini` e nessun referto l'ha mai letto** → **AREA I nuova** (**I1** slippaggio, **I2** `ExecutionMode`, **I3** determinismo, **I4** spread). 🟢 Gratis dal round fallito: **gemelli di determinismo PASSATO** su U30USD e il **DD promesso della 770611 agli atti** (**5,65% IS / 6,54% OOS**, merito **sospeso**). ✍️ **(4) DUE FIRME** (`report/FIRME_2026-09-07.md`, _"firmo il punto 1, lancia l'orologio"_): **H13** — il **pavimento di frequenza si misura per FAMIGLIA** (motore × simboli), soglia invariata a **1,00 op/giorno**, motivata da un conto vero con statistiche MQL5 (**3-5 EA su 26 simboli → 0,29-0,47 op/giorno per simbolo**: *nel campo ogni istanza e' un cecchino*) — e **C10** — **tetto per CLUSTER al 3,0%**, motivato da due portafogli larghi con DD **misurati 32,59% e 45,64%**. 🔴 **C10 E' FIRMATO MA NON ATTIVO: nel Guardian il tetto per cluster NON ESISTE**, e va detto ogni volta che lo si cita — con l'aggravante che la firma gemella (famiglia = **piu' simboli**) rende quel buco **piu' probabile**. 🆕 **(5) C11 nuova — la collisione P0-7**: su un gap in giu' su NASUSD alle **14:30 server** la sedia viva **GATED SHORT 770250 vende** e il motore gap-cash **avrebbe comprato**; **nessun controllo di casa la vede** (il cap C1 conta gli SL vivi, il tetto C8 conta **simbolo+LATO**, e due lati opposti passano entrambi). Oggi **non e' armata** (il candidato e' morto), ma **il precedente resta**. 🥇 **(6) IL FORWARD RICONTATO OGGI**: piccolo **25/08→04/09** — 66 chiusure, **65 ingressi**, 27 magic, **−125,28 €**, **15 magic su 27 in perdita**, e **7,22 ingressi per giornata operativa** contro i **4,85** della finestra 03-28/08 → **la resa sale dal 63% all'89%** del promesso (⚠️ 9 giornate: segnale, non verdetto); 100k **10/08→04/09** — 27 chiusure in **16 giornate**, **+2.854,99 = +2,85%**, peggior giornata realizzata **−647,82 = −0,648%**. ✅ **La correzione della sovrataglia A2 e' verificata DAI DATI**: il lotto ORB passa da **19,70** (02/09) a **4,80/4,60** (03/09) = **÷4,1**. 📏 **(7) LA CONSISTENCY, MISURATA CON LA FORMULA DICHIARATA** (e la riga "43,6%" del v16 **e' superata, perche' li' la formula non c'era**): best day / **profitto dei giorni POSITIVI** — la formula **letterale** FTMO 1-Step — = **37,27%** grezzo, **29,97%** normalizzando la giornata fuori contratto; best day / **profitto totale** = **58,86%** e **59,48%**. ✅ **Sul 2-Step (la nostra prop di riferimento) NON esistono consistency rule dure**: non ci tocca. 🟠 Sul **35% di FundingPips** (solo sui reward) il grezzo **sfora** e il normalizzato **rientra**: la differenza la fa **una singola giornata fuori contratto**, non la strategia. 📏 **(8) LA DISTANZA DALLA PROP, quantificata**: mancano **7.145,01 € = 7,15 punti** al target di fase 1; al ritmo **grezzo** ≈**1,8 mesi**, al ritmo **normalizzato** ≈**3,1 mesi** — dentro la forchetta dell'AREA H (1,8-3,0 mesi a flotta migrata), dalla parte buona; margine **7,7×** sul muro giornaliero **ma sul solo REALIZZATO** (il flottante non e' nel CSV → **la peggior giornata vera e' NON MISURATA**); **16 giornate operative** contro un minimo di 4. ✅ **(9) E DUE COSE CHE NELLO STESSO GIORNO HANNO CHIUSO O MOSSO UN BLOCCO, fatte da altri agenti e qui registrate senza duplicarle**: **(a)** 🥇 `report/CENSIMENTO_CONTRATTI.md` — **il prerequisito della C3, aperto da tre settimane, e' CONSEGNATO**: **47 righe-sedia / 41 sedie uniche** per CONTO, con DD promesso + **fonte** + deposito/rischio del backtest + `n` + merito + frequenza; **40 MISURATE · 3 da riprodurre · 4 NON MISURATE**, merito **PIENO su 15** e **SOSPESO su 28** → **la corsia RISCHIO della C3 e' applicabile su 40 sedie su 47** e il **cancello 2** si muove per la prima volta. 🔴 Non diventa verde: **tre sedie PostNews girano senza nessun DD promesso** (e la **771203** rischia **1,30% per evento** e **ha gia' operato**), il `BREAKOUT_EA_JPY_v3` ha il **magic illeggibile**, la **771322** ha **TRE DD incompatibili** (2,64 / 17,68 / 13,1%), e **l'ultima foto `.chr` e' del 25/08** — **il conto reale non e' mai stato censito**; **(b)** ⚙️ il **tetto per cluster e' stato IMPLEMENTATO nella stessa giornata** (Guardian **v1.13** + `ABTG_PausaGuardian.mqh` **v1.60**, commit `cdb2037`): due input nuovi **spenti di default** (`InpMaxClusterRiskPct=0`, `InpClusterMappa=""`), parametri nuovi **in coda** alla firma di `ABTG_GuardiaIngresso()`, tutto il codice nuovo dentro `if(StringLen(cluster_mappa)>0)` → **no-op verificato, non promesso**. 🔴 **Lo stato cambia di UNA parola sola: da "firmato ma non attivo" a "implementato ma NON COMPILATO e NON COLLAUDATO" — che vuol ancora dire NON ATTIVO**, e va detto ogni volta. 🆕 **(10) M37 · M38 · M39** in COSA MANCA (i deal del logger · **R119 v3** con la chiave giusta · **la definizione dei cluster + il collaudo**, che vengono **prima** di chiamarlo protezione). ✅ **(11) E IN SERATA R119 E' STATO RIFATTO CON LA CHIAVE GIUSTA — ED E' MISURATO.** Secondo giro `..._v3_EXECMODE` (avvio 16:23:55): **G0 MORDE**, **16 CSV su 16**, **gemelli G1 IDENTICI su 8 corse su 8**, **0 rilievi**, stessa finestra **2024.09.26 → 2026.06.30** a tick reali col **preset del conto REALE** (0,65%). 🔴 **L'ipotesi congelata — _"LIMIT contro STOP: 770101 degrada poco, 770611 molto"_ — E' FALSIFICATA SU TUTTE E DUE LE META'**: lo **STOP (`770611`) e' IMMUNE** (OOS **identico alla cifra** a 50, 100 e 500 ms; in IS si muove dello **0,4%** solo a 500), e il **LIMIT (`770101`) e' quello che si muove** — **≤2% sul profitto, ≤3% sul DD**, e **in MEGLIO** (OOS **+1,5%** di profitto e **−0,8%** di DD a 500 ms, con perfino **un trade in piu'** in IS: 175 → 176). ⚠️ **Il meccanismo NON e' spiegato e va capito prima di tarare qualcosa**: *un ritardo che migliora i risultati non e' un costo, e' un cambio di quale pendente viene toccato*. ⚠️ E **non sostituisce lo slippaggio**: `ExecutionMode` ritarda l'**esecuzione**, non modella lo **scarto di prezzo** — **I1 resta NON MISURATO**. ⚠️ Il **verdetto formale contro i criteri S1-S4 non e' ancora scritto**: il referto della macchina dichiara di **non giudicare**. 🟢 **Il regalo che vale piu' del round**: per la **prima volta le due sedie del conto REALE hanno un DD promesso misurato ALLA TAGLIA VIVA** (0,65%, tick, stessa finestra, stesso driver) — **`770101`: n 175 IS / 270 OOS (merito PIENO), PF 1,1540 / 1,4110, DD 3,1749% / 4,3501%** · **`770611`: n 71 / 119 (merito SOSPESO), PF 1,2308 / 1,6749, DD 5,6530% / 6,5389%**. 🔴 E si apre un **conflitto fra due misure di casa dello stesso giorno**: il censimento dei contratti da' alla `770101` **≈6,89% a 0,65%** (da R83), R119 misura **4,35% OOS** alla stessa taglia — **finestre e split diversi, e il preset del reale ha il trailing fisso a 410**: la differenza va **attribuita prima** di scegliere quale numero usa la corsia RISCHIO della C3. 🛑 **Zero modifiche al forward da questo documento, zero preset toccati, zero acquisti autorizzati, nessun candidato promosso, nessuna sedia accesa o spenta.** → **61 parametri (18C · 21P · 18A · 2 chiusi per misura · 1 NON MISURATO · 1 parziale)** | il mandato permanente di Claudio del 07/09 (*"a fine settimana dimmi come siete messi"*) + i tre referti del giorno + le due firme + il forward ricontato sui CSV del 04-05/09 + i tre preset del conto reale letti per intero |
| 05/09/2026 sera | **v19** | 🔴 **BCM CONFERMA: LA DEMO NON SIMULA LO SLIPPAGE, SOLO IL REALE.** Risposta diretta di Claudio col broker (`CONFIG_PROP_SPREAD_SLIPPAGE_2026-09-05.md`, domanda D6): chiude nel senso peggiore la domanda che pesava su tutto il forward. **Conseguenza permanente per il metodo**: ogni riga FORWARD di **H1** (100k e piccolo) e' OTTIMISTA sul fronte esecuzione — manca per costruzione il costo che R109 aveva misurato a **21,5 punti su un solo stop**. Non risolve il conflitto di segno banco/forward (mancare slippage renderebbe il forward piu' ottimista, non lo spiega), ma il rischio di drawdown vero **non lo dice piu' il forward demo da solo**: lo dice il backtest a tick con slippage simulato (gia' la prassi R109/R116/R117) o il conto reale. 🛡️ **Audit di flotta sul filtro spread**: presente in **86 EA su 104**, **spento in 75** — incluse **tutte** le sedie indici vive (DAX/Dow/Nasdaq Apertura, MaxMinNotte). Nessuna deviazione esplicita sulle chiusure d'emergenza del Guardian (proprio quando lo spread e' piu' largo). 📡 **VPS: la domanda e' chiusa con un numero**, non un'ipotesi — ping VPS→BCM misurato **mediana 21,3 ms** (16,5-38,3), zona "accettabile": **non e' li' il collo di bottiglia** (confermato anche dalla documentazione MetaQuotes: lo slippage sullo stop lo decide il tempo di elaborazione del broker, centinaia di ms, non la rete). 🏹 **Tre cacce di frequenza chiuse in un giorno (M5/M15/M30), 0 promossi su 111 candidati esaminati**: M5 e M15 muoiono di COSTO (il pedaggio spread vale 1,1-1,7× il cancello H8 — misurato come curva monotona, non aneddoto); M30 muore di EDGE, non di costo (spread = 1/30 del take, il vincolo e' la monotonia, fallita 3 volte su 3) — e un falso positivo pericoloso (look-ahead bias, t=+8,49 apparente) catturato e smontato con 3 controprove indipendenti prima di essere promosso per errore. 🪦 **R116 LondonFx CHIUSO**: bocciato su entrambe le gambe anche a banco pulito (agenti locali forzati a 1, gemelli riconfermati identici) — EURUSD E −0,1078R/DD 37%, GBPUSD E −0,2357R/DD 53% — morto per geometria, non per costi (spread di Londra e' il piu' stretto misurato in casa). 🎯 **RELATIVO R117 in corso**: 2 difetti di infrastruttura trovati e chiusi in giornata (generico condiviso che rifiutava le celle congelate; autotest dell'EA con una variabile riusata fra due casi) — nessuno dei due tocca la cella firmata N=40/σ=1,35; collaudo del porto PASSATO (scarto 3/20 e 2/20) e gemello interno IDENTICO 9/9 su D30_PORTO. 🛡️ **Spread Logger schierato**: EA di sola lettura installato e vivo sul conto piccolo (autotest 36/36), raccoglierà la prima mappa oraria vera dello spread P95 per simbolo. 🛑 **Zero modifiche al forward da queste scoperte, zero preset toccati, zero candidato promosso.** |
| 03/09/2026 mattina | **v18** | 🔬 **LA MATTINATA IN CUI IL PIANO HA MISURATO SE STESSO — una violazione di contratto scoperta e riparata in 8 ore, un difetto di codice inchiodato e censito, un debito di misura pagato, e il primo candidato della caccia frequenza che arriva a un round firmato.** 🚨 **(1) SOVRATAGLIA ORB 3,3× — TROVATA E CHIUSA**: la pagella automatica del 02/09 scopre che il **+1.680,35 del dry-run 100k e' un DIFETTO** — l'ORB `770611` girava a **1,0% invece dello 0,3% di contratto**, provato dalle **sei coppie appaiate** piccolo/100k (rapporto lotti da **5,86-6,44** a **20,29 e 19,70** fra il 21 e il 24/08 = il rapporto dei saldi) e da una verifica indipendente sullo stop (59,00 pt × 19,70 lotti = **0,996% del conto**). Riportata a contratto, la sedia sul 100k vale **circa −383 €** invece di +496,44. ✅ **Corretta da Claudio il 03/09 alle 08:10** (foto 07:34 a 1.0 → foto 08:10 a **0.3**, nessun altro campo toccato; piccolo verificato a 1,0% e chiuso con Annulla) → riga **A2**. La verifica sul campo e' la **prima coppia gemella futura** (rapporto atteso ~6). 🕶️ **(2) IL MISTERO DEL TRAILING E' RISOLTO, E NON ERA IL TRAILING**: la **FOTO A delle 08:19** inchioda `LARRY DOW S` (772341) **aperta dal 01/09 08:45** su U30USD, viva per tutta la vita del trade ORB → **ipotesi A (`SelPos`/hedging) confermata 3 giorni su 3** (19/08, 21/08, 02/09). I due pannelli sono **identici in ogni riga**: _"il colpevole e' il codice, identico sui due conti: cambia solo il VICINATO"_. 🔎 **(3) AUDIT DI FLOTTA — 126 file**: **26 🔴 vulnerabili · 8 🟠 "mezzo fix" (leggono per ticket, CHIUDONO per simbolo) · 85 🟢 sani · 7 ⚪**; e la scoperta che allarga il caso: la stessa `SelPos()` governa **`HandleOCO()`** e **`InpOneTradePerDay`** → **e' un difetto di RISCHIO**, non di gestione. 📊 **(4) VERIFICA STORICA — il verdetto che l'audit aspettava**: **ZERO chiusure incrociate** su 21 chiusure `expert` esaminate una per una e **zero occasioni** armate → **i 🟠 NON passano davanti ai 🔴**; ma il **danno passivo dei 🔴 e' agli atti**: `ABTG_ORB` nativo ha aperto **il secondo lato vietato dal pannello in 4 giornate su 16**, correlate **4 su 4** con un vicino a ticket piu' basso (**1 probabilita' su 1820**), = **20 operazioni dove il contratto ne prevede 16, +25% di frequenza**. ⚖️ Scritto anche il fatto scomodo: **quelle 4 operazioni hanno guadagnato +201,15 €** — fortuna su quattro, non un argomento. ⚠️ **Correzione di premessa misurata**: non "la posizione piu' vecchia" ma **quella col `position_id` piu' basso** — diverso nel **16,5% dei casi**, perche' i pendenti ereditano il ticket di quando sono stati *piazzati*. → **riga C9 nuova (PROPOSTO)** + **M34/M35 nuove**. 🔴 **E IL CONFLITTO DEL GIORNO, con una firma di IERI**: il fix dell'ORB **richiede una ricompilazione** e **D1** dice _"fase 1 senza ricompilazioni"_ (l'ORB e' **uno dei 5 mirror del 100k in collaudo**: ricompilare **brucia il criterio 4**, rischio X13) → raccomandazione dichiarata: **fixare solo sul terminale del PICCOLO**, dove il difetto morde. 🪑 **(5) FASE 4 VERDE**: **6 sedie mute su 6 VIVE e attaccate** (foto 08:47-08:54) — nessuna morta in silenzio → **H10**. 💸 **(6) SPREAD MISURATO, DEBITO DEL 23/08 PAGATO**: **252 milioni di tick**, **% solo-bid 0,000% su 3 simboli su 3** (→ **le corse a Model 4 usavano lo spread VERO: niente da rifare**), mediane in sessione **1,6-2,0 punti indice**, **DAX di NOTTE 3,5-3,9**, Dow ora 23 con P95 7,0 e max 101. **La convenzione "2.0 NON MISURATO" era giusta in sessione e OTTIMISTA fuori** — e **C2 si applica ORA PER ORA** (take ≥ 3× lo spread **dell'ora**). I round **gia' giudicati non si riscrivono**; dai futuri si cita questa misura. **H12 resta APERTA** perche' il **forex** non e' misurato (→ **M36**). 🏹 **(7) IL VIVAIO SI ORDINA**: 🪦 **V8 NON PROMOSSO confermato DA UNA MISURA** (ablazione: il filtro RSI toglie **9-13%** → e' un incrocio EMA(5/20), famiglia gia' morta due volte) · 🥇 **LondonFx PRIMO SUPERSTITE del passo 0** (EURUSD **19/24**, GBPUSD **24/24**, filtro che **taglia il 73-77%**) → ✍️ **R116 FIRMATO** (_"FIRMO TUTTO, ANCHE LA A SU F5"_: F5=A, ora 8, rischio **0,65%**, cap 2%/giorno acceso, prova di rischio come round separato) e **EA contenitore costruito** (`ABTG_LondonFx.mq5` v1.00, **repo-only, MAI compilato**, **hedge-safe dalla nascita**) · 🟢 **RELATIVO 8/10** (divergenza relativa fra due indici: la frequenza e' **una manopola**) e 🟠 **VGRSI 6/10** in coda (ma i suoi numeri nascono da **ri-ottimizzazione settimanale** = la regola misurata **anti-predittiva**, e i **DD dichiarati sono al muro o sopra**) · ⏸️ DayFlow scavalcato. ⚖️ **(8) M31 INCORPORATA** (eseguita il 02/09, arrivata dopo la chiusura del v17.1): il DD della famiglia Aperture DAX passa da **16,39% realizzato** a **7,36% normalizzato** = **1,18×** il promesso — **l'emergenza cade, lo sforo resta**, e viene **tutto** dalle modalita' BUY/SELL **che non sono piu' in campo** (RETEST-only: DD **0,28%**); la corsia **MERITO resta scattata anche normalizzata** (−277,64 €). 🛑 **Zero modifiche al forward da questo documento, zero preset toccati, zero acquisti autorizzati, nessun candidato promosso, nessuna sedia accesa o spenta.** → **54 parametri (16C · 21P · 16A · 1 chiuso)** | la pagella del 02/09 + le tre foto e la correzione della taglia + audit e verifica del difetto hedging + il censimento delle sedie mute + la misura dello spread + i due referti di sonda (V8, LondonFx) + la firma R116 + la 4ª battuta della caccia + M31 |

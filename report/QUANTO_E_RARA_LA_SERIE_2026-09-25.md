...
(16 controlli, tutto verde). Importa `mc_challenge_ftmo_stato.py` senza modificarlo.
...
## 0. 📌 In cinque righe
1. 🎲 **Nel backtest della flotta non è un evento raro: è una cosa da ogni 2-3 mesi.** Tre giornate con
...
2. ⚖️ **Il pool di casa a deposito fisso li gonfia un po', lo slittamento vero li sgonfia: la banda
   onesta è 4-7 episodi per a1 K=3 e 3-6 per a2 K=4** (§2.2). In tutte e tre le letture resta
   **più volte all'anno**.
3. 🧪 **Il contro-esempio non morde**: sulla sequenza **vera** delle date le serie **non si ammucchiano
   più** che rimescolando le giornate (p = 0,28-0,90; autocorrelazione lag-1 **+0,001**). Il MC di casa
   non le sottostima **per l'ordine dei giorni** (§3).
4. 📉 **Il peggio del backtest è molto peggio di oggi**: 4 giornate a **−12,32%** e 3 a **−10,16%**
   (11-16/12/2025), con una serie negativa di **5 giornate** (due volte) [MISURATO].
5. 🟢 **Nel forward BCM, sulle stesse sedie, l'ultima serie di questa taglia è del 27-29/07**, sul
   piccolo `50503392`: solo `770101`, con **tre stop nello stesso giorno** il 29/07. **In agosto-settembre:
   zero**, su tutti e due i demo. 👉 La risposta alla domanda di Claudio: **dal campo, da due mesi;
   dal backtest, ci si doveva aspettare una ogni 2-3 mesi.**
...
### 2.1 Le frequenze [MISURATO: episodi e finestre · DERIVATO: "ogni N" e "per anno"]
...
### 2.2 Due sensibilità che spostano il numero, in versi opposti
...
- ✅ **Fra giornate, niente ammucchiamento misurabile**: il vero sta dentro il rimescolato, con
  autocorrelazione lag-1 di **+0,001** (banda ±0,129). **Il MC di casa non sottostima le serie per
  colpa dell'ordine.**
- 🟠 **Nello stesso giorno, invece, sì**: **`770202` × `771531` hanno r = +0,62** (n = 22 giornate
  ...
  **+0,008**. Togliendo quella correlazione (R-sedia), gli episodi K=3 scendono da ~5 a ~4. Il MC di
...
- 🧪 **Il metodo vede l'ammucchiarsi quando c'è** (autotest): con dodici perdite attaccate trova 1
  ...
  sedie identiche trova 20 giornate doppie contro 2,9. **Quindi il "non morde" è una misura, non
  una cecità dello strumento.** Con 1 anno e 4-7 episodi, però, la potenza è **bassa**: un
  ammucchiamento modesto non si escluderebbe.
...
osservati. Sul piccolo il DAX a stop perde ~114 EUR, cioè **~2,3% del saldo** e non l'1%, per
l'arrotondamento dei lotti: la % del saldo lo peserebbe il doppio. L'evento FTMO in R vale
−4.909,28 / 1.600 = **−3,068 R**. **[DERIVATO]**

| conto · sedie | periodo · giornate con op. | R per sedia | a K=3 / K=4 ≤ −3,07 R | (b) 3 negative | serie max · peggior K=3 |
|---|---|---|---|---|---|
| **piccolo `50503392`** (1,00%) · `770101` `770202` `770411` `771531` | 20/07 → 22/09 · 39 | `770101` 114,30 (7 stop) · `771531` 36,62 (4 stop a coppia) · `770202` `770411` **nominale** 50,77 | **1 / 1**: **27-29/07** (−3,75 R; il 29/07 da solo −3,07 R = tre stop `770101`) | **1**: 06-10/08 (−1,96 R) | 3 · −3,75 R |
...
- 🟢 **Agosto-settembre: nessuna serie della taglia di quella FTMO su nessuno dei due demo.**
...
6. **Forward**: R stimato su pochi stop (sul 100k **uno per sedia**), R nominale dove lo stop non è mai
   ...
   calendario su 277 cadono nel weekend, come in `st.pool_feriali`).

---
_Autotest: 16/16 verdi. Controlla l'aritmetica dell'evento, il contro-esempio della soglia tonda
...
contro la pagella del 19/08 e la stima di R su gambe sintetiche._

_Cancello: strato 1 (`controlla_riga.py --oggetto md`) **nessun difetto meccanico**. Due rilievi
letti: il documento non ha blocchi ```, e il conto `50504263` compare solo come fonte dei dati, in
lettura. 🔴 **Strato 2 (`controllo-preventivo`) NON eseguito da questa sessione: non è un PASS
completo**, e il documento **non** va a Claudio prima dello strato 2._

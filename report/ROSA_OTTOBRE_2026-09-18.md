...
| **9** 🆕 | `Nasdaq_Apertura_US` **GATED SHORT `770250`** · piccolo @0,35% | NASUSD M15 | **DD 4,54% @0,65% → ~2,4% @0,35%** · pegg. giornata −0,72% · 🔴 **n 104 deal → forbice 45-104**, e **finestra intera, nessuno split** — `CENSIMENTO_CONTRATTI_v2.md` r.352 | 🟢 **DA 🟡 NO (93%) A PASS (+16%)** alla mediana **e PASS (+9%) al p95** — stop **83,2 idx `[INFERITO]`**, `ANCORA...2026-09-18.md` r.410/425. ⚠️ **Il referto ha RITIRATO il proprio «pavimento MISURATO 75,3»**: il tag è sceso da `[MIS]` a `[INF]`, il verdetto no | 🔴 **~0,23 op/g** promessa · **0 operazioni dal deploy del 30/08** | 🔵 **L'esclusione «per costo» NON REGGE PIÙ.** Resta ferma su **(M)** uno split IS/OOS vero e **(M)** perché non opera da 19 giorni. 👉 **Coda all'imbuto, MAI in campo in automatico** |
...
| con **R5 ≥ 40× anche al p95** | 🟢 **2** — `770202` (41,3×) · `770250` (43,8×) | 🆕 **non era mai stato scritto** |
...
| **5** | **`770250`** GatedShort NAS | **1 round (split IS/OOS) + capire perché non opera** | 🔵 **rientra in gioco oggi**: il cancello di costo la promuove. Ma R1 è rotto e in campo **fa zero da 19 giorni** |
...
|---|---|---|---|
| **`770250`** GatedShort NASUSD M15 | 🟡 *«NO, 93% del pavimento»* (~37,2× `[INF]`) | 🟢 **PASS +16% alla mediana e +9% al p95** | **coda all'imbuto**: manca uno **split IS/OOS vero** (1-2 min di tester) |

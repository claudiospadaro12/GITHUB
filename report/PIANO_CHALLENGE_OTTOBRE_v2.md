...
👉 Con l'ORB in **OPPRANGE** il conto migliora di **4,04 punti** (6,54% → 2,50%): 🔴 **ERRATA 12/09/2026 — vedi `report/A4_LA_GEOMETRIA_DELLO_STOP_2026-09-12.md`.** Il **4,04 non regge**: `9,7623 − 3,840 = 5,9223`, e il 4,04 e' una grandezza **a 0,65%** incollata in una parentesi che dice *@1%*. Peggio: le due celle confrontate **NON sono gemelle** — quella da 3,840 cambia **quattro** input (`InpSLMode` + `InpSLBufferPts=500` + `InpTPMode=0` + `InpTP_R=1.5`), non uno. Il numero onesto a UNA manopola e' **−5,47 @1%** (9,7623 -> 4,2956), **ma col profitto a −53,9% e il PF invariato: e' una DELEVERAGE, non un miglioramento**. E in campione **OPPRANGE PERDE** (PF IS 0,962, profitto −738,45), quindi e' **bocciata dai cancelli firmati il 19/08**. 👉 La ragione vera del +1 sedia **non e' il DD: e' il CANCELLO DEL COSTO** (HALFRANGE 29,5x, sotto il pavimento di 40x; OPPRANGE ~64x).
...
| **A4** | ✍️ **FIRMA SULLA GEOMETRIA DELL'ORB DEL GIORNO 1** — OPPRANGE invece di HALFRANGE, sul **conto della challenge**, **senza toccare il reale 10105439** | 👤 **Claudio** | 🟢 **Δ +1 sedia** · **e −4,04 punti di DD** (9,7623 → 3,840 @1%) | 🔴 **ERRATA 12/09/2026 — vedi `report/A4_LA_GEOMETRIA_DELLO_STOP_2026-09-12.md`.** Il **4,04 non regge**: `9,7623 − 3,840 = 5,9223`, e il 4,04 e' una grandezza **a 0,65%** incollata in una parentesi che dice *@1%*. Peggio: le due celle confrontate **NON sono gemelle** — quella da 3,840 cambia **quattro** input (`InpSLMode` + `InpSLBufferPts=500` + `InpTPMode=0` + `InpTP_R=1.5`), non uno. Il numero onesto a UNA manopola e' **−5,47 @1%** (9,7623 -> 4,2956), **ma col profitto a −53,9% e il PF invariato: e' una DELEVERAGE, non un miglioramento**. E in campione **OPPRANGE PERDE** (PF IS 0,962, profitto −738,45), quindi e' **bocciata dai cancelli firmati il 19/08**. 👉 La ragione vera del +1 sedia **non e' il DD: e' il CANCELLO DEL COSTO** (HALFRANGE 29,5x, sotto il pavimento di 40x; OPPRANGE ~64x).
...
| **A5-bis** 🆕 | 🧊 **RIALLINEARE IL CAMPO — `A5` COSI' COM'E' SCRITTA E' SOTTOSTIMATA** (aggiunta **13/09**, vedi `PIANO_PROP.md` **K3**). A5 parla di **15 sorgenti**; quelli in campo sul piccolo **50503392** che differiscono dal repo sono **42 su 42**, compilati fra il **5 e il 18/08**, e il piu' arretrato e' il **`Guardian`** (**414** righe in campo contro **899** nel repo). 👉 **Nessuna correzione fatta da meta' agosto in poi e' realmente in campo**, quindi *"le protezioni firmate sono attive"* e' **[NON MISURATO]** su ogni sedia viva di quel conto | 👤 **Claudio** (ricompilare **cambia i volumi**: e' un gesto sul campo) · 🤖 agenti per l'inventario in sola lettura | 🔴 **Priorita' proposta: pari o superiore a QUALUNQUE round nuovo** — un round compra un numero per il futuro, questo dice se le sedie vive **oggi** sono protette. ⚠️ **Ma il metodo NON e' "ricompila tutto in un colpo"**: il referto che ha trovato il difetto lo sconsiglia esplicitamente (porterebbe in campo un mese di modifiche mai girate, **comprese quelle che cambiano le taglie**). L'ordine e': **(1) Guardian** · **(2) inventario firmato** sedia per sedia · **(3) una sedia per volta**, col rapporto dei lotti prima/dopo come prova. 🟢 **E la prima domanda non costa una ricompilazione**: sul piccolo il giornale dice **"GUARDIAN: nessuna riga"** l'11 **e** il 12/09 — li' il problema non e' la versione, e' che **non c'e' nessuna rete**. Fonti: `IL_CAMPO_E_FERMO_A_AGOSTO_2026-09-11.md` · `IL_GUARDIAN_IN_CAMPO_2026-09-12.md` · `IL_GUARDIAN_CHE_SCHIEREREMO_2026-09-12.md` |
...
| **B4** | 🧪 **R125** (firmato, ~7 min) e/o **R126** | 👤 **Claudio davanti al PC** — il runner e' sola lettura | **Δ +0 sedie per ottobre**; comprano misure per **dopo**. ✏️ **AGGIORNATA 13/09**: il perimetro **e' stato allargato** (corsia ROUND sul solo `C:\MT5_Backtest`, conto **50504400**) e la notte 12/13-09 ha girato **36 righe in corsia ROUND**, fra cui **`R126a/b/d`**. 🔴 **`R125` NON e' fra queste** e resta da lanciare. 🔴 **E i CSV non sono nel repo** (classe 307): restano sul VPS → `PIANO_PROP.md` **M44** |
| **B4-bis** 🆕 | 📥 **CARICARE I CSV DELLA NOTTE 12/13-09** — **34 round con CSV validi** gia' prodotti (29 a uscita 0, **5 a uscita 3 che hanno CSV per costruzione**), 1 giro a vuoto (`canfrz`, `-SoloControllo`), 1 non misurato (`cemad02`) | 👤 **Claudio** (`carica_risultati.ps1`, **zero macchina**: e' un trasferimento) | 🔴 Senza questo passo, *"il tappo si e' sciolto"* **non e' vero**: si e' solo spostato da *"non girano"* a *"girano e nessuno legge"*. Fonte: `backtest_pipeline/coda/referti/REFERTO_RUNNER_20260913_033003.txt` |
...
|---:|---|---:|---|---|
| 1 | **Geometria ORB del giorno 1: OPPRANGE** sul conto challenge | 🟢 **+1** | e' la configurazione che rischia, ed e' **−4,04 punti di DD**. 🔴 **Non tocca il reale 10105439** | **15/09** | 🔴 **ERRATA 12/09/2026 — vedi `report/A4_LA_GEOMETRIA_DELLO_STOP_2026-09-12.md`.** Il **4,04 non regge**: `9,7623 − 3,840 = 5,9223`, e il 4,04 e' una grandezza **a 0,65%** incollata in una parentesi che dice *@1%*. Peggio: le due celle confrontate **NON sono gemelle** — quella da 3,840 cambia **quattro** input (`InpSLMode` + `InpSLBufferPts=500` + `InpTPMode=0` + `InpTP_R=1.5`), non uno. Il numero onesto a UNA manopola e' **−5,47 @1%** (9,7623 -> 4,2956), **ma col profitto a −53,9% e il PF invariato: e' una DELEVERAGE, non un miglioramento**. E in campione **OPPRANGE PERDE** (PF IS 0,962, profitto −738,45), quindi e' **bocciata dai cancelli firmati il 19/08**. 👉 La ragione vera del +1 sedia **non e' il DD: e' il CANCELLO DEL COSTO** (HALFRANGE 29,5x, sotto il pavimento di 40x; OPPRANGE ~64x).
...
| 4 | **Compilazione R4** (F7 + ricarico, mai su `C:\BCM_Reale`) | **+0** | ricompilare **cambia il volume** delle sedie vive | **15/09** · 🔴 **RIVALUTATA 13/09 — vedi A5-bis**: il perimetro vero non e' 15 sorgenti ma **42 su 42** sul piccolo **50503392**, **Guardian compreso** (414 righe in campo contro 899). Il **+0 sedie** resta vero, **ma qui il "+0" non misura il valore**: l'azione non aggiunge sedie, **dice se quelle vive sono protette**, e oggi la risposta e' **[NON MISURATO]**. 👉 Priorita' proposta **pari o superiore a qualunque round nuovo**, ma **con l'ordine del referto** (Guardian → inventario firmato → una sedia per volta), **non** "ricompila tutto in un colpo" |
...
| 🆕 **I 5 zip dei round a uscita 3** (`ROUND_r127c/r127b/r139a/r139b/r139c.zip` sul Desktop del VPS) | 👤 **Claudio**, sola lettura | *"Per ognuno dei 5 zip, copiami la riga `ESITO`: dice `ROUND GIRATO CON RILIEVI (N)` — quanto vale **N**?"* 🔴 **N=1 → era solo il `-Modello 1` (r.408) e il rilievo si chiude. N=2 → c'e' anche un PID sparito, e allora si guarda il Giornale del reale `10105439`.** Perche' la domanda e' cambiata: `PIANO_PROP.md` **K2** |
| 🆕 **I CSV dei 34 round della notte 12/13-09** | 👤 **Claudio** (`carica_risultati.ps1`) | *"Zip dei risultati dal VPS: sono gia' misurati, manca solo il trasferimento. `r126d` da solo ha gia' speso 4.693 secondi di macchina."* |
| 🆕 **Il Guardian sul piccolo `50503392` non scrive niente nel giornale** (11 **e** 12/09) | 👤 **Claudio**, 30 secondi dentro MT5 | *"Sul terminale `50503392` (`BCM Markets MT5 Terminal`) il `ABTG_Guardian` e' davvero attaccato a un grafico, con la faccina sorridente? Perche' in due giorni di giornale non ha scritto una riga."* 🔴 E' la domanda **piu' economica** del blocco A5-bis: non richiede nessuna ricompilazione |
...
`backtest_pipeline/righe/RIGA_ROUND_VPS.ps1` **r.408 · r.572 · r.717-725 · r.745-746 · r.835-837** (i codici d'uscita, letti nel codice) ·
`report/RILIEVO_TERMINALI_NOTTE_2026-09-13.md` 🔴 *(la sua attribuzione dell'uscita 3 e' **contestata** da questo giro: `PIANO_PROP.md` **K2**)* ·
`report/IL_CAMPO_E_FERMO_A_AGOSTO_2026-09-11.md` · `report/IL_GUARDIAN_IN_CAMPO_2026-09-12.md` ·
`report/IL_GUARDIAN_CHE_SCHIEREREMO_2026-09-12.md` · `mql5/Experts/ABTG_VolExpBreak.mq5` (775301, **mai compilato**) ·
`backtest_pipeline/prove/R145a_volexp_kstop_M30_NASUSD.txt` + `R145b_volexp_kstop_M30_U30USD.txt` *(**non** in `CODA.txt`, verificato)*

**Memoria dei difetti:** `backtest_pipeline/CHECKLIST_RIGA_DI_LANCIO.md` classi **224**, **225**, **226**, 🆕 **307** (il runner non pubblica i CSV), 🆕 **308**, 🆕 **309** 🔴 *(con la correzione di K2: la parola "esclusivamente" non regge)*

//+------------------------------------------------------------------+
//|                                              KSQ_FVG_EA.mq5      |
//|                              KSQuantitative — KSQuants           |
//|                    Fair Value Gap EA with Regime Detection        |
//+------------------------------------------------------------------+
#property copyright   "KSQuantitative"
#property link        "https://www.mql5.com"
#property version     "1.00"
#property description "FVG-based EA with ATR/Points TP & SL and Regime Filter"
#property strict
...
enum ENUM_REGIME_FILTER
  {
   REGIME_NONE    = 0,  // No Filter (trade all conditions)
   REGIME_EMA     = 1,  // EMA Trend Filter
   REGIME_ADX     = 2,  // ADX Strength Filter
   REGIME_BOTH    = 3   // EMA + ADX Combined
  };

enum ENUM_LOT_MODE
  {
   LOT_FIXED   = 0,  // Fixed Lot Size
   LOT_RISK    = 1   // % Risk-Based Lot Size
  };

enum ENUM_PARTIAL_MODE
  {
   PARTIAL_OFF    = 0,  // No Partial Close
   PARTIAL_MIDTP  = 1,  // Close 50% at Mid-TP, trail rest
   PARTIAL_FIXED  = 2   // Close % at fixed R:R level
  };
...
struct FVGData
  {
   ...
   bool      isBullish;
   bool      isFilled;
   bool      isTraded;   // Prevent multiple entries on same FVG
  };
...
input string             InpTradeComment     = "KSQ_FVG";  // Trade Comment
...
int OnInit()
  {
   ...
   Print("KSQ FVG EA initialized | Symbol: ", _Symbol, " | Magic: ", InpMagicNumber);
...
void OnTick()
  {
   ...
   if(bars == g_prevBars) return;
   g_prevBars = bars;

   //--- Refresh daily drawdown tracker
   RefreshDailyBalance();
   ...
   for(int i = 0; i < g_fvgCount; i++)
     {
      ...
      //--- BULLISH FVG Entry: price pulls back INTO the zone
      if(g_fvgArray[i].isBullish && InpShowBullish)
        {
         // Entry condition: last bar LOW touched zone AND close was bullish (rejection candle)
               ...
               if(trade.Buy(lots, _Symbol, askPrice, sl, tp, InpTradeComment))
                 {
                  ...
                  string msg = "KSQ FVG BUY | " + _Symbol + " | Lots: " +
      ...
      //--- BEARISH FVG Entry: price rallies INTO the zone
            ...
            if(lots > 0 && sl > 0 && tp > 0 &&
               HasSufficientMargin(ORDER_TYPE_SELL, lots, bidPrice))
              {
               if(trade.Sell(lots, _Symbol, bidPrice, sl, tp, InpTradeComment))
                 {
                  g_fvgArray[i].isTraded = true;
                  string msg = "KSQ FVG SELL | " + _Symbol + " | Lots: " +
                               DoubleToString(lots, 2) + " | Zone: " +
                               DoubleToString(g_fvgArray[i].lower, _Digits) + "-" +
                               DoubleToString(g_fvgArray[i].upper, _Digits);
                  Print(msg);
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      //--- Break-Even Logic (runs before trailing)
      if(InpUseBreakEven)
      ...
      if(closeVol < curVol)
        {
         if(trade.PositionClosePartial(ticket, closeVol))
            Print("KSQ FVG: Partial close ", closeVol, " lots on ticket #", ticket);
        }
     }
  }
   ...
   if(dailyDD >= InpMaxDailyDD)
     {
      ...
      if(TimeCurrent() - lastAlert > 3600)
        {
         Print("KSQ FVG: Daily drawdown limit reached (", DoubleToString(dailyDD, 2), "%)");
   ...
   if(totalDD >= InpMaxTotalDD)
     {
      ...
      if(TimeCurrent() - lastAlert2 > 3600)
        {
         Print("KSQ FVG: Total drawdown limit reached (", DoubleToString(totalDD, 2), "%)");
...
//+------------------------------------------------------------------+
//| CHART INFO DISPLAY (Dashboard)                                    |
//+------------------------------------------------------------------+
void OnChartEvent(const int id, const long &lp, const double &dp, const string &sp)
  {
   // Build a live dashboard comment
   string regime = "UNKNOWN";
   if(InpRegimeFilter == REGIME_NONE) regime = "DISABLED";
   else
     {
   ...
   string info = "=== KSQ FVG EA ===\n" +
                 "Symbol   : " + _Symbol + "  TF: " + EnumToString(Period()) + "\n" +
                 "FVGs Active : " + IntegerToString(g_fvgCount) + "\n" +

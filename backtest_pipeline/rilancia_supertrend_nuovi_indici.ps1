# =====================================================================
#  rilancia_supertrend_nuovi_indici.ps1  --  SCREEN OHLC del
#  SupertrendReversal sui NUOVI indici BCM (H1 e H4).
#  Il SupRev e' l'unico motore che generalizza (DAX+Nasdaq+oro):
#  proviamolo su Dow, Stoxx50, CAC40, FTSE, Nikkei.
#  Screen OHLC 1-min (affidabile su H1/H4, veloce). I vincitori poi
#  li validiamo in real-tick e diventano _Ottimizzato.
#  Griglia: StMult 2.5/3.0/3.5 x AtrP 8/9/10 x TP_RR 2.0/2.5/3.0, rischio 1%.
#  Ripresa: salta le prove gia' salvate in .\risultati_supertrend_nuovi_indici\
#
#  !! I simboli devono essere in Market Watch con storico scaricato.
#     Se un nome e' diverso sul tuo BCM, dimmelo e correggo l'.ini.
# =====================================================================
...
$Targets=@(
  @{sym="U30USD";tf="H1"}, @{sym="U30USD";tf="H4"},   # Dow Jones
  @{sym="E50EUR";tf="H1"}, @{sym="E50EUR";tf="H4"},   # Euro Stoxx 50
  @{sym="F40EUR";tf="H1"}, @{sym="F40EUR";tf="H4"},   # CAC 40
  @{sym="100GBP";tf="H1"}, @{sym="100GBP";tf="H4"},   # FTSE 100
  @{sym="225JPY";tf="H1"}, @{sym="225JPY";tf="H4"}    # Nikkei 225
)
$Work= if($PSScriptRoot){$PSScriptRoot}else{(Get-Location).Path}; Set-Location $Work
Write-Host "=== SCREEN SupertrendReversal su NUOVI INDICI (Dow/Stoxx/CAC/FTSE/Nikkei, H1+H4) ===" -ForegroundColor Cyan
...
foreach($t in $Targets){$ini="valid_SupRevScr_$($t.sym)_$($t.tf)"; $dl+=@{u="$RawBase/backtest_pipeline/ini/$ini.ini";o="ini\$ini.ini"}}
...
$Results=Join-Path $Work "risultati_supertrend_nuovi_indici"; New-Item -ItemType Directory -Force -Path $MqlExperts,$Results|Out-Null
...
foreach($t in $Targets){
  $n++
  $ini="valid_SupRevScr_$($t.sym)_$($t.tf)"
  ...
  else{Write-Host ("        (manca il CSV: simbolo {0} non in Market Watch/senza storico? o MT5 aperto?)" -f $t.sym) -ForegroundColor Yellow}
}
Write-Host "`n=== FINITO === risultati in $Results" -ForegroundColor Cyan
Write-Host "Mandami i CSV: scelgo i vincenti (PF>1.2 con abbastanza trade) e li validiamo in real-tick." -ForegroundColor White

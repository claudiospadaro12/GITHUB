# =====================================================================
#  rilancia_supertrend_validazione.ps1  --  VALIDAZIONE REAL-TICK dei
#  3 vincitori SupertrendReversal su indici: DAX H1, DAX H4, Nasdaq H1.
#  Griglia stretta robusta (StMult 3.0/3.5, AtrP 9/10, TP_RR 2.5/3.0),
#  rischio 1%. Conferma i numeri OHLC prima del forward.
#  Ripresa: salta le prove gia' salvate in .\risultati_supertrend_validazione\
# =====================================================================
...
$Targets=@(
  @{ini="valid_SupRevRT_D30EUR_H1";sym="D30EUR";tf="H1"},
  @{ini="valid_SupRevRT_D30EUR_H4";sym="D30EUR";tf="H4"},
  @{ini="valid_SupRevRT_NASUSD_H1";sym="NASUSD";tf="H1"}
)
$Work= if($PSScriptRoot){$PSScriptRoot}else{(Get-Location).Path}; Set-Location $Work
Write-Host "=== VALIDAZIONE REAL-TICK SupertrendReversal (DAX H1/H4, Nasdaq H1) ===" -ForegroundColor Cyan
...
$Results=Join-Path $Work "risultati_supertrend_validazione"; New-Item -ItemType Directory -Force -Path $MqlExperts,$Results|Out-Null
...
foreach($t in $Targets){
  $n++
  $done=Join-Path $Results "valid_SupRevRT_$($t.sym)_$($t.tf).csv"
  ...
  Write-Host ("   [{0}/{1}] {2} {3} (real tick)..." -f $n,$Targets.Count,$t.sym,$t.tf) -ForegroundColor Cyan
  (Start-Process -FilePath $Terminal -ArgumentList "/config:`"$(Join-Path $Work "ini\$($t.ini).ini")`"" -PassThru).WaitForExit()
  if(Test-Path $csv){Copy-Item $csv -Destination $done -Force; Remove-Item $csv -Force; Write-Host ("        OK -> valid_SupRevRT_{0}_{1}.csv" -f $t.sym,$t.tf) -ForegroundColor Green}
...
Write-Host "Mandami i 3 CSV: confermo i numeri real-tick e promuoviamo gli _Ottimizzato." -ForegroundColor White

# =====================================================================
#  rilancia_live5m_v2.ps1  --  VALIDAZIONE REAL-TICK del DAX Live5m_v2
#  (versione migliorata: range filter + stop al floor + slippage 100).
#  Domanda: la ricetta v2 salva il breakout M5 o e' morto come il nativo?
#  Real tick, rischio 1%. Se regge -> creo anche il Nasdaq Live5m_v2.
#  Ripresa: salta la prova gia' salvata in .\risultati_live5m_v2\
# =====================================================================
...
$EA="ABTG_DAX_Live5m_v2"
$Targets=@(
  @{ini="valid_DAX_Live5m_v2";sym="D30EUR"}
)
$Work= if($PSScriptRoot){$PSScriptRoot}else{(Get-Location).Path}; Set-Location $Work
Write-Host "=== VALIDAZIONE REAL-TICK DAX Live5m_v2 (rischio 1%) ===" -ForegroundColor Cyan
...
$Results=Join-Path $Work "risultati_live5m_v2"; New-Item -ItemType Directory -Force -Path $MqlExperts,$Results|Out-Null
...
foreach($t in $Targets){
  $n++
  $done=Join-Path $Results "valid_$($EA)_$($t.sym).csv"
  ...
  if(Test-Path $csv){Copy-Item $csv -Destination $done -Force; Remove-Item $csv -Force; Write-Host ("        OK -> valid_$($EA)_{0}.csv" -f $t.sym) -ForegroundColor Green}
...
Write-Host "Mandami il CSV: se qualche combo va PF>1.2 in real-tick, la ricetta v2 salva il breakout M5 e faccio il Nasdaq v2. Se sono tutte <1, il M5 e' morto e lasciamo perdere." -ForegroundColor White

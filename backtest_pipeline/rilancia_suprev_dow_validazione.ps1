# =====================================================================
#  rilancia_suprev_dow_validazione.ps1  --  VALIDAZIONE REAL-TICK dei
#  vincenti dello screen SupRev sui nuovi indici: Dow H4, Dow H1,
#  + CAC 40 H4 (bonus). Griglia stretta sul cluster, rischio 1%.
#  Conferma i numeri OHLC prima di promuovere a _Ottimizzato.
#  Ripresa: salta le prove gia' salvate in .\risultati_suprev_dow_validazione\
# =====================================================================
...
$Targets=@(
  @{ini="valid_SupRevRT_U30USD_H4";sym="U30USD"},   # Dow H4 (il piu' forte)
  @{ini="valid_SupRevRT_U30USD_H1";sym="U30USD"},   # Dow H1
  @{ini="valid_SupRevRT_F40EUR_H4";sym="F40EUR"}    # CAC 40 H4 (bonus)
)
$Work= if($PSScriptRoot){$PSScriptRoot}else{(Get-Location).Path}; Set-Location $Work
Write-Host "=== VALIDAZIONE REAL-TICK SupRev nuovi indici (Dow H4/H1, CAC H4) ===" -ForegroundColor Cyan
...
$Results=Join-Path $Work "risultati_suprev_dow_validazione"; New-Item -ItemType Directory -Force -Path $MqlExperts,$Results|Out-Null
...
foreach($t in $Targets){
  ...
  else{Write-Host "        (manca il CSV: MT5 aperto? o simbolo senza storico?)" -ForegroundColor Yellow}
}
Write-Host "`n=== FINITO === risultati in $Results" -ForegroundColor Cyan
Write-Host "Mandami i 3 CSV: confermo i numeri real-tick e creo gli _Ottimizzato del Dow." -ForegroundColor White

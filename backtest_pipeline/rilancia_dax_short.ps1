# =====================================================================
#  rilancia_dax_short.ps1  --  test DAX Apertura SOLO-SHORT (ST off,
#  ora 8, stessa config buona del long) in real tick. Per capire se lo
#  short ha edge o no (assicurazione per mercato ribassista).
# =====================================================================
...
Write-Host "=== TEST DAX APERTURA SOLO-SHORT (ora 8, ST off) ===" -ForegroundColor Cyan
...
if((Get-Process -Name "terminal64" -ErrorAction SilentlyContinue) -and -not $Force){Write-Host "!!! Chiudi MetaTrader prima (0 CSV altrimenti). Controlla anche Gestione attivita'." -ForegroundColor Red; exit 1}
...
Write-Host "Ottimizzo SHORT (real tick)..." -ForegroundColor Cyan
(Start-Process -FilePath $Terminal -ArgumentList "/config:`"$(Join-Path $Work "ini\$ini.ini")`"" -PassThru).WaitForExit()
if(Test-Path $old){Copy-Item $old -Destination (Join-Path $Results "valid_DAX_Apertura_SHORT_${sym}.csv") -Force; Write-Host "OK -> risultati_validazione_aperture\valid_DAX_Apertura_SHORT_${sym}.csv" -ForegroundColor Green}
else{Write-Host "(manca il CSV: MT5 era aperto? Chiudilo e rilancia)" -ForegroundColor Yellow}
Write-Host "Mandami il CSV: confronto short vs long." -ForegroundColor White

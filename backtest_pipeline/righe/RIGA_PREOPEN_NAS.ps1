# =====================================================================
#  MARCATORE_RIGA_PREOPEN_NAS_v1
#  RIGA_PREOPEN_NAS.ps1  --  IL LIVELLO PRE-APERTURA SUL NASDAQ
#  ABTG_Nasdaq_Apertura_US  su  NASUSD  M15, TICK REALI
#  Terzo gemello della famiglia PREOPEN (dopo Dow e DAX): STESSA forma,
#  STESSI criteri, altro mercato.
#
#  >>> E QUI C'E' UNA COSA CHE NEGLI ALTRI DUE NON C'E', E VA LETTA
#      PRIMA DI TUTTO IL RESTO:
#      QUESTA SEDIA E' SPENTA. Il magic 770201 e' stato spento il
#      18/08/2026 alle 09:41 (FIRMA 5, "SPEGNILE TUTTE E TRE"), ed e'
#      l'unica sedia che il censimento marca SENZA CONTRATTO
#      (report/CONTRATTI_SEDIE.md riga 45). Ha QUATTRO verdetti negativi
#      indipendenti agli atti: tick reali 31/07 (PF 0,82 - DD 17%),
#      walk-forward 05/08 (19 celle OOS negative su 20), R83+R84
#      (12 configurazioni, 12 OOS negative), R107 (la geometria viva del
#      Dow trasposta qui: long PF 1,110 - short PF 0,460).
#      Conseguenza sul SIGNIFICATO del round, non sulla sua meccanica:
#      questo NON e' un round che rimette in campo una sedia, e' UNA
#      PORTA CHE SI CHIUDE CON UN NUMERO. Un risultato positivo produce
#      una CELLA CANDIDATA che, per tornare in campo, dovrebbe passare
#      dalla porta di rientro del criterio C3 -- altra decisione, altra
#      firma.
#
#  >>> QUESTO E' UN ROUND CON CRITERI DI MERITO GIA' CONGELATI.
#      NON e' un conta-operazioni come i tre PASSO 0 preparati oggi.
#      I criteri stanno in prove\PREOPEN_RETEST_NAS_M15.txt, sezioni
...
#      La pagina da leggere PRIMA e' righe\RIGA_PREOPEN_NAS_DA_MANDARE.md
#
# ---------------------------------------------------------------------
...
#   FASE RIF    (0c) la cella di RIFERIMENTO (InpRangeMode=0, cioe' la
#               geometria di R107_NAS_00_riflong) rifatta SU M15, sui due
#               lati: e' il denominatore del criterio "+0,10 di PF".
#               >>> SI CHIAMA RIF E NON RIFERIMENTO, e non e' pignoleria: un
#               RIFERIMENTO riproduce un numero AGLI ATTI e fa anche da gate G0
#               ("il banco e' sano"). Qui non c'e' nessuna sedia viva con
#               questa geometria, quindi NON C'E' NESSUN GATE G0 in
#               questo round, e va detto ogni volta che si legge un
#               numero.
...
#   - NON tocca il forward. Il magic 770201 e' SPENTO dal 18/08, ma e'
#     nella lista dei VIETATI lo stesso: un'identita' spenta resta
#     un'identita' occupata (R107 lo dice cosi'), e domani quel magic
#     potrebbe tornare in campo dalla porta di rientro.
#     Sono VIETATI anche i 761200/761201 di R107 e i cinque blocchi del
#     round PREOPEN DOW e del round PREOPEN DAX: bruciati, non si
#     riusano.
...
#     NASUSD dal 2024.09.26 sono agli atti (sonda del 17/08, COMPLETO).
#   - NON misura la CORRELAZIONE COL CANDIDATO DOW di questa stessa
#     famiglia, che arma alla STESSA ORA (14:30 server) col MECCANISMO
#     IDENTICO e sugli STESSI valori dell'asse. ROTTA_PROP regola 1: il
#     drawdown della prop e' UNO. Se passassero tutti e due, la
#     sovrapposizione delle GIORNATE va misurata PRIMA di promuoverli
#     insieme -- ed e' il passo successivo dichiarato, non questo round.
...
#     1 passata singola (costo) + 2x(3x2)x2 = 24 passate di riferimento
...
#
#  LA RIGA CHE SI INCOLLA sta in righe\RIGA_PREOPEN_NAS_DA_MANDARE.md
# =====================================================================
[CmdletBinding()]
param(
  ...
  [string]$SoloFase      = "",       # "" = tutto | COSTO | RIF | GRIGLIA
  ...
  [int]$Spread           = 0         # 0 = spread CORRENTE, ma SCRITTO nell'ini
                                     ...
                                     #  terminale. E' quello che hanno fatto R101
                                     #  e R107, cioe' i round da cui vengono i
                                     #  numeri agli atti di questa famiglia.
)
...
$Work   = Join-Path $env:USERPROFILE "abtg_preopen_nas"
...
$PuntoIndice = 1.0     # 1 punto indice = 1,00 di PREZZO su NASUSD.
                       #  MISURATO IN R97, ed e' un gate FIRMATO di quel
                       #  round: cifre=2, tick=0.10 -> 1 punto indice = 100
                       #  punti MT5. R88 aveva dovuto correggere la
                       #  premessa sbagliata "1 punto indice = 10 point":
                       #  qui il numero non si assume, si cita la misura.
                       #  (R97_REFERTO par. 3, R109_CRITERI par. 308-311,
                       #   README_ABTG_Aperture riga 141.)
                       #  Su un indice il "pip" NON ESISTE.
...
$CritDeltaPF = 0.10    # e la regione deve BATTERE il riferimento di +0,10 di PF
...
#    770201 e' IL MAGIC DI QUESTA SEDIA, SPENTA dal 18/08: e' vietato
#    lo stesso, perche' un'identita' spenta resta occupata (R107) e
#    perche' la porta di rientro del criterio C3 potrebbe riaccenderla.
#    Sono vietati anche i magic BRUCIATI dai round precedenti (R83,
#    R101, R107) e dai due gemelli PREOPEN preparati il 28/08 (DOW e
#    DAX): riusarli mescolerebbe i deal di due misure diverse nella
#    cache del tester e negli export per-trade, che portano il magic
#    nel nome del file.
$MagicVietati = @(770201,                                  # <<< QUESTA SEDIA (SPENTA
                                                           #     dal 18/08: un'identita'
                                                           #     spenta resta occupata)
                  770101,770202,770411,770511,770601,770611,770901,
                  971501,770402,970901,970912,970913,770532,
                  761200,761201,                           # R107 (bruciati)
                  773200,773201,773230,773231,             # R101 riferimento DAX
                  773300,773301,                           # R101 riferimento Dow
                  773500,773501,773600,773601,             # round PREOPEN DOW (28/08)
                  773700,773701,773800,773801,773900,773901,
                  781600,781601,781700,781701,             # round PREOPEN DAX (28/08)
                  781800,781801,781900,781901,782000,782001,
                  777010,777011,777020,777021,777030,777031, # R83 NASUSD
...
#  parariferimento spazzolato, e CONTIENE 'Peggior Giornata %':
...
function VerdettoS0a([double]$takeNetMed){
  ...
  # dentro l'incertezza del suo riferimento e' il modo piu' elegante di
...
#  nessun parariferimento in comune, e chiamarle "adiacenti" gonfierebbe le
...
$LAVORI = @()
$LAVORI += (Lav "rif_long"   "PREOPEN_RIF_NAS_M15.txt"        "RIF"     "LONG"  "RIFERIMENTO 0c: livello ORB (RangeMode=0) su M15, lato LONG"  782300 782301 "1" "0" "0" @("InpRangeMode","InpPrevWindowMin"))
$LAVORI += (Lav "rif_short"  "PREOPEN_RIF_NAS_M15_SHORT.txt"  "RIF"     "SHORT" "RIFERIMENTO 0c: livello ORB (RangeMode=0) su M15, lato SHORT" 782400 782401 "0" "1" "0" @("InpRangeMode","InpPrevWindowMin","InpAllowLong","InpAllowShort"))
$LAVORI += (Lav "griglia_long" "PREOPEN_RETEST_NAS_M15.txt"       "GRIGLIA" "LONG"  "LA GRIGLIA: livello PRE-apertura, lato LONG"            782100 782101 "1" "0" "1" @())
$LAVORI += (Lav "griglia_short" "PREOPEN_RETEST_NAS_M15_SHORT.txt" "GRIGLIA" "SHORT" "LA GRIGLIA: livello PRE-apertura, lato SHORT"          782200 782201 "0" "1" "1" @("InpAllowLong","InpAllowShort"))
$ProvaCosto = "PREOPEN_COSTO_NAS_M15.txt"
$ProvaBase  = "PREOPEN_RETEST_NAS_M15.txt"

$Fasi = @("COSTO","RIF","GRIGLIA")
...
$roCentro   = -1
$MagicCosto = -1
$PwRif    = -1           # il valore a cui il riferimento pinna InpPrevWindowMin
...
$Verdetti    = @{}         # NASCE QUI, non dentro il try: il blocco del VERDETTO lo
                           ...
                           #  RIFERIMENTO lo lasciava $null e "$Verdetti.ContainsKey(...)"
...
$RifPf     = @{}         # spostati fuori per simmetria con $Verdetti
$RifCel    = @{}

try{
  Titolo ("PREOPEN NAS -- il livello pre-apertura -- modo " + $Modo)
  ...
  if($Periodo -ne "M15"){
    throw ("-Periodo e' " + $Periodo + ": i file prova dichiarano @PERIODO M15 e il riferimento 0c esiste PROPRIO per confrontare a pari TF. Su un TF diverso il round non risponde alla domanda che ha scritto.")
  }
  GateDate "argomenti della riga" $DaQuando $Fino

  # le due finestre, CALCOLATE con la stessa formula del driver generico
  # (FrazioneIS 0.40) e poi confrontate con quelle agli atti di R107: se
  # divergono, i numeri di questo round non sono confrontabili con quelli.
  ...
  if($IS_A -ne "2025.06.09" -or $OOS_Da -ne "2025.06.10"){
    [void]$Rilievi.Add("LE FINESTRE NON SONO QUELLE DI R107: qui IS finisce " + $IS_A + " e OOS parte " + $OOS_Da +
      ", agli atti di R107 (da cui viene la geometria di riferimento NASUSD, e di R101 per Dow e DAX) erano 2025.06.09 / 2025.06.10. O e' cambiata la FrazioneIS del driver generico, o e' cambiata la finestra passata a questa riga: il confronto coi numeri agli atti NON e' piu' a pari finestra.")
  }

  Dico ("pin ......... " + $Pin)
  Dico ("fasi ........ " + $(if($FaseSel -eq ""){ "COSTO -> RIF -> GRIGLIA (tutte)" } else { "SOLO " + $FaseSel }))
  ...
  foreach($nf in @($ProvaBase,$ProvaCosto,"PREOPEN_RETEST_NAS_M15_SHORT.txt","PREOPEN_RIF_NAS_M15.txt","PREOPEN_RIF_NAS_M15_SHORT.txt")){
  ...
  # IL BLOCCO R30 esiste SOLO nel Nasdaq (il Dow e il DAX non ce l'hanno)
  # e i file prova lo pinnano SPENTO. Se sparisse dal sorgente sarebbero
  # 15 PARAMETRI ORFANI, e MT5 li ignorerebbe IN SILENZIO: il round
  # crederebbe di misurare "il motore del Dow" e misurerebbe altro.
  foreach($r30 in @("InpUseVolRegime","InpUseSRFilter")){
    if($srcEA -notmatch $r30){ throw ("nel sorgente al pin non c'e' " + $r30 + ": i file prova di questo round pinnano il blocco R30, e un parariferimento che l'EA non ha viene ignorato IN SILENZIO da MT5.") }
  }
  # E il Nasdaq NON deve avere InpAllowReverse (ce l'ha solo il DAX): se
  # un giorno lo avesse, i file prova andrebbero rifatti perche' non lo
  # pinnano, e girerebbe al suo default compilato senza che si veda.
  if($srcEA -match '(?m)^\s*input\s+bool\s+InpAllowReverse'){
    [void]$Rilievi.Add("il sorgente al pin ORA HA InpAllowReverse, che quando i file prova sono stati scritti (28/08) esisteva solo sul DAX. I file prova di questo round NON lo pinnano: girerebbe al suo default compilato. Va aggiunto ai cinque file prova prima di leggere qualunque numero.")
  }
  ...
  foreach($f in @(Get-ChildItem -LiteralPath $Prove -Filter *.txt)){
    ...
    foreach($rr in $righeF){
      ...
      if($h.ContainsKey($nome)){ throw ($f.Name + ": DUE righe per '" + $nome + "'. In [TesterInputs] un parariferimento doppio fa fare a MT5 ZERO passate.") }
      $h[$nome] = $val
  ...
  foreach($lv in $LAVORI){
    ...
    if($vr -ne $lv.VRange){ throw ($lv.Prova + ": InpRangeMode vale "  + $vr + ", il lavoro " + $lv.Id + " lo vuole " + $lv.VRange + " (0=ORB, il riferimento; 1=pre-apertura, il candidato). E' L'UNICA cosa che il round sta misurando: se e' sbagliata qui, non c'e' niente da leggere.") }
    ...
    if(($h["InpSessionHour"] -split '\|\|')[0] -ne "14"){ throw ($lv.Prova + ": InpSessionHour deve essere 14 (ORA SERVER BCM = 15:30 italiane). 15 sarebbe l'ora italiana: cestinare.") }
    if(($h["InpSessionMin"]  -split '\|\|')[0] -ne "30"){ throw ($lv.Prova + ": InpSessionMin deve essere 30.") }
    ...
    if(($h["InpMinStopPts"]  -split '\|\|')[0] -ne "500"){ throw ($lv.Prova + ": InpMinStopPts deve essere 500 (pavimento SL, lezione R109). Non si spegne.") }
    if(($h["InpSkipIfTight"] -split '\|\|')[0] -ne "0"){  throw ($lv.Prova + ": InpSkipIfTight deve essere 0 (il pavimento si APPLICA, non fa saltare il trade). E' la geometria di R107_NAS_00_riflong.") }
    # IL BLOCCO R30 DEVE ESSERE SPENTO: con i due interruttori a 0 il
    # motore Nasdaq E' il motore Dow (MISURATO nel sorgente il 25/08:
    # UpdateVolRegime() ritorna alla prima riga, VolRegimeSL() ritorna lo
    # stop invariato, SRBlocked() ritorna false subito). Acceso, il round
    # misurerebbe DUE cose insieme: il livello e un filtro mai validato.
    if(($h["InpUseVolRegime"] -split '\|\|')[0] -ne "0"){ throw ($lv.Prova + ": InpUseVolRegime deve essere 0. E' un opt-in del round 30 MAI VALIDATO: acceso, questo round misurerebbe il livello E un filtro insieme.") }
    if(($h["InpUseSRFilter"]  -split '\|\|')[0] -ne "0"){ throw ($lv.Prova + ": InpUseSRFilter deve essere 0. Stessa ragione di InpUseVolRegime.") }
  ...
  # --- A QUALE VALORE IL RIFERIMENTO PINNA InpPrevWindowMin. Non si scrive a
  #     memoria: si LEGGE nei due file riferimento, e i due devono dire lo
  #     stesso. Serve per ritrovare le righe del riferimento dentro il CSV,
  #     che porta una colonna per OGNI input (MISURATO su un CSV vero di
  #     questo EA: 11 colonne di statistiche + 80 di parametri).
  foreach($nfM in @("PREOPEN_RIF_NAS_M15.txt","PREOPEN_RIF_NAS_M15_SHORT.txt")){
    $hM = $mappe[$nfM]
    ...
    if($PwRif -lt 0){ $PwRif = $vM }
    elseif($PwRif -ne $vM){ throw ("i due file RIFERIMENTO pinnano InpPrevWindowMin a valori diversi (" + $PwRif + " e " + $vM + "): i due lati non sarebbero la stessa cella.") }
  }
...
"@
  ...
  # -------------------------------------------------------------------
  #  5. LE CORSE (RIFERIMENTO e GRIGLIA), via driver generico
  # -------------------------------------------------------------------
  ...
  foreach($lv in $LAVORI){
    if($FaseSel -ne "" -and $FaseSel -ne $lv.Fase){
      ...
      # perderebbe il riferimento e nessun confronto sarebbe piu' possibile.
...
if($PwRif -lt 0){ $PwRif = 60 }

Titolo "RACCOLTA"
$Cart = Join-Path $Dsk ("PREOPEN_NAS_" + $Modo + "_" + $Stamp)
New-Item -ItemType Directory -Force -Path $Cart | Out-Null

ScriviRef "====================================================================="
ScriviRef (" PREOPEN NAS -- il livello PRE-APERTURA su " + $Simbolo + " " + $Periodo)
...
ScriviRef " NON stanno qui: stanno in prove\PREOPEN_RETEST_NAS_M15.txt, sezioni"
...
# --- il riferimento, lato per lato
try{
  $RifPf.Clear()   # gia' creato PRIMA del try -- "LATO,roIdx" -> PF OOS del riferimento
  $RifCel.Clear()
  ScriviRef ">>> UN CONFONDIMENTO VERO, misurato nel sorgente"
  ScriviRef "    (ABTG_Nasdaq_Apertura_US.mq5, riga 668 e righe 703-731: refEndMin ="
  ScriviRef "    openMin quando RangeMode non e' OPENING): il candidato"
  ScriviRef "    (InpRangeMode=1) arma alle 14:30 server e chiude la finestra alle 17:30"
  ScriviRef "    (3h00); il riferimento (InpRangeMode=0, InpRangeMinutes=35) arma alle"
  ScriviRef "    15:05 e chiude alla stessa ora (2h25). Il delta fra candidato e"
  ScriviRef "    riferimento e' in parte TEMPO (+35 min, +24% di finestra), non solo"
  ScriviRef "    LIVELLO. Chi legge un vantaggio del candidato deve sapere che una parte"
  ScriviRef "    non misurata di quel vantaggio potrebbe essere la finestra piu' lunga."
  ScriviRef "    (Sul DAX gli stessi 35 minuti valgono solo +6,5%, perche' li' la"
  ScriviRef "     finestra e' di 9 ore e mezza: qui e' il caso peggiore dei tre.)"
  ScriviRef ""
  ScriviRef ">>> E UN SECONDO CAPPELLO, CHE QUI VIENE PRIMA DEL PRIMO:"
  ScriviRef "    QUESTO NON E' UN METRO, E' UN RIFERIMENTO. Un metro riproduce un numero"
  ScriviRef "    AGLI ATTI e fa anche da gate G0 ('il banco e' sano'). Sul Nasdaq non"
  ScriviRef "    esiste nessuna sedia viva con questa geometria -- il magic 770201 e'"
  ScriviRef "    SPENTO dal 18/08 -- quindi IN QUESTO ROUND NON C'E' NESSUN GATE G0, e"
  ScriviRef "    dalle righe qui sotto NON si puo' concludere che il banco sia sano."
  ScriviRef "    Servono a UNA cosa sola: dare un DENOMINATORE alla griglia."
  ScriviRef ""
  foreach($lato in @("LONG","SHORT")){
    $lv = @($LAVORI | Where-Object { $_.Fase -eq "RIF" -and $_.Lato -eq $lato })[0]
    ScriviRef ("--- PASSO 0c -- IL RIFERIMENTO A PARI TF (M15), lato " + $lato + " ---")
    if($null -eq $lv -or $null -eq $lv.DatiOOS){
      ScriviRef ("  NON MISURATO (" + $(if($null -eq $lv){"lavoro assente"}else{$lv.Esito}) + "): senza il riferimento il criterio '+0,10 di PF' NON E' CALCOLABILE su questo lato.")
      ScriviRef ""
      continue
    }
    $gm   = Griglia $lv.DatiOOS @($PwRif) $ROuso
    $gmIS = Griglia $lv.DatiIS  @($PwRif) $ROuso
    ScriviRef ("  (le righe del riferimento si ritrovano nel CSV con InpPrevWindowMin=" + $PwRif + ", che li' e' INERTE)")
    ScriviRef "  offset    IS: prof / PF / DD% / n           OOS: prof / PF / DD% / n     peggGio%   gemelli"
    for($j=0; $j -lt @($ROuso).Count; $j++){
      $c     = $gm["0,$j"]
      $celIS = $gmIS["0,$j"]
      if($null -eq $c){ continue }
      if($null -ne $c.Pf){ $RifPf[($lato + "," + $j)] = [double]$c.Pf }
      $RifCel[($lato + "," + $j)] = $c
      ...
      if($c.Gem -ne "IDENTICI" -and $c.Gem -ne "NON MISURATO"){
        [void]$Problemi.Add("RIFERIMENTO " + $lato + " offset " + [int]($ROuso[$j]) + ": gemelli " + $c.Gem + " -- il banco non e' deterministico, il numero non si legge.")
      }
    }
    if($lato -eq "LONG"){
      ScriviRef "  ATTESA DICHIARATA (e NON un gate, perche' qui non c'e' G0): su M5 la"
      ScriviRef "  stessa geometria e' agli atti in R107 (cella NAS 00_riflong, offset 400)"
      ScriviRef "  con IS +1.371 / PF 1,080 / n 85 e OOS +1.873 / PF 1,110 / DD 5,62% /"
      ScriviRef "  n 113. Se qui su M15 esce vicino, il TF su questo motore e' quasi"
      ScriviRef "  neutro; se esce lontano, il TF conta -- ed e' esattamente perche' il"
      ScriviRef "  riferimento 0c e' stato preteso. In tutti e due i casi il denominatore"
      ScriviRef "  del round e' IL NUMERO MISURATO QUI."
      ScriviRef "  >>> LA BARRA VERA, LATO LONG: col riferimento a PF 1,110 il criterio"
      ScriviRef "  '+0,10 di PF' chiede al candidato circa 1,21."
      ScriviRef "  >>> E IL CAMPIONE E' SOTTILE: 113 operazioni OOS stanno sotto le 150"
      ScriviRef "  dell'Emendamento A. Come in R107 (cancello G4), su questa famiglia il"
      ScriviRef "  MERITO resta SOSPESO per campione. Il RISCHIO no: quello si giudica."
    }else{
      ScriviRef "  PROMEMORIA: il filtro EMA e' ACCESO (1/50 su H4) e in MonitorRetest lo"
      ScriviRef "  short passa solo con gBias 0 o -1: in 21 mesi di Nasdaq sopra la sua EMA"
      ScriviRef "  H4 lo short entra poco. Un n basso qui e' ATTESO, e se il riferimento"
      ScriviRef "  short ha n(OOS) sotto 30 il confronto '+0,10 di PF' sul lato short NON"
      ScriviRef "  SI FA."
      ScriviRef "  Il numero agli atti su M5 (R107) e': IS +8.399 / PF 3,220 / n 58 e"
      ScriviRef "  OOS -10.569 / PF 0,460 / DD 11,34% / n 59 -- l'IS piu' verde e l'OOS"
      ScriviRef "  piu' rosso di tutto quel round. Un IS spettacolare seguito da un OOS"
      ScriviRef "  rosso, se si ripete anche qui, NON e' un'anomalia da spiegare: e' la"
      ScriviRef "  firma gia' fotografata su questo simbolo e questo lato."
      ScriviRef "  >>> Conseguenza sul cancello: col riferimento a PF 0,460, 'batterlo di"
      ScriviRef "  +0,10' darebbe 0,56, lontanissimo dal pavimento. Su questo lato il"
      ScriviRef "  cancello che morde e' il PF>=1,10 ASSOLUTO, e vanno superati tutti e due."
    }
    ScriviRef ""
  }
  ...
  foreach($lato in @("LONG","SHORT")){
    ...
    foreach($rg in $regioni){
      ...
      $kRif = $lato + "," + $cj
      if(-not $RifPf.ContainsKey($kRif)){
        ScriviRef ("        confronto col RIFERIMENTO: NON CALCOLABILE (il riferimento a offset " + [int]($ROuso[$cj]) + " su questo lato non e' stato misurato). La regione NON si promuove: un candidato senza denominatore non e' un candidato.")
        [void]$dettagli.Add("regione di " + @($rg.Membri).Count + " celle: riferimento mancante")
        continue
      }
      $mCel = $RifCel[$kRif]
      $pfRif = [double]$RifPf[$kRif]
      $dPF = [double]$cc.Pf - $pfRif
      ScriviRef ("        RIFERIMENTO a pari offset (RangeMode=0, M15): PF " + (Fmt3 $pfRif) + "  n " + (FmtN $mCel.N))
      ...
      if($null -eq $mCel.N -or [double]$mCel.N -lt $CritN){
        ScriviRef ("        -> IL RIFERIMENTO HA n = " + (FmtN $mCel.N) + " (sotto " + $CritN + "): il denominatore non regge un confronto di MERITO (valvola R59 applicata al RIFERIMENTO). La regione NON si promuove.")
        [void]$dettagli.Add("regione di " + @($rg.Membri).Count + " celle: riferimento con n insufficiente")
        continue
      }
      if($dPF -ge $CritDeltaPF){
        ScriviRef ("        -> QUESTA REGIONE PASSA: >= " + $CritRegione + " celle adiacenti, tutti i criteri, e BATTE il riferimento di " + $dPF.ToString("+0.000",$INV) + " di PF.")
        $passa = $true
        ...
        ScriviRef ("        -> NON passa: pareggia o sta sotto il riferimento. 'Se pareggia il riferimento, il livello nuovo non serve' (file prova).")
...
ScriviRef ""
ScriviRef "  >>> I QUATTRO CAPPELLI CHE VANNO INSIEME ALLA PAROLA 'PASSA':"
ScriviRef "      1. 35 MINUTI: il candidato opera dalle 14:30, il riferimento dalle"
ScriviRef "         15:05 (misurato nel sorgente). Parte del delta e' TEMPO, non"
ScriviRef "         LIVELLO, ed e' +24% di finestra: e' il caso peggiore dei tre"
ScriviRef "         gemelli della famiglia."
...
ScriviRef "      3. NESSUN GATE G0: questo round non ha un METRO, ha un RIFERIMENTO."
ScriviRef "         Non esiste nessun numero agli atti da riprodurre, quindi non si"
ScriviRef "         puo' dire 'il banco e' sano' sulla base di questa corsa."
ScriviRef "      4. LA SEDIA E' SPENTA dal 18/08 con QUATTRO verdetti negativi"
ScriviRef "         indipendenti alle spalle (31/07, 05/08, R83+R84, R107). Un PASSA"
ScriviRef "         qui e' un fatto che va guardato con sospetto PRIMA che con"
ScriviRef "         entusiasmo: la prima cosa da cercare non e' l'edge, e' il difetto"
ScriviRef "         di misura."
if($DatiDaDisco){
  ScriviRef ""
...
ScriviRef "  - un PASSA qui NON riaccende la sedia 770201. Produce una CELLA"
ScriviRef "    CANDIDATA che, per tornare in campo, deve passare dalla PORTA DI"
ScriviRef "    RIENTRO del criterio C3 ('una sedia spenta rientra se una misura"
ScriviRef "    nuova le rida' una ragione'): altra decisione, altra firma."
ScriviRef "  - la CORRELAZIONE COL CANDIDATO DOW di questa stessa famiglia NON e'"
ScriviRef "    misurata qui, ed e' il rischio piu' concreto del round: il"
ScriviRef "    candidato Dow arma alla STESSA ORA (14:30 server), col MECCANISMO"
ScriviRef "    IDENTICO, sugli STESSI valori dell'asse -- cioe' sulle stesse"
ScriviRef "    finestre d'orologio -- su un indice che nella stessa mezz'ora si"
ScriviRef "    muove insieme a questo. ROTTA_PROP regola 1, e il drawdown della"
ScriviRef "    prop e' UNO."
ScriviRef "    >>> IL PASSO SUCCESSIVO, GIA' PROGRAMMATO E NON FATTO QUI: se"
ScriviRef "        passassero tutti e due, due PASSATE SINGOLE (cella Dow e cella"
ScriviRef "        Nasdaq, magic vergini distinti) sulla stessa finestra, i due"
ScriviRef "        export per-trade abtg_trades_<EA>_<simbolo>_<magic>.csv in"
ScriviRef "        Common\\Files, e il conto delle GIORNATE in comune. Nota di"
ScriviRef "        onesta': sovrapposizione_sedie.py NON serve a questo -- legge"
ScriviRef "        gli statement del FORWARD, non i per-trade di un backtest."
ScriviRef "  - il gemello DAX e' un round A PARTE (RIGA_PREOPEN_DAX.ps1),"
ScriviRef "    preparato insieme a questo; il gemello DOW e' girato il 28/08."
...
ScriviRef ""
ScriviRef 'COME SI RIPRENDE: si riparte dalla pagina righe/RIGA_PREOPEN_NAS_DA_MANDARE.md,'
ScriviRef 'NON da questa riga. Per rifare una fase sola: -SoloFase COSTO|RIF|GRIGLIA.'

$refPath = Join-Path $Cart "REFERTO_PREOPEN_NAS.txt"
...
Write-Host "FILE ATTESI NELLO ZIP: REFERTO_PREOPEN_NAS.txt + i 5 file prova + gli 8 CSV IS/OOS + gen_preopen_costo.ini + REPORT_COSTO.htm" -ForegroundColor Gray

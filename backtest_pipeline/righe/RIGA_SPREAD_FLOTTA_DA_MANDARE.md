...
## 📌 IL PIN — **`50d2f7b66c9b135a04fb19ac2a39bccb00bc1b8b`**  ✅ INSERITO (verificato con `git cat-file`: l'ULTIMO commit che tocca `RIGA_SPREAD_FLOTTA.ps1`, 11/09 -- contiene il driver **v3** + il motore `ABTG_SpreadOrario.mq5` v2 + la riparazione della chiusura chirurgica descritta due riquadri sopra). _Il pin `c5dbd68` del 31/08 e il marcatore `MARCATORE_RIGA_SPREAD_FLOTTA_v2` sono **BRUCIATI**: non incollarli più. **E il pin `e1c81430c...` del 03/09, che stava scritto qui prima, e' BRUCIATO ANCHE LUI, corretto il 14/09 dal cancello di giudizio**: e' lo script PRIMA della riparazione del 12/09 appena descritta -- a quel pin il bersaglio non e' il banco e la chiusura finale non e' filtrata (uccide OGNI terminal64 della macchina, conto REALE compreso). Lancia SOLO i blocchi di questa pagina, col pin scritto qui sopra._

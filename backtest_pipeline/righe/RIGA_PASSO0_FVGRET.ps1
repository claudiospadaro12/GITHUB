...
#     01_long   solo long             magic 776100/776101
#     02_short  solo short            magic 776400/776401
# ---------------------------------------------------------------------
#  QUESTO NON E' UN ROUND E NON DA' NESSUN VERDETTO.
#  E' il PASSO 0 preteso dal dossier della caccia SMC del 26/08/2026
#  (caccia_strategie\CACCIA_SMC_OB_FVG_2026-08-26.md, scheda P1):
#     "La frequenza e' il primo dato del round, prima di qualunque PF."
#  Si misura QUANTE OPERAZIONI produce il meccanismo. Il PF che esce dal
#  CSV si LEGGE ma NON si giudica: non ci sono criteri firmati, non c'e'
#  ablazione, e tre celle non sono un round.
#
#  L'IPOTESI, i tre esiti A/B/C e il cancello C2 stanno scritti in testa
#  a prove\ABTG_FvgRetest.txt e NON si riscrivono qui: un criterio
#  ricopiato in tre posti e' un criterio che prima o poi diverge.
#
#  ------------------------------------------------------------------
#  PERCHE' ESISTE QUESTO FILE invece di tre righe di
#  walkforward_generico.ps1 incollate a mano. Tre motivi MISURATI:
#
#   1. L'INCLUDE. ABTG_FvgRetest.mq5 fa
...
#     776xxx, cercati uno per uno in tutto il repo il 28/08/2026: zero
#     occorrenze). Il magic del SORGENTE (775501) e' VIETATO qui dentro.
...
#     U30USD e NASUSD, non sul DAX). Va letto sul simbolo.
...
#
#  LA RIGA CHE SI INCOLLA sta in righe\RIGA_PASSO0_FVGRET_DA_MANDARE.md
# =====================================================================
[CmdletBinding()]
param(
...
$Work   = Join-Path $env:USERPROFILE "abtg_passo0_fvg"
...
#  NESSUN ALTRO. Contare "due righe diverse" non basterebbe: DUE righe
#  SBAGLIATE darebbero lo stesso conteggio. 'Val' dice quanto devono
#  VALERE i due lati in quel file: se due file fossero SCAMBIATI il
#  diff resterebbe verde e questo no.
# =====================================================================
...
$CELLE = @()
$CELLE += (C "00_nudo"  "ABTG_FvgRetest.txt"          "IL MOTORE NUDO, due lati insieme -- porta i gemelli" 776000 776001 "1" "1" @())
$CELLE += (C "01_long"  "PASSO0_FVGRET_01_long.txt"   "SOLO LONG -- la frequenza del lato long"             776100 776101 "1" "0" @("InpAllowShort"))
$CELLE += (C "02_short" "PASSO0_FVGRET_02_short.txt"  "SOLO SHORT -- la frequenza del lato short"           776400 776401 "0" "1" @("InpAllowLong"))

# --- I MAGIC VIETATI: il magic del SORGENTE dell'EA e i blocchi dei
#     round recenti. Un'identita' non in campo resta comunque occupata.
$MagicVietati = @(775501, 776200, 776201,
...
try{
  Titolo "PASSO 0 -- FAIR VALUE GAP (ABTG_FvgRetest) -- modo $Modo"
  ...
  $fNudo = Join-Path $Prove "ABTG_FvgRetest.txt"
  if(-not (Test-Path -LiteralPath $fNudo)){
    Scarica ($RawPin + "/backtest_pipeline/prove/ABTG_FvgRetest.txt") $fNudo
  }
  ...
  $hNudo = $mappe["ABTG_FvgRetest.txt"]
  ...
  foreach($c in $Ordinati){
    ...
    # GATE DELLA BASELINE: il regime NUDO e l'ingresso al RITORNO sono
    # la baseline dichiarata del round. Se qualcuno li muove, il PASSO 0
    # sta misurando un altro motore.
    if(($h["InpRegimeMode"] -split '\|\|')[0] -ne "0"){ throw ($c.Prova + ": InpRegimeMode deve essere 0 (REG_NONE): e' la baseline dichiarata.") }
    if(($h["InpEntryMode"]  -split '\|\|')[0] -ne "0"){ throw ($c.Prova + ": InpEntryMode deve essere 0 (ENTRY_RITORNO): e' la tesi del motore.") }
  ...
  foreach($c in $Ordinati){
    ...
    # di casa vieta (punto 71, prerequisito "questo script non usa $args").
...
# =====================================================================
Titolo "RACCOLTA"
$Cart = Join-Path $Dsk ("PASSO0_FVGRET_" + $Modo + "_" + $Stamp)
New-Item -ItemType Directory -Force -Path $Cart | Out-Null

$RefTxt = New-Object System.Collections.ArrayList
[void]$RefTxt.Add("=====================================================================")
[void]$RefTxt.Add(" PASSO 0 -- FAIR VALUE GAP (ABTG_FvgRetest) su " + $Simbolo + " " + $Periodo)
[void]$RefTxt.Add("=====================================================================")
...
[void]$RefTxt.Add("  C. n enorme           -> stringere InpGapPctOfMax (la TAGLIA del")
[void]$RefTxt.Add("                           vuoto), NON il rischio e NON un filtro nuovo")
[void]$RefTxt.Add("")
...
[void]$RefTxt.Add("   MISURATO NEL SORGENTE: IngressiAmmessi() esce con")
[void]$RefTxt.Add("   'if(CountPositions()>0) return(false)' PRIMA di guardare il lato,")
[void]$RefTxt.Add("   e CercaIngresso() fa 'return' dopo la prima decisione della barra.")
[void]$RefTxt.Add("   Con un lato spento lo slot resta libero: entrano segnali che nel")
[void]$RefTxt.Add("   00_nudo erano stati buttati. E' un fatto del motore.")
[void]$RefTxt.Add("2. LA FINESTRA E' UN SOLO REGIME RIALZISTA (21 mesi di feed BCM sugli")
[void]$RefTxt.Add("   indici, criterio C6). Il lato SHORT parte svantaggiato PER REGIME,")
...
[void]$RefTxt.Add("   che resta a disco e' l'ULTIMA finestra girata.")
[void]$RefTxt.Add("   E la soglia (6,0 punti indice) va convertita: su D30EUR il")
[void]$RefTxt.Add("   rapporto punti MT5 / punti indice NON E' AGLI ATTI. [DA VERIFICARE]")
[void]$RefTxt.Add("")
if($Fatale -ne ""){
...
[void]$RefTxt.Add("")
[void]$RefTxt.Add('COME SI RIPRENDE: si riparte dalla pagina righe/RIGA_PASSO0_FVGRET_DA_MANDARE.md,')
[void]$RefTxt.Add('NON da questa riga: $p e $pin nascono dentro il blocco e non sopravvivono.')

$refPath = Join-Path $Cart "REFERTO_PASSO0_FVGRET.txt"
...
Write-Host "FILE ATTESI NELLO ZIP: REFERTO_PASSO0_FVGRET.txt + i file prova girati + i CSV IS/OOS" -ForegroundColor Gray

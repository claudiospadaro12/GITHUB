...
#
#  LA RIGA CHE SI INCOLLA sta in righe\RIGA_CRT_TICK_DIAG_DA_MANDARE.md
# =====================================================================
[CmdletBinding()]
param(
  [string]$Pin          = "",
...
$Work      = Join-Path $env:USERPROFILE "abtg_crt_tick_diag"
$Prove     = Join-Path $Work "prove"
$ProvaNome = $EA + "_TICK_DIAG.txt"
...
# i gemelli sul magic: l'UNICO asse Y. Vergini (769107/769108).
$MagicGemelli = @(769107,769108)
...
try{
  Titolo ("CRT DIAG TICK BCM -- verdetto nel toro (" + $EA + ") -- modo " + $Modo)
  ...
  Dico ("cella ....... wick 2.0, mid 0, side 2 | GATE ON ADX(D1)<=100 (ADX spento, DIAGNOSTICO) | gemelli 769107/769108")
  ...
  Dico "per-trade CSV vecchi cancellati (769107/769108, se c'erano)" "Gray"
  ...
  if($SoloControllo){
    ...
    if($trovati -gt 0){ $PerTrade_ok = "TROVATI " + $trovati + " su 2 (gemelli 769107/769108)" }
...
Titolo "RACCOLTA"
$Cart = Join-Path $Dsk ("CRT_TICK_DIAG_" + $Modo + "_" + $Stamp)
...
[void]$RefTxt.Add("cella: wick 2.0, mid 0, side 2 | GATE ON ADX(D1)<=100 (ADX spento, DIAGNOSTICO), ATR OFF (ADX-only) | gemelli 769107/769108")

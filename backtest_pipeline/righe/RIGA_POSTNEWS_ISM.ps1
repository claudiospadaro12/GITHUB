# =====================================================================
#  MARCATORE_RIGA_POSTNEWS_ISM_v1
#  RIGA_POSTNEWS_ISM.ps1 -- PASSO 0 (conta-occasioni) del preset
#  ISM1500/EURUSD di ABTG_PostNews.mq5 (v1.10). Modello 1 (1 minuto OHLC
#  = SCREENING, mai un verdetto), @PERIODO M5, UNA finestra IS/OOS, UN
#  solo asse: InpMagic 774701/774706 (2 celle gemelle, DEVONO uscire
#  identiche: e' un controllo di coerenza del banco, non una griglia).
#  Preset:  mql5\Presets\ABTG_PostNews_ISM1500_EURUSD.set
#  Prova:   prove\POSTNEWS_ISM_00_conta.txt  (@DAQUANDO OMESSO APPOSTA)
#  Calendario: mql5\Files\abtg_news_ism1500_2010_2023_UTC.csv
#              (464 righe = 1 intestazione + 463 eventi; di questi 431
#               portano l'etichetta ISM1500OK, cioe' quella del preset)
#  E' la SORELLA di RIGA_POSTNEWS_NFP.ps1 (MARCATORE_RIGA_POSTNEWS_NFP_v2):
#  stesso motore, stessa struttura, stessi gate. Cambia l'EVENTO, il
#  SIMBOLO, il calendario e i magic. Le tre differenze VERE di logica
#  rispetto alla riga NFP sono dichiarate qui sotto ai punti >>> PAVIMENTO,
#  >>> -Fino 2023.12.31 e >>> CONTA-OCCASIONI: non sono rifiniture, sono
#  scelte che cambiano i numeri e vanno lette prima di lanciare.
# ---------------------------------------------------------------------
...
#      cui BCM possiede DAVVERO le barre M1 di EURUSD, non 2010.01.01 a
...
#
#  >>> PAVIMENTO 2010.01.01 -- LA DIFFERENZA DI LOGICA PIU' IMPORTANTE
#      RISPETTO ALLA RIGA NFP, e nasce da un fatto GIA' MISURATO.
...
#      su QUESTO simbolo puo' essere una data DICHIARATA dal broker, non
#      una data OPERATIVA.
#      Se la riga la prendesse alla lettera, con -Fino 2023.12.31 e lo
#      split 0,40 l'IS andrebbe dal 1971 al 1992 circa: un intervallo in
#      cui QUESTO calendario (2010-2023) ha ZERO eventi. Il CSV IS
#      uscirebbe con Trades=0 e sembrerebbe "niente edge" mentre e'
#      "non e' girata" -- esattamente l'errore del 07/08 che questo
#      round esiste per non ripetere.
#      Percio': -DaQuando usato = IL PIU' TARDI fra la data MISURATA e
#      -PavimentoDaQuando (default 2010.01.01, cioe' il mese in cui
#      comincia il calendario: il primo evento utile e' 2010.01.04).
#      Tutti e due i numeri finiscono nel referto, e se il pavimento
#      MORDE la riga lo scrive nei RILIEVI. Se la data misurata e' PIU'
#      TARDI del pavimento, vince la misura e non cambia niente rispetto
#      alla riga NFP.
...
#      converrebbe: questa scelta RIDUCE il campione IS (189 occasioni
#      invece di 224). Non e' scelta per superare una soglia.
...
#      titolo che contiene ISM1500OK) e le divide fra IS e OOS con lo
#      STESSO split del generico. Il numero finisce nel referto accanto
#      alla colonna Trades.
#      >>> E ACCANTO CI STA SCRITTO CHE UN'OCCASIONE NON E'
#          UN'OPERAZIONE: ogni giornata utile piazza DUE pendenti e
#          quante gambe scattino non lo sa nessuno prima della passata
#          (0, 1 o 2). L'Emendamento A si applica alle OPERAZIONI
#          (colonna Trades), MAI alle occasioni. Le occasioni servono a
#          capire se il round e' anche solo GIRATO.
#      Se le occasioni IS sono ZERO la riga lo dichiara un PROBLEMA: un
#      CSV a Trades=0 su una finestra senza eventi non e' un risultato.
#
...
#      backtest).
#
#  >>> QUESTA CELLA NON PUO' PROMUOVERE NIENTE. Non perche' il campione
#      sia per forza sottile (qui, a differenza dell'NFP, le occasioni
#      IS sono 189 e le operazioni POTREBBERO superare 150), ma perche'
#      IL BANCO E' MODELLO 1 (OHLC): uno screening. In questa casa
#      l'illusione OHLC ha gia' revocato una promozione (SupRev DOW H4,
#      PF 2,77 OHLC contro 0,79 a tick reali). Il referto stampa i
#      numeri, applica l'Emendamento A alla colonna Trades (soglie di
#      casa 150/30) e NON promuove. Il giudizio di RISCHIO (Emendamento
#      B) vale a qualunque n ma su questo OPTFRAME si legge SOLO su
#      Equity DD % (l'EA non esporta un Peggior Giornata %): dichiarato,
#      non inventato.
#
...
#      ATTESO: con questo preset e questo calendario N DEVE valere 431
#      (dal 2010.01.04 al 2023.12.20). Un N diverso da 431 e' un
#      PROBLEMA: il filtro non sta selezionando quello che crediamo.
#
...
#
#  >>> I MAGIC 774701/774706 SONO STATI VERIFICATI VERGINI ORA,
#      DALL'ASSISTENTE, SUL REPO (04/09/2026): 774701 compare solo nel
#      preset e nel prova nuovi, 774706 non compariva da nessuna parte.
#      >>> 774702 NON SI USA QUI: e' RISERVATO nel preset al gemello
#          CROSS-SIMBOLO (ISM su USDJPY), che e' un altro round.
...
#      preset, 774701, sia la CELLA CHE FA DA LEAD nell'asse). Se preset
...
#      autotest ricalcolati (3 AT_Caso + 2 falliti++ = 5, NON un numero
#      scritto a mano), include censiti (Trade/Trade.mqh + il Guardian,
#      nessun terzo), hedge-safe (zero Position*(_Symbol) fuori commenti),
#      OnTester presente;
#   3. gate sul prova: @SIMBOLO EURUSD, @PERIODO M5, @DAQUANDO/@FINOA
#      ASSENTI (se compaiono ci si ferma: sarebbero una data indovinata
#      che scavalca la misura), asse InpMagic esatto (2 celle), 31 fissi
...
#      del preset combaciano uno a uno, InpMagic preset (774701) = lead
#      dell'asse; + gate sul calendario (464 righe, intestazione esatta,
#      431 righe ISM1500OK contate a macchina);
...
#   8. MISURA STORICO M1 EURUSD (automatica, SOLO barre, MAX 2 tentativi):
#      installa+compila ABTG_HistoryDownloader.mq5 nello STESSO terminale,
#      lo lancia in automatico (AllowLiveTrading=false), aspetta con
#      l'euristica di scarica_storico.ps1 (log + crescita di bases\, MAI
#      "il silenzio e' finito"), rilegge ABTG_StoricoScaricato.csv riga
#      EURUSD/M1: PrimaDataServer, poi PAVIMENTO, diventa -DaQuando. Se
#      manca ancora dopo 2 tentativi: FERMATO, e QUESTO e' il risultato;
...
#  11. collaudi: 2 righe per CSV (attese), gemelli 774701/774706
#      IDENTICI su tutte le colonne tranne Pass/InpMagic, canarino NEWS
#      letto dai log del tester (5 radici, atteso N=431), autotest "casi
...
#   - Etichetta CSV: "ISM00" (nessun materiale la fissava; segue lo stile
#     "NFP00" della riga sorella).
...
#     li fissava diversamente per questo preset.
...
#     ma nessun materiale ha mai cronometrato QUESTA misura su EURUSD:
#     stima onesta. [ATTENZIONE: R102 dichiara 10 MILIONI di barre M1
#     EURUSD sul disco di quel PC. Se sono gia' li', la misura e'
#     veloce; se vanno scaricate, 45 minuti possono non bastare e la
#     riga si ferma dicendolo -- si rilancia con -TimeoutStoricoMin
#     piu' alto, il download resta sul disco.]
...
#     referto stampa il numero, la CHIAMATA resta a chi legge (non si
#     inventa una soglia qui).
...
#   - L'ablazione news+10 contro news+15 (InpActionMin) NON e' in questa
#     riga: il prova ha UN SOLO asse (InpMagic). E' il round successivo.
#
#  QUANTO CI METTE [STIMA, non una previsione]: compilazione EA + misura
#  storico M1 EURUSD senza tick (10-45 minuti, MOLTO variabile) + 2
...
#
#  LA RIGA CHE SI INCOLLA sta in righe\RIGA_POSTNEWS_ISM_DA_MANDARE.md
# =====================================================================
[CmdletBinding()]
param(
...
$MAGIC1 = 774701; $MAGIC2 = 774706
$NCELLE_ATTESE = 2
$NEWSFILE    = "abtg_news_ism1500_2010_2023_UTC.csv"
$PRESET_FILE = "ABTG_PostNews_ISM1500_EURUSD.set"
$PROVA_FILE  = "POSTNEWS_ISM_00_conta.txt"
...
$CAL_RIGHE_ATTESE   = 464          # righe totali del file = 1 intestazione + 463 eventi
$CAL_INTESTAZIONE   = "Data Ora;Impatto;Valuta;Titolo"
$TITLE_MATCH        = "ISM1500OK"  # deve combaciare con InpNewsTitleMatch del preset
$CAL_UTILI_ATTESI   = 431          # righe che passano i filtri di QUESTO preset (= quello che l'EA stampa come "UTILI per questo preset")
$CAL_GIORNATE_ATTESE= 431          # giornate distinte (qui coincidono: un evento per giornata)
$CAL_PRIMO_ATTESO   = "2010.01.04"
$CAL_ULTIMO_ATTESO  = "2023.12.20"
...
$Dsk    = TrovaDesktop
$Work   = Join-Path $env:USERPROFILE "abtg_postnews_ism"
$Prove  = Join-Path $Work "prove"
$Sentinella = Join-Path $Work "POSTNEWS_ISM_IN_CORSO.txt"
...
$ZipNome = "POSTNEWS_ISM_" + $Modo + "_" + $Stamp
$DaQuandoMisurato = ""
$DaQuandoUsato    = ""
...
# preset ABTG_PostNews_ISM1500_EURUSD.set: fase 4 lo riverifica).
# L'ordine non conta per il gate, ma e' quello del file, per leggibilita'.
# NB: rispetto alla riga NFP cambiano OTTO chiavi -- ActionHour 15, ActionMin
# 15 (NFP 13:45), ExpiryMin 25 (NFP 59; ExpiryHour resta 16), NewsFile,
# NewsTitleMatch, CloseAtExpiry true (NFP false), RiskPercent 0.65 (NFP 1.30),
# Comment. Tutti presi 1:1 dal preset frozen.
$FissiAttesi = [ordered]@{
  "InpUsaGuardian"="true"; "InpActionHour"="15"; "InpActionMin"="15"; "InpExpiryHour"="16"; "InpExpiryMin"="25"
  ...
  "InpFridayCloseMin"="50"; "InpRiskPercent"="0.65"; "InpRiskRefSLpips"="50.0"; "InpComment"="ISM1500 PostNews"
...
# InpMagic del preset e' il LEAD dell'asse (774701).
...
function MisuraStoricoUnaVolta([string]$mq5Path,[string]$metaEditor,[string]$terminal,[string]$dataFolder,[int]$timeoutMin){
  ...
  $setFile = Join-Path $presetDir "abtg_storico_postnews_ism_m1.set"
@"
InpSimboli=$Simbolo
InpTimeframes=M1
...
"@ | Set-Content -Path $setFile -Encoding ASCII
  ...
  $ini = Join-Path $Work "gen_storico_postnews_ism.ini"
...
ScriptParameters=abtg_storico_postnews_ism_m1.set
Symbol=$Simbolo
Period=M1
...
# I GEMELLI 774701/774706: righe IDENTICHE su tutte le colonne tranne
...
try{
  Titolo ("POSTNEWS ISM1500/EURUSD -- PASSO 0 (" + $EA + ") -- modo " + $Modo)
  ...
  Dico ("pin ......... " + $Pin)
  Dico ("simbolo ..... " + $Simbolo + " (fisso: il gemello CROSS-SIMBOLO su USDJPY e' un altro round, magic 774702)")
  ...
  Remove-Item -Path (Join-Path $Prove "POSTNEWS_ISM_*.txt") -Force -ErrorAction SilentlyContinue
  ...
  if($incl -contains $INC){
    ...
    if($IncEraLi){ $IncBackup = $TIncMqh + ".prima_postnews_ism_" + $Stamp; Copy-Item -LiteralPath $TIncMqh -Destination $IncBackup -Force; $bkTxt = "backup " + $IncBackup }
  ...
  # -------------------------------------------------------------------
  #  8. MISURA STORICO M1 EURUSD (automatica, max 2 tentativi) + PAVIMENTO
  # -------------------------------------------------------------------
  ...
  if($dMis -lt $dPav){
    ...
    $PavimentoTxt = "APPLICATO: la misura ha dato " + $DaQuandoMisurato + ", PRIMA del pavimento " + $PavimentoDaQuando + " -> si usa il pavimento. Motivo (header, >>> PAVIMENTO): su questo simbolo BCM puo' DICHIARARE barre M1 molto piu' vecchie di quelle operative (R102: EURUSD 'dal 1971.01.03' con prima operazione 1999.01.18), e con lo split 0,40 l'IS finirebbe in anni dove questo calendario ha ZERO eventi."
...
[void]$R.Add("=====================================================================")
[void]$R.Add(" POSTNEWS ISM1500/EURUSD -- PASSO 0 CONTA-OCCASIONI (" + $EA + " v" + $VERSIONE_ATTESA + ")")
...
[void]$R.Add("pin:  " + $Pin)
[void]$R.Add("simbolo di questa corsa: " + $Simbolo + " (fisso; il gemello CROSS-SIMBOLO su USDJPY, magic 774702, e' un ALTRO round)")
...
[void]$R.Add("  5. 'SL/TP sono tarati': offset 3.0/2.0 e SL 25/TP 30 sono COPIATI dalla sedia NFP su USDJPY e MAI rimisurati su EURUSD. Il preset lo dichiara in testa. 25 pip su USDJPY e 25 pip su EURUSD non sono la stessa distanza.")
[void]$R.Add("  6. 'lo spread e' quello vero': a Modello 1 non lo e'. Lo spread reale BCM su EURUSD (mediana 0,100-0,200 pip in Londra, R115) e' la ragione per cui il candidato nasce qui, non una misura di questa corsa.")
[void]$R.Add("  7. niente sull'ablazione news+10 contro news+15: questo prova ha UN SOLO asse (InpMagic) e l'ora d'azione e' FISSA a 15:15.")
[void]$R.Add("")
...
[void]$R.Add("")
[void]$R.Add('COME SI RIPRENDE: dalla pagina righe/RIGA_POSTNEWS_ISM_DA_MANDARE.md, NON da questa')
...
$refPath = Join-Path $Cart "REFERTO_POSTNEWS_ISM1500_EURUSD.txt"
...
Write-Host ("FILE ATTESI NELLO ZIP: REFERTO_POSTNEWS_ISM1500_EURUSD.txt + COMPILAZIONE.log (+ _leggibile.txt) + il prova + il preset + ABTG_StoricoScaricato_M1_" + $Simbolo + ".csv + i CSV " + $EA + "_" + $Simbolo + "_IS_ohlc_" + $Etichetta + ".csv e _OOS_ (2 righe l'uno) + gli .ini veri gen_*.ini + i log del tester cresciuti") -ForegroundColor Gray

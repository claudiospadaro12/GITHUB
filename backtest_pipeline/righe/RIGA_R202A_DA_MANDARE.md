# 🚀 R202A — QUANTO SPESSO DEVE SCATTARE LA PROTEZIONE, sedia Dow `770202`

**21/09/2026** · branch `lavoro` · pin della riga: **`3d5274261904313eb43884f6af642d25cea28d63`**
File prova: `backtest_pipeline/prove/R202a_obiettivo_DOW_U30USD.txt` (80 pin, **4 celle**)
...
>
> ⚠️ **E l'asse non muove solo il parziale.** `ABTG_Dow_Apertura_US.mq5` **r.1456** (funzione `TpTotalR()`):
...
> copiarla dal Nasdaq. *(Il cancello che contiene il breakeven a TP1 sta a r.1730.)*
...
(`report/STOP_VS_SPREAD_FTMO_2026-09-20.md`: stop mediano **123,80** `[MIS n=446]`, spread mediano **2,00**):
...
(pavimento duro di casa: **13,3×** → margine **+16%**)

🟠 **Margine sottile.** La cella `0.25` si gira lo stesso — serve a vedere **il verso**
— ma **se vincesse NON si promuove senza una prova a spread maggiorato**.

⚠️ E `InpMaxSpread=0` — **il filtro spread è SPENTO**, come in campo — mentre il **massimo** misurato su `U30USD` all'ora 14 è **47,00 punti indice**, a cui quell'obiettivo vale **0,66×** lo spread.

### 🎯 L'ATTESA, DICHIARATA **PRIMA** DEI NUMERI

| # | cosa mi aspetto | e se non succede |
|---|---|---|
| **a** | la cella `1.0` **riproduce R172D** entro l'arrotondamento | se non riproduce, **il round non è confrontabile**: si cerca il pin che balla, non si legge il resto |
| **b** | scendendo, `n` **sale** (primo obiettivo più vicino = più parziali contati) | se `n` **non** sale ci sono **DUE** spiegazioni e vanno separate **prima** di scrivere il verdetto. **(1)** `InpTP1_R` non fa quello che credo. **(2)** il TP dell'ordine sta a `InpTP1_R × 3`, cioè a **sole 3 volte** la distanza del parziale: nel tester lo SL/TP del broker è valutato **prima** di `OnTick()`, quindi nella cella `0.25` un tick che salta da sotto 0,25R a sopra 0,75R chiude il **100% al TP** e il parziale **non scatta mai** — *una* uscita invece di due. Più probabile proprio qui, con `InpMaxSpread=0` e spread **massimo** 47,00 pt. 👉 **Si separano contando le POSIZIONI (`position_id`), non i deal** |
...
|---|---|
| **IS** | n **74** · PF **1,22173** · DD **5,6726%** · profitto **2.241,62** |
| **OOS** | n **130** · PF **1,27175** · DD **4,3944%** · profitto **5.395,25** |
...
`mql5/Presets/FTMO/ABTG_Dow_Apertura_US_770202_FTMO.set`). Il pin resta **1,0 apposta**: `R172D` è girato a 1,0 e
...
- Il file prova è verificato **per differenza** contro `R172D`: cambiano **DUE righe e
  basta** (`InpTP1_R` da pin ad asse, `InpBEatR` da asse a pin `0.0`); direttive `@`
  **identiche**, **80 pin** prima e dopo.

...
### ⚓ LE ANCORE (da R172D, stessa macchina, stasera)
...
- `python3 backtest_pipeline/controlla_prova.py backtest_pipeline/prove/R202a_obiettivo_DOW_U30USD.txt` → **verde** (4 celle, 0 problemi).
- `python3 backtest_pipeline/controlla_riga.py --oggetto md backtest_pipeline/righe/RIGA_R202A_DA_MANDARE.md` → **verde**.
...
- **Il file prova è verificato per differenza**: costruito dai pin di `R172D`, cambiano
  **DUE righe e basta** (`InpTP1_R` da pin ad asse, `InpBEatR` da asse a pin `0.0`); le
  direttive `@` sono **identiche** alla sorgente e i pin sono **80** prima e dopo.

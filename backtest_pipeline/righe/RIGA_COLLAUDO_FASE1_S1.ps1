...
#  fase 1 (criterio 7 = cap C1, criterio 8 = fail-open), pacchetto
#  report/COLLAUDO_ENFORCEMENT_FASE1_2026-09-02.md, azione G2.
#
...
#  nessun grafico, nessun EA, nessun ordine. Legge i log (con
...
#                                 righe VIETATE, traccia delle soglie,
#                                 e scrive il SEGNAPOSTO d'inizio sessione
...
$cart    = Join-Path $desktop ("COLLAUDO_FASE1_S1_" + $stamp)
$zip     = Join-Path $desktop ("COLLAUDO_FASE1_S1_" + $stamp + ".zip")
$segnPath= Join-Path $desktop "COLLAUDO_FASE1_S1_SEGNAPOSTO.txt"
try { New-Item -ItemType Directory -Force -Path $cart | Out-Null } catch { }

Riga "===============================================================" "Cyan"
Riga ("COLLAUDO ENFORCEMENT FASE 1 -- SESSIONE 1 (cap C1 + fail-open)") "Cyan"
...
if (-not $DataFolder) {
  ...
  try {
    ...
    if ($ATTESE.Count -lt 10) {
      ...
      $P_artefatto = "OK (" + $nA + " ATTESA, " + $nV + " VIETATA, " + $nC + " CAMPO, dal pin " + $Pin + ")"
      Riga ("ARTEFATTO dal pin : " + $nA + " ATTESA, " + $nV + " VIETATA, " + $nC + " CAMPO") "Green"
    }
  } catch {
  ...
  if ($P_modo -eq "LETTURA PRIMA") {
    try {
      @("inizio sessione 1 (ora LOCALE VPS): " + $Avvio.ToString("yyyy-MM-dd HH:mm:ss", $INV)) |
  ...
  #     (e' il diario a macchina dei gesti di Claudio sul cap:
  #      ogni cambio di parametri riavvia il Guardian e ristampa
  #      la riga delle soglie col valore NUOVO)
  # ---------------------------------------------------------------
  Riga ""
  ...
  foreach ($r in $RIGHE) {
    ...
    if (Contiene $r "pausa morbida=") {
      ...
      if ($m.Success) {
        ...
        $ultimoCap = $cap
        $nota = ""
        if ($cap -eq 3.25 -and $pausa -eq 4.00) { $nota = "  <-- configurazione FIRMATA (4,0 / 3,25)" }
        elseif ($cap -lt 3.25)                  { $nota = "  <-- CAP ABBASSATO per la prova" }
...
$refPath = Join-Path $cart "RIGA_REFERTO_COLLAUDO_FASE1_S1.txt"
try {

# =====================================================================
#  MARCATORE_RIGA_R116BIS_LONDONFX_v1
#  RIGA_R116BIS_LONDONFX.ps1 -- R116-BIS: LA RIPETIZIONE SU BANCO PULITO
#  della gamba che il 03/09 e' uscita SPORCA. Stesso EA, stessi criteri,
#  stessi cancelli, stessi prova: cambia SOLO IL BANCO (un agente solo).
#  (Modello 4) su M15, EURUSD + GBPUSD, UN simbolo per giro (-Simbolo).
# ---------------------------------------------------------------------
#  PERCHE' ESISTE, e perche' NON e' una seconda griglia su un motore
#  morto (regola della seconda caccia, 19/08):
#   - il 03/09 la corsa GBPUSD ha chiuso con PROBLEMI 2 -- "GEMELLI
#     DIVERGONO sul motore 3 (774001 contro 774002)" su IS e su OOS.
#     Per la sanita' 5.0.1 dei criteri quel banco e' SPORCO e quei
#     numeri NON si leggono. Referto archiviato in
#     risultati_archivio\r116_londonfx\ (CORSA_GBPUSD_..._BANCO_SPORCO);
#   - il 04/09 la causa e' stata CONFERMATA, non sospettata: CLASSE 129
#     della checklist -- piu' agenti locali MT5 vivi insieme fanno
#     divergere davvero le celle che dovrebbero uscire identiche (5
#     corse su RIGA_POSTNEWS_ISM, 5 divergenze DIVERSE; con Core 1 solo,
#     gemelli identici alla prima corsa);
#   - qui NON si tocca un parametro del motore e NON si rilegge un
#     verdetto: si RIFA' LA MISURA sul banco che la sanita' pretende.
#     Se i numeri tornano come il 03/09, quella bocciatura diventa
#     leggibile; se cambiano, il 03/09 non era una misura, e si dice.
#  LA GAMBA EURUSD DEL 03/09 NON VA RIFATTA PER OBBLIGO: aveva PROBLEMI 0
#  e gemelli IDENTICI su 3 coppie, cioe' il gate di identita' -- che e'
#  la prova di casa che il banco era pulito -- E' PASSATO. Si puo' rifare
#  come CONFERMA (blocco facoltativo della pagina), dichiarandolo cosi'.
...
#
#  QUANTO CI METTE -- adesso c'e' un numero MISURATO, non una stima: il
#  03/09 la gamba EURUSD ha fatto compilazione + 12 passate a tick reali
#  in circa 70 secondi (avvio 17:51:17, CSV OOS scritto alle 17:52:26) e
#  la GBPUSD in circa 2,5 minuti -- ma CON PIU' AGENTI.
#  [INFERITO, ed e' proprio quello che questo giro misura]: a UN SOLO
#  agente il tempo sale piu' o meno quanti agenti si spengono, quindi
#  resta nell'ordine dei MINUTI, non delle ore. Il numero vero di questo
#  banco lo dice questo giro, e va scritto nel referto.
#
#  LA RIGA CHE SI INCOLLA sta in righe\RIGA_R116BIS_LONDONFX_DA_MANDARE.md
# =====================================================================
[CmdletBinding()]
param(
...
# Etichetta R116B_: entra nel NOME dei CSV, e cambia rispetto al 03/09
# APPOSTA -- i CSV del giro sporco restano dove sono e NON possono
# essere riletti per sbaglio al posto di questi (ne' viceversa).
$Etichetta   = "R116B_" + $Prefisso
$EtichettaF2 = "R116B_" + $Prefisso + "_SLIP"
...
function GateGemelli($righe,[string]$et){
  ...
  foreach($m in @(1,2,3)){
    ...
    if($diff.Count -gt 0){ $ok = $false; [void]$Problemi.Add($et + ": GEMELLI DIVERGONO sul motore " + $m + " (" + $MAGIC1 + " contro " + $MAGIC2 + "): " + ($diff -join ", ") + ". Banco sporco: il round e' FERMO (sanita' 5.0.1). PRIMA CAUSA DA ESCLUDERE: classe 129, piu' agenti locali vivi -- si legge la riga 'agenti locali durante la corsa' del referto PRIMA di dare la colpa all'EA.") }
  }
...
# ---------------------------------------------------------------------
#  IL MONITOR DEGLI AGENTI -- CLASSE 129, la ragione di questo bis
# ---------------------------------------------------------------------
#  La classe 129 dice due cose. La prima: con piu' agenti locali vivi le
#  celle gemelle DIVERGONO davvero (causa confermata il 04/09, non
#  indiziata). La seconda: nessuno script puo' SPEGNERE gli agenti da
#  fuori MT5 -- e' un pannello del Tester, e resta un passo MANUALE.
#  Quello che si puo' fare, e che qui si fa, e' MISURARLI: un job in
#  parallelo conta ogni 400 ms i processi metatester64 vivi e tiene il
#  MASSIMO contemporaneo. E' lo stesso strumento con cui la causa e'
#  stata confermata (Get-Process metatester64 in loop ogni 300 ms).
#  Un gate che non legge niente non e' un gate verde: se il massimo e'
#  maggiore di 1 il referto lo scrive nei PROBLEMI, e l'identita' dei
#  gemelli non prova piu' niente -- nemmeno quando esce verde.
#  LIMITE DICHIARATO: questo e' un MISURATORE, non un interruttore. Non
#  puo' impedire la corsa sbagliata, puo' solo impedire che venga letta
#  come buona. Spegnere gli agenti resta un gesto di Claudio.
$MonFile          = Join-Path $Work "agenti_max.txt"
$script:MonJob    = $null
$script:MonAttivo = $false
$script:AgentiMax = -1      # -1 = MAI MISURATO (diverso da 0 = misurato, visto zero)
$AgentiTxt        = "NON MISURATO"

function MonitorAvvia(){
  Remove-Item -LiteralPath $MonFile -Force -ErrorAction SilentlyContinue
  $script:MonJob    = $null
  $script:MonAttivo = $false
  try{
    $script:MonJob = Start-Job -ScriptBlock {
      param($f)
      $max = 0
      while($true){
        $n = @(Get-Process metatester64 -ErrorAction SilentlyContinue).Count
        if($n -gt $max){ $max = $n; Set-Content -LiteralPath $f -Value ("" + $max) -Encoding ASCII }
        Start-Sleep -Milliseconds 400
      }
    } -ArgumentList $MonFile
    $script:MonAttivo = ($null -ne $script:MonJob)
  }catch{ $script:MonJob = $null; $script:MonAttivo = $false }
}

function MonitorFerma(){
  if($null -ne $script:MonJob){
    try{ Stop-Job   -Job $script:MonJob -ErrorAction SilentlyContinue }catch{}
    try{ Remove-Job -Job $script:MonJob -Force -ErrorAction SilentlyContinue }catch{}
    $script:MonJob = $null
  }
  if($script:MonAttivo){
    # il monitor E' partito: da qui in poi lo 0 e' una MISURA, non un buco
    $letto = 0
    if(Test-Path -LiteralPath $MonFile){
      $righeM = @(Get-Content -LiteralPath $MonFile -ErrorAction SilentlyContinue)
      if(@($righeM).Count -ge 1){
        $vM = 0
        if([int]::TryParse((("" + $righeM[0]).Trim()), [ref]$vM)){ $letto = $vM }
      }
    }
    if($letto -gt $script:AgentiMax){ $script:AgentiMax = $letto }
  }
  $script:MonAttivo = $false
}
...
function LanciaGenerico([string]$drv,[string]$prova,[string]$et){
  ...
  MonitorAvvia                     # classe 129: gli agenti si contano MENTRE gira, non dopo
  $global:LASTEXITCODE = $null
  & powershell $argv
  $rc = $LASTEXITCODE              # letto PRIMA di fermare il monitor: nessun cmdlet in mezzo
  MonitorFerma
...
try{
  ...
  Dico ("banco ....... MODELLO 4 (TICK REALI) | Spread=0 dichiarato | deposito " + $Deposito + " | rischio 0,65% | ora 8 SERVER = Londra 08:00-16:00 | UN SOLO AGENTE LOCALE (classe 129, causa CONFERMATA il 04/09: con piu' agenti i gemelli divergono davvero). Il monitor conta i metatester64 vivi: se ne vede piu' di uno, il round e' FERMO e il referto lo scrive nei PROBLEMI.") "Yellow"
...
[void]$R.Add("compilazione: " + $Compilato + "   <- l'EA e' gia' stato compilato PULITO il 03/09 (Result: 0 errors, 0 warnings, 1212 ms). Qui si RICOMPILA allo stesso pin: se adesso FALLISCE non e' un esito atteso, e' un fatto NUOVO da guardare (pin diverso? include diverso? MetaEditor diverso?)")
...
# --- CLASSE 129: quanti agenti erano vivi mentre giravano i gemelli ---
if($AgentiMax -lt 0){
  $AgentiTxt = "NON MISURATO (il monitor non e' partito -- Start-Job non disponibile -- oppure il giro si e' fermato prima di lanciare il generico). Il banco di questa corsa NON e' dichiarato: l'identita' dei gemelli si legge con la riserva della classe 129."
  if($Modo -ne "CONTROLLO"){ [void]$Rilievi.Add("MONITOR DEGLI AGENTI NON ATTIVO: non so quanti metatester64 fossero vivi. Se i gemelli divergono, la prima causa da escludere resta la classe 129 (piu' agenti locali), non l'EA.") }
} elseif($AgentiMax -eq 0){
  $AgentiTxt = "0 processi metatester64 visti (campionamento ogni 400 ms). ATTESO nel giro a vuoto (MT5 non gira). In una CORSA vera vuol dire cache del tester oppure campionamento troppo rado: NON e' la prova che il banco fosse a un agente."
  if($Modo -ne "CONTROLLO"){ [void]$Rilievi.Add("agenti: 0 processi metatester64 visti durante una corsa vera. Non e' una prova di banco pulito, e' una misura mancata: i gemelli si leggono con la riserva della classe 129.") }
} elseif($AgentiMax -eq 1){
  $AgentiTxt = "1 (massimo contemporaneo, campionato ogni 400 ms) -- BANCO A UN AGENTE, come la classe 129 pretende da una riga con l'asse GEMELLO. E' la condizione in cui l'identita' dei gemelli vuol dire qualcosa."
} else {
  $AgentiTxt = "" + $AgentiMax + " processi metatester64 VIVI INSIEME (massimo contemporaneo, campionato ogni 400 ms) -- CLASSE 129: il banco NON e' a un agente."
  [void]$Problemi.Add("AGENTI PARALLELI (classe 129): visti fino a " + $AgentiMax + " processi metatester64 contemporanei. Questa riga ha un asse GEMELLO (due celle che DEVONO uscire identiche): con piu' agenti la divergenza e' ATTESA e l'identita' non prova niente, nemmeno quando esce verde. Il round e' FERMO: Strategy Tester -> scheda Agenti -> lasciare acceso il solo Core 1, poi rilanciare la stessa riga.")
}
[void]$R.Add("agenti locali durante la corsa (classe 129, il motivo di questo bis): " + $AgentiTxt)
...
[void]$R.Add("")
[void]$R.Add('COME SI RIPRENDE: dalla pagina righe/RIGA_R116BIS_LONDONFX_DA_MANDARE.md, NON da questa')
...
$refPath = Join-Path $Cart ("REFERTO_R116BIS_LONDONFX_" + $Simbolo + ".txt")

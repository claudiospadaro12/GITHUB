...
#  ABTG_SondaRelativo e' un CONTATORE: NESSUN ordine, nessun lotto,
#  nessun magic, nessuna sedia. Conta gli attraversamenti dello
#  z-score del RAPPORTO fra DUE simboli (gamba che si scambia / metro
#  U30USD che si legge) e misura MFE/MAE/tenuta/convergenza di
#  posizioni FINTE. UNA CORSA PER INVOCAZIONE, quattro prova ammessi:
#    -Prova D30_M5    D30EUR M5    prove\RELATIVO_D30_M5.txt
#    -Prova D30_M15   D30EUR M15   prove\RELATIVO_D30_M15.txt
#    -Prova NAS_M5    NASUSD M5    prove\RELATIVO_NAS_M5.txt
#    -Prova NAS_M15   NASUSD M15   prove\RELATIVO_NAS_M15.txt
#  finestra 2024.09.26 -> 2026.06.30 (dichiarata nei prova e gattata),
#  MODELLO 2 ("Solo prezzi di apertura": il segnale nasce su barra
#  chiusa e non si apre niente), 2 assi x 7 valori = 49 passate.
# ---------------------------------------------------------------------
#  LE SCELTE, DICHIARATE (perche' NON erano fissate):
#  - UNA CORSA PER INVOCAZIONE, non quattro in sequenza: il CSV che la
#    sonda scrive si chiama OptResults_<EA>_<SIMBOLO>.csv SENZA il TF
#    nel nome (letto nel sorgente, OptFrame_FileName), quindi D30_M5 e
#    D30_M15 scriverebbero lo stesso file; il generico lo sposta dopo
#    ogni gamba, ma quattro corse da 49 passate in un giro solo sono
#    ore di macchina senza un artefatto intermedio. Quattro blocchi,
#    quattro zip, quattro referti: se una corsa muore, le altre tre
#    esistono.
#  - I QUATTRO PROVA SI SCARICANO TUTTI, ANCHE SE NE GIRA UNO: il
#    prova promette che i quattro sono IDENTICI carattere per carattere
#    nel blocco dei parametri (differiscono SOLO per @SIMBOLO e
#    @PERIODO) e che "il diff si verifica meccanicamente". Qui si
#    verifica: una terza differenza FERMA tutto.
#  - NESSUN -Simbolo / -Periodo passato al generico: li legge dai
#    prova (@SIMBOLO/@PERIODO), come i prova stessi prescrivono ("un
#    valore overridato da fuori sarebbe uno stato nascosto"). Il driver
#    li GATTA contro l'atteso del -Prova scelto.
#  - FrazioneIS 1.0 (UNA TRANCHE): la sonda non seleziona celle, non
#    esiste un OOS da tenere in cassaforte. La gamba "OOS" del generico
#    e' DEGENERE (0 giorni): il rosso del generico su *_OOS e' ATTESO.
#  - Deposito 100000: INERTE (zero ordini), il generico lo vuole.
#  - LE SOGLIE DI LETTURA NON SONO SCRITTE QUI: si LEGGONO dai #define
#    del sorgente scaricato al pin (REL_C1_..., REL_C3_..., REL_SPREAD_
#    ..., REL_C5_..., REL_C6_..., REL_C8_...) e si CONFRONTANO con i
#    numeri che la pagina promette (8,40 DAX / 5,40 NAS, 2,00/gg,
#    RR 0,70, 25/40 %, 12 barre). Se pagina e sorgente divergono, e' un
#    PROBLEMA: il criterio si cambia prima dei numeri, non dopo.
#  - ALTOPIANO, NON PICCO (regola di casa, Emendamento A): il verdetto
#    per lato NON e' "esiste una cella che passa". E' "esiste un
#    blocco 2x2 di celle CONTIGUE nella griglia (N, sigma) in cui
#    C1, C3, C5, C6 e C8 stanno in piedi INSIEME". Una cella isolata =
#    RUMORE = NO. Il criterio e' scritto qui e nella pagina PRIMA di
#    vedere un numero.
#
#  >>> IL TETTO DELLE ~100.000 BARRE DEL TESTER (CLAUDE.md, 25/08:
#      M5 ~1,3 anni, M15 ~4 anni per corsa). I due prova M5 chiedono
#      21 MESI (2024.09.26 -> 2026.06.30 = ~1,76 anni) e lo DICHIARANO
#      ("possono ECCEDERE il tetto, NON si spezza in tranche, la sonda
#      dichiara da sola la finestra effettiva"). Questa riga NON
#      corregge il prova in silenzio e NON parte in silenzio: sui M5
#      SI FERMA con un messaggio, e riparte solo con l'interruttore
#      esplicito -AccettoTettoBarre, che finisce nel referto come
#      scelta DICHIARATA. Dopo la corsa il referto stampa la finestra
#      EFFETTIVA (Gamba Prima Barra Epoch, Giorni Contati) e cerca la
#      firma del tetto (classe 36: Barre Valutate IDENTICHE sul
#      simbolo gemello dello stesso TF, se quel CSV esiste in workdir).
#      L'alternativa che rispetta il tetto e' un @DAQUANDO piu' vicino
#      (~2025.03.10 per 1,3 anni): NON e' stata applicata, e' scritta
#      nella pagina come opzione (b) per Claudio.
#
#  >>> EA MAI COMPILATO: si compila QUI (metaeditor64 con invocazione
#      DIRETTA, log letto qualunque sia la codifica, riga "Result: N
#      errors" ricopiata nel referto). Se fallisce, QUELLO e' il
#      risultato del passo. Nessun #include: la sonda e' autosufficiente
#      per scelta, e il driver lo verifica (zero righe #include).
...
#      che MT5 si ricorda dall'ultima griglia).
#  >>> SENTINELLA + FOTO (classe 116): prima di scrivere nel terminale
#      (MQL5\Experts\<EA>.mq5/.ex5) si scrive <work>\SONDARELATIVO_IN_
#      CORSO.txt con l'elenco dei file toccati. La raccolta gira SEMPRE
#      (anche dopo un gate, anche dopo la compilazione fallita), la
#      foto PRIMA/DOPO dei file del terminale sta nel referto, e i
#      residui si DICHIARANO (i due file .mq5/.ex5 della sonda restano
#      nel terminale: sono un contatore senza ordini, e il generico li
#      riscrive comunque a ogni corsa). Se al giro dopo la sentinella
#      c'e' ancora, il giro precedente e' stato interrotto e si dice.
#  >>> EXIT CODE A TRE STATI (classe 108): letto / non-zero / NON
#      LETTO. Il verdetto sta sugli ARTEFATTI DATATI (CSV fresco e
#      contato), mai sul numero.
#  >>> CSV DATATO PRIMA DI LETTERLO (classe CSV stantio): la workdir
#      e' riusabile, -Rifai sta SEMPRE nell'argv, e un CSV piu' vecchio
#      dell'avvio della corsa NON si legge.
#  >>> IL TIMBRO data: DEL REFERTO E' L'ORA DI AVVIO (classe 110): la
#      corsa M5 dura decine di minuti, l'ora attuale NON e' il metro.
#
#  QUANTO CI METTE [STIMA, non una previsione]: 49 passate open-prices
#  su ~21 mesi (due simboli letti a ogni barra) + 2 avvii del terminale
#  + 1 compilazione. M5: 15-45 minuti a prova. M15: 8-25 minuti.
#  Il giro a vuoto e' questione di 1-3 minuti (ma COMPILA).
#
#  LA RIGA CHE SI INCOLLA sta in righe\RIGA_SONDARELATIVO_DA_MANDARE.md
# =====================================================================
[CmdletBinding()]
param(
  ...
  [string]$Pin              = "",
  # -Prova: UNO dei quattro nomi ammessi. Obbligatorio.
  [ValidateSet("D30_M5","D30_M15","NAS_M5","NAS_M15")]
  [string]$Prova            = "",
...
$Dsk     = TrovaDesktop
$Work    = Join-Path $env:USERPROFILE "abtg_sondarelativo"
...
# --- LE QUATTRO CORSE AMMESSE: etichetta -> simbolo, TF, file prova.
$CORSE = [ordered]@{
  "D30_M5"  = @{ Simbolo="D30EUR"; Periodo="M5";  File="RELATIVO_D30_M5.txt";  SpreadPagina=$PAGINA_SPREAD_D30 }
  "D30_M15" = @{ Simbolo="D30EUR"; Periodo="M15"; File="RELATIVO_D30_M15.txt"; SpreadPagina=$PAGINA_SPREAD_D30 }
  "NAS_M5"  = @{ Simbolo="NASUSD"; Periodo="M5";  File="RELATIVO_NAS_M5.txt";  SpreadPagina=$PAGINA_SPREAD_NAS }
  "NAS_M15" = @{ Simbolo="NASUSD"; Periodo="M15"; File="RELATIVO_NAS_M15.txt"; SpreadPagina=$PAGINA_SPREAD_NAS }
}
...
$AssiAttesi = [ordered]@{ "InpFinestraN"="20||10||5||40||Y"; "InpSogliaIngressoSigma"="1.05||0.75||0.15||1.65||Y" }
...
$Righe49    = $null      # tutte le righe lette
...
function GateGemelli($letture){
  ...
  return "VALIDO: i 4 prova hanno il blocco dei parametri IDENTICO riga per riga e differiscono SOLO per @SIMBOLO e @PERIODO"
}
...
function AnalizzaCsv([string]$csvIS,[datetime]$tC,$sg){
  ...
  $altroSim = "NASUSD"; $altraEt = "NAS_" + $Periodo
  if($Simbolo -eq "NASUSD"){ $altroSim = "D30EUR"; $altraEt = "D30_" + $Periodo }
...
try{
  ...
  Dico ("banco ....... MODELLO 2 (SOLO PREZZI DI APERTURA), 49 passate (2 assi x 7). Deposito " + $Deposito + " (inerte: zero ordini)")
  ...
  Dico ("file prova scaricati: " + @(Get-ChildItem $Prove -Filter "RELATIVO_*.txt").Count + " su 4 (ne gira UNO, gli altri tre servono al gemellaggio)") "Green"
  ...
  # -------------------------------------------------------------------
  Titolo "3. GATE SUI QUATTRO PROVA (direttive nude + input nei due versi + 2 assi + 49 celle + fissi + gemellaggio)"
  $letture = [ordered]@{}
  foreach($k in @($CORSE.Keys)){
    ...
    if($k -eq $Prova){ $CelleTxt = "" + $esito.Celle + " (2 assi: InpFinestraN 10..40 passo 5 x InpSogliaIngressoSigma 0,75..1,65 passo 0,15), ricontate dai pin ||Y al pin" }
  }
  ...
  Set-Content -LiteralPath $Sentinella -Value @(("scritto il " + (Get-Date).ToString("yyyy-MM-dd HH:mm:ss",$INV) + " da RIGA_SONDARELATIVO.ps1 (" + $Prova + ")"), $dstMq5, $dstEx5, $dstCsv) -Encoding ASCII

...
try{
  ...
  [void]$RefTxt.Add("  ATTENZIONE AI SEGNI: ISdd/OOSdd sono POSITIVI (piu' basso = meglio), le due")
  [void]$RefTxt.Add("  colonne PeggGio sono NEGATIVE perche' sono PERDITE (piu' vicino a zero =")
  [void]$RefTxt.Add("  meno profonda: -0,37 e' PEGGIO di -0,31). Le due colonne non si leggono con")
  [void]$RefTxt.Add("  lo stesso verso, ed e' il criterio (c) del par. 6 a dipenderne.")
  ...
  [void]$RefTxt.Add("  in OOS: (a) profitto > metro; (b) DD non peggiore del metro; (c) peggior")
  [void]$RefTxt.Add("  giornata NON PIU' PROFONDA di quella del metro (denominatore FISSO; con un")
  [void]$RefTxt.Add("  n/d il criterio (c) e' NON VALUTABILE e la candidatura resta SOSPESA, mai")
  [void]$RefTxt.Add("  promossa); (d) profitto IS > 0. Fra i dial che passano vince il PIU' BASSO")
  [void]$RefTxt.Add("  (D2-bis). Vietato aggiungere dial a risultati visti (D5).")
  [void]$RefTxt.Add("  >>> ATTENZIONE AL SEGNO -- e' il punto in cui questo referto si leggerebbe")
  [void]$RefTxt.Add("      ROVESCIATO. (b) e' un numero POSITIVO (un DD di 7,83 e' PEGGIO di 2,66:")
  [void]$RefTxt.Add("      li' il '<=' del par. 6 si applica alle cifre come stanno). (c) e' un")
  [void]$RefTxt.Add("      numero NEGATIVO, perche' e' una PERDITA: -0,37% e' PEGGIO di -0,31%. Il")
  [void]$RefTxt.Add("      '<=' del par. 6 e' scritto sulla PROFONDITA' della perdita, non sul numero")
  [void]$RefTxt.Add("      con segno: applicato alle cifre COSI' COME SONO STAMPATE darebbe il")
  [void]$RefTxt.Add("      criterio (c) per SODDISFATTO proprio nel caso in cui la cella e' PEGGIO")
  [void]$RefTxt.Add("      del metro. Per (c) si legge: passa se il numero della cella e' PIU' VICINO")
  [void]$RefTxt.Add("      A ZERO di quello del metro.")
  ...
  foreach($cellaRef in @($Ordinati | Where-Object { -not $_.Metro })){
    ...
    [void]$RefTxt.Add("     (a) profitto OOS : " + (FmtE $cellaRef.ProfOOS) + "  contro metro " + (FmtE $rifMetro.ProfOOS) + "   [piu' alto = meglio]")
    [void]$RefTxt.Add("     (b) DD OOS       : " + (Fmt2 $cellaRef.DdOOS) + "%  contro metro " + (Fmt2 $rifMetro.DdOOS) + "%   [numeri POSITIVI: piu' basso = meglio]")
    [void]$RefTxt.Add("     (c) pegg.giornata: " + (FmtPg $cellaRef.PgPctFisso) + "% (fisso)  contro metro " + (FmtPg $rifMetro.PgPctFisso) + "%   [numeri NEGATIVI: piu' VICINO A ZERO = meglio. NON confrontare col '<=' le cifre col segno: lo rovescia]")
    [void]$RefTxt.Add("     (d) profitto IS  : " + (FmtE $cellaRef.ProfIS) + "   [deve essere > 0]")

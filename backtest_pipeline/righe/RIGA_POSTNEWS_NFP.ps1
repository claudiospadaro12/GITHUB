...
#  solo asse: InpMagic 771203/771208 (2 celle gemelle, DEVONO uscire
#  identiche: e' un controllo di coerenza, non una griglia).
#  Preset:  mql5\Presets\ABTG_PostNews_NFP_USDJPY.set
#  Prova:   prove\POSTNEWS_NFP_00_conta.txt  (@DAQUANDO OMESSO APPOSTA)
#  Calendario: mql5\Files\abtg_news_postnews_2010_2025_UTC.csv (599 eventi)
# ---------------------------------------------------------------------
...
#      dati risolti alla fase 4 -- non chiama scarica_storico.ps1 come
...
#      backtest).
#
#  >>> QUESTA CELLA NON PUO' PROMUOVERE NIENTE (dichiarato nel prova
#      stesso, "ATTESA: NO"): 12 eventi/anno x 2 gambe non arrivano a
#      150 operazioni IS nemmeno prendendo tutto lo storico. Il referto
#      stampa i numeri, applica SOLO l'Emendamento A (unita' = OPERAZIONE,
#      soglie di casa 150/30, MAI una promozione da qui) e segnala che
#      il giudizio di RISCHIO (Emendamento B) su questo OPTFRAME puo'
#      leggersi SOLO su Equity DD % (l'EA non esporta un Peggior
#      Giornata %): dichiarato, non inventato.
#
...
#      mai un warning sepolto in fondo.
#
...
#
#  >>> IL BLOCCO MAGIC 771203/771208 E' STATO VERIFICATO VERGINE ORA,
#      DALL'ASSISTENTE, SUL REPO (03/09/2026): nessun'altra occorrenza
#      in mql5/ fuori dal preset e dal prova nuovi. La riga NON puo'
#      riverificarlo a runtime: il PC di backtest non ha il repo
#      clonato (come tutte le righe di questa casa).
#
...
#      derivato a mano, e il gate di fase 6 verifica che i 31 valori
#      fissi combacino UNO A UNO fra prova e preset (+ che InpMagic del
#      preset, 771203, sia la CELLA CHE FA DA LEAD nell'asse). Se preset
...
#      del preset combaciano uno a uno, InpMagic preset (771203) = lead
#      dell'asse;
...
#      USDJPY/M1: PrimaDataServer diventa -DaQuando. Se manca ancora dopo
#      2 tentativi: FERMATO, e QUESTO e' il risultato del passo;
...
#      IS/OOS, -DaQuando MISURATO, -Modello 1, -Rifai;
#  11. collaudi: 2 righe per CSV (attese), gemelli 771203/771208
#      IDENTICI su tutte le colonne tranne Pass/InpMagic, canarino NEWS
#      letto dai log del tester (5 radici), autotest "casi falliti" letto
#      dai log (atteso 0 su ogni riga trovata);
...
#   - Etichetta CSV: "NFP00" (quella suggerita nel prova stesso).
#   - Deposito 100000, FrazioneIS 0.40, -Fino 2026.06.30: default di casa,
#     nessun materiale li fissava diversamente per questo preset.
#   - -DaQuandoRichiesta 2010.01.01: il punto da cui CHIEDE lo storico
#     al broker (il calendario comincia 2010.01.08); il valore VERO che
#     finisce nel test e' quello MISURATO (PrimaDataServer), quasi
...
#     ma nessun materiale ha mai cronometrato QUESTA misura: stima onesta.
...
#
#  QUANTO CI METTE [STIMA, non una previsione]: compilazione EA (nuovo,
#  mai compilato) + misura storico M1 USDJPY senza tick (10-45 minuti,
#  MOLTO variabile: dipende da quanti anni servono davvero) + 2 passate
#  OHLC M1 su una finestola forex pluriennale (secondi-minuti, non tick
#  reali) + 2-3 avvii del terminale. Totale onesto: 20-60 minuti, quasi
#  tutti nella misura storico. Nessuna corsa di casa ha mai cronometrato
#  QUESTA misura per USDJPY: il numero vero lo dice questo giro.
#
#  LA RIGA CHE SI INCOLLA sta in righe\RIGA_POSTNEWS_NFP_DA_MANDARE.md
# =====================================================================
[CmdletBinding()]
param(
  ...
  [switch]$SaltaMisuraStorico,             # SOLO PER RIPETIZIONE nella STESSA sessione di lavoro (classe 116-quater sotto): NON salta la prima volta
...
$MAGIC1 = 771203; $MAGIC2 = 771208
$NCELLE_ATTESE = 2
$NEWSFILE    = "abtg_news_postnews_2010_2025_UTC.csv"
$PRESET_FILE = "ABTG_PostNews_NFP_USDJPY.set"
$PROVA_FILE  = "POSTNEWS_NFP_00_conta.txt"
$HISTDL      = "ABTG_HistoryDownloader.mq5"
# --- casi dell'autotest: NON un #define nel sorgente (a differenza di
#     ABTG_LondonFx). Si RICALCOLANO dal file appena scaricato al pin
#     (fase 2): questi due numeri sono l'atteso di OGGI (v1.10), non un
#     valore che il gate si fida ciecamente.
...
$Dsk    = TrovaDesktop
$Work   = Join-Path $env:USERPROFILE "abtg_postnews_nfp"
$Prove  = Join-Path $Work "prove"
$Sentinella = Join-Path $Work "POSTNEWS_IN_CORSO.txt"
...
$ZipNome = "POSTNEWS_NFP_" + $Modo + "_" + $Stamp
$DaQuandoMisurato = ""
...
# preset .set: fase 4 lo riverifica). L'ordine non conta per il gate,
# ma e' quello del file, per leggibilita'.
$FissiAttesi = [ordered]@{
  "InpUsaGuardian"="true"; "InpActionHour"="13"; "InpActionMin"="45"; "InpExpiryHour"="16"; "InpExpiryMin"="59"
  "InpRestrictToNews"="true"; "InpUseNewsFilter"="true"; "InpNewsFile"=$NEWSFILE; "InpNewsCommon"="true"
  "InpNewsMinImpact"="3"; "InpNewsCurrencies"="USD"; "InpNewsTitleMatch"="Unemployment Rate"
  "InpNewsShiftMinutes"="0"; "InpBuyOffsetPips"="3.0"; "InpSellOffsetPips"="2.0"; "InpTPpips"="30.0"
  "InpSLpips"="25.0"; "InpUseOCO"="false"; "InpCloseAtExpiry"="false"; "InpUseTrail25"="false"
  "InpTrailTriggerPips"="25.0"; "InpTrailNewSLpips"="15.0"; "InpFridayClose"="true"; "InpFridayCloseHour"="21"
  "InpFridayCloseMin"="50"; "InpRiskPercent"="1.30"; "InpRiskRefSLpips"="50.0"; "InpComment"="NFP PostNews"
...
# InpMagic del preset e' il LEAD dell'asse (771203).
...
function MisuraStoricoUnaVolta([string]$mq5Path,[string]$metaEditor,[string]$terminal,[string]$dataFolder,[int]$timeoutMin){
  ...
  $setFile = Join-Path $presetDir "abtg_storico_postnews_m1.set"
@"
InpSimboli=$Simbolo
InpTimeframes=M1
...
"@ | Set-Content -Path $setFile -Encoding ASCII
  ...
  $ini = Join-Path $Work "gen_storico_postnews.ini"
...
ScriptParameters=abtg_storico_postnews_m1.set
Symbol=$Simbolo
Period=M1
...
# =====================================================================
#  6 SEZIONI del referto del canarino NEWS (letto dai LOG, non dal CSV).
# =====================================================================
...
# I GEMELLI 771203/771208: righe IDENTICHE su tutte le colonne tranne
...
try{
  Titolo ("POSTNEWS NFP/USDJPY -- PASSO 0 (" + $EA + ") -- modo " + $Modo)
  ...
  Dico ("pin ......... " + $Pin)
  Dico ("simbolo ..... " + $Simbolo + " (fisso, nessun override: questo preset non ha una gamba gemella)")
  Dico ("banco ....... MODELLO 1 (1 minuto OHLC = SCREENING) | @PERIODO " + $Periodo + " | deposito " + $Deposito + " | rischio 1,30%/evento (0,65%/ordine)") "Yellow"
  ...
  # NESSUN #define per i casi dell'autotest (a differenza di ABTG_LondonFx):
  # si RICALCOLA dal sorgente appena scaricato, sempre, non da un numero
  # scritto qui una volta per tutte.
  ...
  # -------------------------------------------------------------------
  #  4. GATE DI COERENZA COL PRESET .set
  # -------------------------------------------------------------------
  Titolo "4. GATE DI COERENZA prova <-> preset .set (31 fissi uno a uno + InpMagic lead)"
  $mappaSet = LeggiSet $presetPath
  $GemPreset = GateCoerenzaPreset $mappaSet
  Dico ("coerenza prova/preset: " + $GemPreset) "Green"

  # gate sul calendario: header esatto, 599 eventi (600 righe con l'header)
  ...
  if($incl -contains $INC){
    ...
    if($IncEraLi){ $IncBackup = $TIncMqh + ".prima_postnews_" + $Stamp; Copy-Item -LiteralPath $TIncMqh -Destination $IncBackup -Force; $bkTxt = "backup " + $IncBackup }
  ...
  if($esito.Ex5 -and $nErr -le 0){
    ...
    if($esito.Muto -or @($esito.Log).Count -eq 0){
      ...
      $Compilato = "FALLITA (MetaEditor lanciato, nessun .ex5 fresco; errori dal log: " + $(if($nErr -ge 0){ "" + $nErr } else { "NON LETTI" }) + ") -- EA MAI COMPILATO PRIMA: QUESTO E' IL RISULTATO DEL PASSO."
    }
    $k = 0
  ...
  # -------------------------------------------------------------------
  #  8. MISURA STORICO M1 USDJPY (automatica, max 2 tentativi)
  # -------------------------------------------------------------------
  Titolo "8. MISURA STORICO M1 USDJPY (ABTG_HistoryDownloader, SOLO barre, SENZA tick, max 2 tentativi)"
  if($SaltaMisuraStorico){
    ...
    while($tentativo -lt 2 -and -not $trovato){
      $tentativo++
      Dico ("tentativo " + $tentativo + "/2 di misura storico M1 USDJPY...") "Cyan"
      ...
      $StoricoCsvCopia = Join-Path $Work "ABTG_StoricoScaricato_M1_USDJPY.csv"
    ...
    if(-not $trovato -or $null -eq $verdettoRiga){
      $StoricoTxt = "FERMATO dopo " + $tentativo + " tentativi: nessuna riga USDJPY/M1 con verdetto COMPLETO o 'IL BROKER NON HA PIU' STORICO' in " + $Work + "\ABTG_StoricoScaricato_M1_USDJPY.csv. Vedi StoricoTentativi nel referto."
    ...
    if($verdettoRiga.Verdetto -eq "IL BROKER NON HA PIU' STORICO"){ [void]$Rilievi.Add("il broker NON ha M1 fino a " + $DaQuandoRichiesta + ": il muro vero e' " + $DaQuandoMisurato + ". La finestra IS/OOS parte da li', REGIME piu' corto del calendario 2010-2025 -- dichiarato, non un errore.") }
    Dico $StoricoTxt "Green"
  }
  Dico ("-DaQuando che verra' usato per la corsa: " + $DaQuandoMisurato) "Yellow"
  ...
  # -------------------------------------------------------------------
  Titolo ("10. CORSA: " + $NCELLE_ATTESE + " celle x 2 finestre, Modello 1 (OHLC M1), -DaQuando " + $DaQuandoMisurato)
  $dIni = [datetime]::ParseExact($DaQuandoMisurato,"yyyy.MM.dd",$INV)
  ...
  $tCorsa = Get-Date
  LanciaGenerico $drv $provaPath $DaQuandoMisurato
  if($SoloControllo){
...
[void]$R.Add("=====================================================================")
[void]$R.Add(" POSTNEWS NFP/USDJPY -- PASSO 0 CONTA-OCCASIONI (" + $EA + " v" + $VERSIONE_ATTESA + ")")
...
[void]$R.Add("pin:  " + $Pin)
[void]$R.Add("simbolo di questa corsa: " + $Simbolo + " (fisso, nessuna gamba gemella su questo preset)")
[void]$R.Add("finestra: IS " + $WinIS + " | OOS " + $WinOOS + " (split " + $FrazioneIS + " come lo calcola il generico) -- -DaQuando = " + $(if($DaQuandoMisurato){ $DaQuandoMisurato } else { "n/d (misura non arrivata)" }) + ", MISURATA da questa riga, mai scritta a mano")
[void]$R.Add("banco: MODELLO 1 (1 minuto OHLC, SOLO SCREENING -- un numero OHLC non e' mai un verdetto) | @PERIODO " + $Periodo + " | deposito " + $Deposito + " | rischio 1,30%/evento (0,65%/ordine su 50 pip)")
...
[void]$R.Add("")
[void]$R.Add("--- MISURA STORICO M1 USDJPY (fase 8) ---")
...
[void]$R.Add("  NESSUNA PROMOZIONE PUO' USCIRE DA QUESTA CELLA (dichiarato nel prova stesso: il campione IS non arriva a 150 nemmeno con tutto lo storico). Il giudizio possibile e' SOLO la prova di rischio (Emendamento B) e la lettura 'campione sottile' (Emendamento A/valvola R59).")
[void]$R.Add("")
[void]$R.Add("--- COSA NON SI PUO' DIRE con questi dati ---")
[void]$R.Add("  1. 'ha edge' o 'non ha edge' (campione sotto 150, dichiarato PRIMA) | 2. 'regge nel tempo' (nessuna prova di regime qui, Emendamento C fuori scopo del Passo 0)")
[void]$R.Add("  3. 'il DD sara' quello' (Modello 1 = OHLC, screening, mai un verdetto: il passo a tick reali sulla finestra recente e' il prossimo, SE questo screening interessa) | 4. promuovere qualunque cella da qui")
[void]$R.Add("  5. l'asterisco della slide: sulle 10 disoccupazioni USA di novembre (su 186, 5% del campione) l'ora d'azione fissa 13:45 server legge le candele SBAGLIATE (l'Europa ha gia' cambiato ora, gli USA no): un risultato positivo resta valido con l'asterisco, uno negativo va riletto.")
[void]$R.Add("")
...
[void]$R.Add("")
[void]$R.Add('COME SI RIPRENDE: dalla pagina righe/RIGA_POSTNEWS_NFP_DA_MANDARE.md, NON da questa')
...
$refPath = Join-Path $Cart "REFERTO_POSTNEWS_NFP_USDJPY.txt"
...
Write-Host ("FILE ATTESI NELLO ZIP: REFERTO_POSTNEWS_NFP_USDJPY.txt + COMPILAZIONE.log (+ _leggibile.txt) + il prova + il preset + ABTG_StoricoScaricato_M1_USDJPY.csv + i CSV " + $EA + "_" + $Simbolo + "_IS_ohlc_" + $Etichetta + ".csv e _OOS_ (2 righe l'uno) + gli .ini veri gen_*.ini + i log del tester cresciuti") -ForegroundColor Gray
Write-Host "NOTA: il canarino '[PostNews][NEWS] ... UTILI per questo preset N' e' nel REFERTO, letto dai log: se N=0 la corsa NON CONTA (vedi PROBLEMI/RILIEVI)." -ForegroundColor Gray

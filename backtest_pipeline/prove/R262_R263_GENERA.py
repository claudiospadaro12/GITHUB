...
#   --deposito-nuovo 100000 --rischio-base 2.0 (la chiusura del giorno e'
#   un MINORANTE del flottante intraday).
#   >>> --rischio-base 2.0 E' OBBLIGATORIO (classe 843): il default dello
#   script e' 1.0, e con quello la riga "a rischio 2.00% [DERIVATO x2.00]"
#   RADDOPPIA un per-trade che e' GIA' a 2% (provato sul per-trade 765271
#   di R247, a 1%: stampa 6,38% misurato e "a rischio 2.00%: DD ~12,76%",
#   giusto li'; su un per-trade GIA' a 2% da ~12% la stessa riga direbbe
#   ~25% "a rischio 2.00%"). Con 2.0 quella riga esce x1,00 = la misura
#   stessa. Il numero di R263e/f e' SOLO la riga
#   "DD MASSIMO del saldo chiuso", mai una riga [DERIVATO].
...
def genera(centro_ema, centro_tp):
    ...
    for L, (m1, m2), date, cosa in (
        ...
        corpo = ['#',
                 ...
                 '#  <per-trade>@100000 --deposito-nuovo 100000 --rischio-base 2.0',
                 '#  (parte (b); SENZA --rischio-base 2.0 la riga [DERIVATO] "a rischio',
                 '#  2.00%" raddoppia un per-trade gia\' a 2%: classe 843, R263a par. 5;',
                 '#  il numero e\' SOLO "DD MASSIMO del saldo chiuso"); tetto: righe',

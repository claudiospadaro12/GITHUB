# =====================================================================
#  rilancia_marco_emiliano.ps1  --  CACCIA AL MOTORE M5 con i filtri
#  di Emiliano dentro il motore di Marco (tutti ON/OFF).
#  Ipotesi: il breakout M5 nudo e' morto, ma la DIREZIONE dalla
#  correlazione S&P (regola #1 di Emiliano) + volumi + EMA lo salvano.
#  Isola i filtri uno per uno. Real tick, rischio 1%.
#
#  Sequenza:
#    1) DAX baseline  (tutti OFF)  -> sanity: deve ridare ~PF 1.49
#    2) DAX emiliano  (correlazione/volumi/EMA/direzione swept)
#    3) NAS baseline  (tutti OFF)  -> conferma Nasdaq nudo morto
#    4) NAS emiliano  (i filtri salvano il Nasdaq?)
#
#  !! IMPORTANTE per la correlazione: il simbolo S&P (InpCorrSymbol,
#     ora "SPXUSD") DEVE essere nella Market Watch BCM con storico
#     scaricato, altrimenti il filtro blocca TUTTO (0 trade = falso morto).
#     Se sul tuo broker l'S&P si chiama diverso (US500 / SP500 / SPX500),
#     DIMMELO e correggo gli .ini prima di lanciare.
#
#  Ripresa: salta le prove gia' salvate in .\risultati_marco_emiliano\
# =====================================================================
...
$Targets=@(
  @{ini="valid_Marco_DAX_base";     sym="D30EUR"},
  @{ini="valid_Marco_DAX_emiliano"; sym="D30EUR"},
  @{ini="valid_Marco_NAS_base";     sym="NASUSD"},
  @{ini="valid_Marco_NAS_emiliano"; sym="NASUSD"}
)
$Work= if($PSScriptRoot){$PSScriptRoot}else{(Get-Location).Path}; Set-Location $Work
Write-Host "=== CACCIA MOTORE M5: filtri Emiliano nel motore Marco (real tick) ===" -ForegroundColor Cyan
...
$Results=Join-Path $Work "risultati_marco_emiliano"; New-Item -ItemType Directory -Force -Path $MqlExperts,$Results|Out-Null
...
foreach($t in $Targets){
  ...
  if(Test-Path $csv){Copy-Item $csv -Destination $done -Force; Remove-Item $csv -Force; Write-Host ("        OK -> $($t.ini).csv" -f $t.sym) -ForegroundColor Green}
  else{Write-Host "        (manca il CSV: MT5 aperto? o simbolo S&P non in Market Watch?)" -ForegroundColor Yellow}
}
Write-Host "`n=== FINITO === risultati in $Results" -ForegroundColor Cyan
Write-Host "Mandami i 4 CSV. Confronto: se con correlazione/volumi il PF sale (specie sul Nasdaq), abbiamo trovato il motore M5." -ForegroundColor White
